...
def upgrade() -> None:
    ...
    op.add_column(
        "flow_executions", sa.Column("group_id", sa.String(length=100), nullable=True)
    )
    op.create_index(
        op.f("ix_flow_executions_group_id"),
        "flow_executions",
        ["group_id"],
        unique=False,
    )
    ...
    op.create_index(
        op.f("ix_flow_node_executions_group_id"),
        "flow_node_executions",
        ["group_id"],
        unique=False,
    )
...
def downgrade() -> None:
    ...
    op.drop_index(
        op.f("ix_flow_node_executions_group_id"), table_name="flow_node_executions"
    )
