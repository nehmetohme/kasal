...
def upgrade() -> None:
    ...
    for table_name in tables_to_update:
        ...
        result = conn.execute(
            sa.text(
                f"SELECT column_name FROM information_schema.columns WHERE table_name='{table_name}' AND column_name='tenant_id'"
            )
        )
        ...
        result = conn.execute(
            sa.text(
                f"SELECT column_name FROM information_schema.columns WHERE table_name='{table_name}' AND column_name='created_by_email'"
            )
        )
        if not result.fetchone():
            ...
            try:
                with op.batch_alter_table(table_name, schema=None) as batch_op:
                    batch_op.add_column(
                        sa.Column(
                            "created_by_email", sa.String(length=255), nullable=True
                        )
                    )
                    batch_op.create_index(
                        f"ix_{table_name}_created_by_email", ["created_by_email"]
                    )
            except Exception as e:
        ...
        if table_name in ["executionhistory", "execution_trace", "flow_executions"]:
            result = conn.execute(
                sa.text(
                    f"SELECT column_name FROM information_schema.columns WHERE table_name='{table_name}' AND column_name='tenant_email'"
                )
            )
            if not result.fetchone():
                ...
                try:
                    with op.batch_alter_table(table_name, schema=None) as batch_op:
                        batch_op.add_column(
                            sa.Column(
                                "tenant_email", sa.String(length=255), nullable=True
                            )
                        )
                        batch_op.create_index(
                            f"ix_{table_name}_tenant_email", ["tenant_email"]
                        )
                except Exception as e:
