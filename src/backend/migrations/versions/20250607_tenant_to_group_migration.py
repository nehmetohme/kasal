...
def upgrade():
    ...
    for table_name in tables_needing_group_id:
        try:
            ...
            if "group_id" not in columns:
                # Add group_id column
                op.add_column(
                    table_name, sa.Column("group_id", sa.String(100), nullable=True)
                )
                ...
                if "tenant_id" in columns:
                    connection.execute(
                        sa.text(
                            f"UPDATE {table_name} SET group_id = tenant_id WHERE tenant_id IS NOT NULL"
                        )
                    )
    ...
    tables_needing_group_email = ["execution_logs", "executionhistory", "llmlog"]
    ...
    for table_name in tables_needing_group_email:
        try:
            ...
            if "group_email" not in columns:
                op.add_column(
                    table_name, sa.Column("group_email", sa.String(255), nullable=True)
                )
                op.create_index(
                    f"ix_{table_name}_group_email", table_name, ["group_email"]
                )
                ...
                if "tenant_email" in columns:
                    connection.execute(
                        sa.text(
                            f"UPDATE {table_name} SET group_email = tenant_email WHERE tenant_email IS NOT NULL"
                        )
                    )
...
def downgrade():
    ...
    tables_with_group_email = ["execution_logs", "executionhistory", "llmlog"]
