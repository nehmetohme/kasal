...
def upgrade() -> None:
    ...
    op.add_column(
        "schedule",
        sa.Column(
            "execution_type", sa.String(20), nullable=False, server_default="crew"
        ),
    )
    ...
    op.add_column(
        "schedule", sa.Column("flow_id", postgresql.UUID(as_uuid=True), nullable=True)
    )

    # Add nodes column (JSON for flow node configuration)
    op.add_column("schedule", sa.Column("nodes", sa.JSON(), nullable=True))

    # Add edges column (JSON for flow edge configuration)
    op.add_column("schedule", sa.Column("edges", sa.JSON(), nullable=True))

    # Add flow_config column (JSON for flow-specific configuration)
    op.add_column("schedule", sa.Column("flow_config", sa.JSON(), nullable=True))
