...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ...
            name="fk_hitl_approvals_execution_id",
        ),
    )
    ...
    op.create_index(
        "ix_hitl_approvals_execution_id", "hitl_approvals", ["execution_id"]
    )
    op.create_index("ix_hitl_approvals_flow_id", "hitl_approvals", ["flow_id"])
    op.create_index(
        "ix_hitl_approvals_gate_node_id", "hitl_approvals", ["gate_node_id"]
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "events", sa.JSON(), nullable=False, server_default='["gate_reached"]'
        ),
