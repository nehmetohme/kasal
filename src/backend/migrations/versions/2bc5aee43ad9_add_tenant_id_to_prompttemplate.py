...
def upgrade() -> None:
    ...
    result = conn.execute(
        sa.text(
            "SELECT column_name FROM information_schema.columns WHERE table_name='prompttemplate' AND column_name='tenant_id'"
        )
    )
    ...
    result = conn.execute(
        sa.text(
            "SELECT column_name FROM information_schema.columns WHERE table_name='prompttemplate' AND column_name='created_by_email'"
        )
    )
    if not result.fetchone():
        ...
        with op.batch_alter_table("prompttemplate", schema=None) as batch_op:
            ...
            batch_op.create_index(
                "ix_prompttemplate_created_by_email", ["created_by_email"]
            )
    else:
