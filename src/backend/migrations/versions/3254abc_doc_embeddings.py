...
def upgrade() -> None:
    ...
    try:
        connection.execute(
            sa.text(
                "CREATE INDEX IF NOT EXISTS ix_documentation_embeddings_source ON documentation_embeddings (source)"
            )
        )
        connection.execute(
            sa.text(
                "CREATE INDEX IF NOT EXISTS ix_documentation_embeddings_title ON documentation_embeddings (title)"
            )
        )
    ...
    try:
        ...
        logger.warning(
            "The table has been created, but vector similarity search will not be available."
        )
        logger.warning(
            "You can install pgvector manually and create the index later if needed."
        )
