...
def upgrade() -> None:
    ...
    connection.execute(
        sa.text(
            "ALTER TABLE documentation_embeddings ALTER COLUMN embedding TYPE vector(1536) USING NULL"
        )
    )
...
def downgrade() -> None:
    ...
    try:
        connection.execute(
            sa.text("DROP INDEX IF EXISTS documentation_embeddings_embedding_idx")
        )
    except Exception as e:
    ...
    connection.execute(
        sa.text(
            "ALTER TABLE documentation_embeddings ALTER COLUMN embedding TYPE float[] USING NULL"
        )
    )
