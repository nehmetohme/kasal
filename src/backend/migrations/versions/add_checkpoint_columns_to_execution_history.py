...
def upgrade() -> None:
    ...
    op.add_column(
        "executionhistory", sa.Column("flow_uuid", sa.String(255), nullable=True)
    )
    op.add_column(
        "executionhistory", sa.Column("checkpoint_status", sa.String(50), nullable=True)
    )
    op.add_column(
        "executionhistory",
        sa.Column("checkpoint_method", sa.String(255), nullable=True),
    )

    # Create index on flow_uuid for efficient lookups
    op.create_index(
        op.f("ix_executionhistory_flow_uuid"),
        "executionhistory",
        ["flow_uuid"],
        unique=False,
    )
