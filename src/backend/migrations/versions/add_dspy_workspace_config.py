...
def upgrade() -> None:
    ...
    table_name = (
        "databricks_configs"
        if "databricks_configs" in tables
        else "databricksconfig" if "databricksconfig" in tables else None
    )
    if not table_name:
    ...
    with op.batch_alter_table(table_name, schema=None) as batch_op:
        ...
        batch_op.add_column(
            sa.Column(
                ...
                sa.Boolean(),
                nullable=True,
                server_default=sa.text(def_bool),
            )
        )
        batch_op.add_column(
            sa.Column(
                "dspy_intent_detection",
                sa.Boolean(),
                nullable=True,
                server_default=sa.text(def_true),
            )
        )
        batch_op.add_column(
            sa.Column(
                "dspy_agent_generation",
                sa.Boolean(),
                nullable=True,
                server_default=sa.text(def_true),
            )
        ...
        batch_op.add_column(
            sa.Column(
                "dspy_min_examples", sa.Integer(), nullable=True, server_default="10"
            )
        )
        batch_op.add_column(
            sa.Column(
