...
def upgrade():
    ...
    op.execute(
        "UPDATE databricksconfig SET knowledge_volume_enabled = FALSE WHERE knowledge_volume_enabled IS NULL"
    )
    op.execute(
        "UPDATE databricksconfig SET knowledge_volume_path = 'main.default.knowledge' WHERE knowledge_volume_path IS NULL"
    )
    op.execute(
        "UPDATE databricksconfig SET knowledge_chunk_size = 1000 WHERE knowledge_chunk_size IS NULL"
    )
    op.execute(
        "UPDATE databricksconfig SET knowledge_chunk_overlap = 200 WHERE knowledge_chunk_overlap IS NULL"
    )
