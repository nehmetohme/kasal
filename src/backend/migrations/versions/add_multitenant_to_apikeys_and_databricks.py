...
def upgrade():
    ...
    if "apikey" not in inspector.get_table_names():
        ...
        op.create_index(
            "ix_apikey_created_by_email", "apikey", ["created_by_email"], unique=False
        )
        op.create_unique_constraint(
            "uq_apikey_name_group", "apikey", ["name", "group_id"]
        )
    else:
        # Add columns to existing apikey table
        op.add_column(
            "apikey", sa.Column("group_id", sa.String(length=100), nullable=True)
        )
        op.add_column(
            "apikey",
            sa.Column("created_by_email", sa.String(length=255), nullable=True),
    ...
    if "databricksconfig" not in inspector.get_table_names():
        ...
        op.create_index(
            "ix_databricksconfig_group_id",
            "databricksconfig",
            ["group_id"],
            unique=False,
        )
        op.create_index(
            "ix_databricksconfig_created_by_email",
            "databricksconfig",
            ["created_by_email"],
            unique=False,
        )
