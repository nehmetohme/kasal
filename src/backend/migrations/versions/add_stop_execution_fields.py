...
def upgrade() -> None:
    ...
    op.add_column(
        "executionhistory", sa.Column("stopped_at", sa.DateTime(), nullable=True)
    )
    op.add_column(
        "executionhistory", sa.Column("stop_reason", sa.String(), nullable=True)
    )
    op.add_column(
        "executionhistory",
    ...
    op.add_column(
        "executionhistory",
        sa.Column(
            "partial_results", postgresql.JSON(astext_type=sa.Text()), nullable=True
        ),
    )
    op.add_column(
        "executionhistory",
        sa.Column("is_stopping", sa.Boolean(), nullable=False, server_default="false"),
    )

    # Create index on is_stopping for quick queries of stopping executions
    op.create_index(
        "idx_executionhistory_is_stopping",
        "executionhistory",
        ["is_stopping"],
        unique=False,
        postgresql_where=sa.text("is_stopping = TRUE"),
    )
