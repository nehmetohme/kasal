...
def upgrade():
    ...
    with op.batch_alter_table("execution_logs", schema=None) as batch_op:
        ...
        batch_op.create_index(
            "idx_execution_logs_tenant_timestamp", ["tenant_id", "timestamp"]
        )
        batch_op.create_index(
            "idx_execution_logs_tenant_exec_id", ["tenant_id", "execution_id"]
        )
...
def downgrade():
    ...
    try:
        op.drop_constraint(
            "uq_tenant_users_tenant_user", "tenant_users", type_="unique"
        )
    except Exception:
        pass
