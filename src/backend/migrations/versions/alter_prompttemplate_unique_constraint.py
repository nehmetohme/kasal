    ...
    try:
        ...
        for (conname,) in rows:
            try:
                conn.execute(
                    sa.text(
                        f"ALTER TABLE prompttemplate DROP CONSTRAINT IF EXISTS {conname}"
                    )
                )
            except Exception:
                pass
...
def downgrade() -> None:
    ...
    if dialect == "sqlite":
        ...
        try:
            op.drop_constraint(
                "uq_prompttemplate_name_group", "prompttemplate", type_="unique"
            )
        except Exception:
            pass
        ...
        try:
            op.create_index(
                "ix_prompttemplate_name", "prompttemplate", ["name"], unique=True
            )
        except Exception:
            pass
    else:
        ...
        for idx in [
            "uq_prompttemplate_name_base",
            "uq_prompttemplate_name_group_not_null",
        ]:
            try:
        ...
        try:
            op.create_unique_constraint(
                "uq_prompttemplate_name", "prompttemplate", ["name"]
            )
        except Exception:
            pass
