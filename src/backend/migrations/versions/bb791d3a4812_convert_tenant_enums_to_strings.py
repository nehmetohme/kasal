...
def upgrade() -> None:
    ...
    with op.batch_alter_table("tenants", schema=None) as batch_op:
        batch_op.alter_column(
            ...
            existing_type=sa.Enum(
                "ACTIVE", "SUSPENDED", "ARCHIVED", name="tenantstatus"
            ),
    ...
    with op.batch_alter_table("tenant_users", schema=None) as batch_op:
        batch_op.alter_column(
            "role",
            existing_type=sa.Enum(
                "ADMIN", "MANAGER", "USER", "VIEWER", name="tenantuserrole"
            ),
            type_=sa.String(length=50),
            existing_nullable=False,
    ...
    with op.batch_alter_table("tenant_users", schema=None) as batch_op:
        batch_op.alter_column(
            "status",
            existing_type=sa.Enum(
                "ACTIVE", "INACTIVE", "SUSPENDED", name="tenantuserstatus"
            ),
            type_=sa.String(length=50),
            existing_nullable=False,
            existing_server_default="ACTIVE",
...
def downgrade() -> None:
    ...
    tenantuserrole_enum = sa.Enum(
        "ADMIN", "MANAGER", "USER", "VIEWER", name="tenantuserrole"
    )
    tenantuserrole_enum.create(op.get_bind())

    tenantuserstatus_enum = sa.Enum(
        "ACTIVE", "INACTIVE", "SUSPENDED", name="tenantuserstatus"
    )
