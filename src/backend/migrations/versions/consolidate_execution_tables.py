...
def upgrade() -> None:
    ...
    op.add_column(
        "executionhistory",
        sa.Column(
            "execution_type", sa.String(20), nullable=True, server_default="crew"
        ),
    )
    op.create_index(
        op.f("ix_executionhistory_execution_type"),
        "executionhistory",
        ["execution_type"],
        unique=False,
    )
    ...
    op.create_index(
        op.f("ix_executionhistory_flow_id"),
        "executionhistory",
        ["flow_id"],
        unique=False,
    )
...
def downgrade() -> None:
    ...
    op.drop_index(
        op.f("ix_executionhistory_execution_type"), table_name="executionhistory"
    )
