...
def upgrade() -> None:
    ...
    op.add_column(
        "executionhistory", sa.Column("mlflow_trace_id", sa.String(), nullable=True)
    )
    op.add_column(
        "executionhistory",
        sa.Column("mlflow_experiment_name", sa.String(), nullable=True),
    )

    # Add index on mlflow_trace_id for faster lookups
    op.create_index(
        "ix_executionhistory_mlflow_trace_id", "executionhistory", ["mlflow_trace_id"]
    )
