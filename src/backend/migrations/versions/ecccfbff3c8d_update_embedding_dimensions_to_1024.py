...
def upgrade() -> None:
    ...
    op.execute(
        "ALTER TABLE documentation_embeddings ALTER COLUMN embedding TYPE vector(1024)"
    )
...
def downgrade() -> None:
    ...
    op.execute(
        "ALTER TABLE documentation_embeddings ALTER COLUMN embedding TYPE vector(1536)"
    )
