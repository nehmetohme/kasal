...
def upgrade():
    ...
    op.drop_constraint(
        "flow_executions_flow_id_fkey", "flow_executions", type_="foreignkey"
    )

    # Make flow_id nullable
    op.alter_column(
        "flow_executions", "flow_id", existing_type=postgresql.UUID(), nullable=True
    )
...
def downgrade():
    ...
    op.alter_column(
        "flow_executions", "flow_id", existing_type=postgresql.UUID(), nullable=False
    )

    # Re-add the foreign key constraint
    op.create_foreign_key(
        "flow_executions_flow_id_fkey", "flow_executions", "flows", ["flow_id"], ["id"]
    )
