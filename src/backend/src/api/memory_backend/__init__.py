...
- ``vectorsearch_router`` — Databricks Vector Search endpoints/indexes
- ``lakebase_router``   — Lakebase (pgvector) connection + tables
- ``records_router``    — memory contents: stats, clear, unified record browser

All four share one service provider (``dependencies.get_memory_backend_service``),
...
router.include_router(vectorsearch_router.router)
