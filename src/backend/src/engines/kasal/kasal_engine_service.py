...
class KasalEngineService(BaseEngineService):
    ...
    async def run_execution(self, execution_id: str, execution_config: Dict[str, Any], group_context: GroupContext = None, session = None) -> str:
        ...
        try:
            ...
            if group_context and group_context.primary_group_id:
                ...
                logger.info(f"[KasalEngineService] Added group_id to config: {group_context.primary_group_id}")
            ...
            logger.info(f"[KasalEngineService] Processing {len(agent_configs)} agents for execution {execution_id}")
            for idx, agent_config in enumerate(agent_configs):
                agent_id = agent_config.get('id', f'agent_{idx}')
                logger.info(f"[KasalEngineService] Agent {agent_id} config keys: {list(agent_config.keys())}")
                if 'knowledge_sources' in agent_config:
                    ks = agent_config['knowledge_sources']
                    logger.info(f"[KasalEngineService] Agent {agent_id} has {len(ks)} knowledge_sources: {ks}")
                else:
                    logger.debug(f"[KasalEngineService] Agent {agent_id} has NO knowledge_sources")
            ...
            try:
                ...
                logger.info(f"[KasalEngineService] DEBUG: Config before subprocess for {execution_id}:")
                ...
                logger.error(f"[KasalEngineService] Error running CrewAI execution {execution_id}: {str(e)}", exc_info=True)
                try:
                    ...
                    logger.critical(f"[KasalEngineService] CRITICAL: Failed to update status to FAILED for {execution_id} after run_execution error: {update_err}", exc_info=True)
                raise
            
            # Event listeners are now initialized in the subprocess
            # This ensures they're in the same process as the crew execution
            logger.debug(f"[KasalEngineService] Event listeners will be initialized in subprocess for {execution_id}")
            
            # Status is already RUNNING from creation, no need to update
            logger.info(f"[KasalEngineService] Execution {execution_id} ready to start (status already RUNNING)")
            ...
            logger.info(f"[KasalEngineService] Starting process-based execution for {execution_id}")
            
            # Create a task for process-based crew execution with exception handler
            async def run_with_exception_handler():
                run_succeeded = False
                try:
                    logger.info(f"[KasalEngineService] About to call run_crew_in_process for {execution_id}")
                    ...
                    run_succeeded = True
                    logger.info(f"[KasalEngineService] run_crew_in_process completed for {execution_id}")
                except Exception as e:
                    logger.error(f"[KasalEngineService] CRITICAL: Exception in run_crew_in_process for {execution_id}: {e}", exc_info=True)
            ...
            logger.info(f"[KasalEngineService] Created execution task for {execution_id}")
    ...
    async def run_flow(self, execution_id: str, flow_config: Dict[str, Any], group_context: GroupContext = None, user_token: str = None) -> str:
        ...
        try:
            ...
            # Status is already RUNNING from creation, no need to update
            flow_logger.info(f"[KasalEngineService] Execution {execution_id} ready to start flow (status already RUNNING)")

            # Use process-based execution for true termination capability
            flow_logger.info(f"[KasalEngineService] Starting process-based flow execution for {execution_id}")

            # Create a task for process-based flow execution with exception handler
            async def run_with_exception_handler():
                try:
                    flow_logger.info(f"[KasalEngineService] About to call run_flow_in_process for {execution_id}")
                    ...
                    flow_logger.info(f"[KasalEngineService] run_flow_in_process completed for {execution_id}")
                except Exception as e:
                    flow_logger.error(f"[KasalEngineService] CRITICAL: Exception in run_flow_in_process for {execution_id}: {e}", exc_info=True)
                    # Write to file as backup
                    import traceback
            ...
            flow_logger.info(f"[KasalEngineService] Created flow execution task for {execution_id}")
            ...
            flow_logger.info(f"[KasalEngineService] Stored job info for {execution_id} in running_jobs")

            return execution_id

        except Exception as e:
            flow_logger.error(f"[KasalEngineService] Error in run_flow for {execution_id}: {str(e)}", exc_info=True)
            await self._update_execution_status(
                execution_id,
                ExecutionStatus.FAILED.value,
