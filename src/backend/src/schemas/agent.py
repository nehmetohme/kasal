...
class AgentBase(BaseModel):
    ...
    max_rpm: Optional[int] = Field(
        default=10, description="Maximum requests per minute to avoid rate limits"
    )
    ...
    inject_date: bool = Field(
        default=True,
        description="Injects current date into agent's context for time-sensitive tasks. Enabled by default.",
    )
    date_format: Optional[str] = Field(
        default=None,
        description="Custom date format string (e.g., '%B %d, %Y' for 'February 05, 2026'). Defaults to ISO format if not specified.",
    )
