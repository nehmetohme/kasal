...
class TaskContext(BaseModel):
    ...
    required_skills: Optional[List[str]] = Field(
        None, description="Skills required for the task"
    )
...
class Task(BaseModel):
    ...
    expected_output: Optional[str] = Field(
        None, description="Expected output from the task"
    )
...
class ConnectionRequest(BaseModel):
    ...
    agents: List[Agent] = Field(
        ..., description="List of agents available for task assignment"
    )
    tasks: List[Task] = Field(..., description="List of tasks to be assigned to agents")
    model: str = Field(
        "gpt-4-turbo", description="LLM model to use for generating connections"
    )
    instructions: Optional[str] = Field(
        None, description="Additional instructions for connection generation"
    )
...
class TaskAssignment(BaseModel):
    ...
    reasoning: str = Field(
        ..., description="Reasoning for assigning this task to the agent"
    )
...
class ApiKeyTestResponse(BaseModel):
    ...
    anthropic: ApiKeyTestResult = Field(
        ..., description="Anthropic API key test results"
    )
