...
class LogMessage(BaseModel):
    ...
    execution_id: str = Field(
        ..., description="ID of the execution this log relates to"
    )
    ...
    type: Literal["live", "historical"] = Field(
        ..., description="Type of message (live or historical)"
    )
...
class ExecutionLogResponse(BaseModel):
    ...
    content: str = Field(..., description="Content of the log message")
    timestamp: str = Field(
        ..., description="ISO-formatted timestamp when the log was created"
    )
