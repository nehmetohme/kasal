...
class SchedulerJobBase(BaseModel):
    ...
    job_data: Dict[str, Any] = Field(
        default_factory=dict, description="Job specific configuration data"
    )
...
class SchedulerJobSchema(SchedulerJobBase):
    ...
    last_run_at: Optional[datetime] = Field(
        None, description="When the job was last run"
    )
    next_run_at: Optional[datetime] = Field(
        None, description="When the job will run next"
    )
