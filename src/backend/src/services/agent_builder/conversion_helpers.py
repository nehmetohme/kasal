...
def extract_crew_yaml_data(
    ...
    for agent_id, agent_config in agents_yaml.items():
        ...
        logger.info(
            f"[extract_crew_yaml_data] Agent {agent_id} keys: {list(agent_config.keys())}"
        )
        ...
        if "knowledge_sources" in agent_config:
            ...
            logger.info(
                f"[extract_crew_yaml_data] Agent {agent_id} has {len(knowledge_sources)} knowledge_sources"
            )
            for idx, source in enumerate(knowledge_sources):
                logger.info(
                    f"[extract_crew_yaml_data] Agent {agent_id} knowledge_source[{idx}]: {source}"
                )
        else:
            logger.debug(
                f"[extract_crew_yaml_data] Agent {agent_id} has NO knowledge_sources field"
            )
    ...
    for task_id, task_config in tasks_yaml.items():
        ...
        logger.info(
            f"[extract_crew_yaml_data] Task {task_id} keys: {list(task_config.keys())}"
        )
        ...
        if "tool_configs" in task_config:
            ...
            logger.info(
                f"[extract_crew_yaml_data] Task {task_id} has tool_configs with keys: {list(tool_configs.keys())}"
            )
            ...
            for tool_name, tool_cfg in tool_configs.items():
                ...
                logger.info(
                    f"[extract_crew_yaml_data] Task {task_id} tool_config[{tool_name}]: {safe_cfg}"
                )
        else:
            logger.info(
                f"[extract_crew_yaml_data] Task {task_id} has NO tool_configs field"
            )

        tasks_data.append(task_data)

    logger.info(
        f"[extract_crew_yaml_data] Extraction complete: {len(agents_data)} agents, {len(tasks_data)} tasks"
    )
