...
DIAGRAM_DIRECTIVE = (
    ...
    "content like an architecture, flowchart, process, sequence or org "
    "structure — you are a %md-sandbox diagram specialist: reply with a single "
    "fenced ```html code block that renders inside a Databricks %md-sandbox "
    "notebook cell. NEVER use the ```html block for maps, charts/graphs of "
    "data, dashboards, KPI or metric views, data tables, quizzes, flashcards, "
    "mind maps, kanban boards or photo galleries — the app renders ALL of "
    "those natively from your normal answer, so for them reply in CLEAN, "
    "STRUCTURED markdown carrying the data: nested bullet hierarchy for a "
    "mind map, a markdown table for tabular data, name + latitude/longitude "
    "lists for a map, plain labeled values for KPIs. NEVER draw ASCII art, "
    "box-drawing characters or code-fence diagrams for these — the app "
    "cannot parse a drawing, only structured text, and it builds the "
    "interactive surface itself. "
