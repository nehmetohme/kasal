...
class StructureExpander:
    ...
    def _create_combined_measures(
        ...
        for struct_name in structure_names:
            ...
            combined_formula = self._combine_formula_and_structure(
                base_kbi, structure, structures
            )
            ...
            if structure.formula:
                ...
                aggregation_type = (
                    structure.aggregation_type or base_kbi.aggregation_type
                )
            ...
            display_sign = (
                structure.display_sign
                if structure.display_sign is not None
    ...
    def _combine_formula_and_structure(
        self, base_kbi: KPI, structure: Structure, all_structures: Dict[str, Structure]
    ) -> str:
        """
    ...
    def _resolve_structure_references(
        self, formula: str, base_kbi: KPI, all_structures: Dict[str, Structure]
    ) -> str:
        """
        ...
        for struct_name in definition.structures.keys():
            if struct_name not in visited:
                if has_circular_dependency(struct_name, visited, set()):
                    errors.append(
                        f"Circular dependency detected involving structure: {struct_name}"
                    )
            ...
            if kpi.apply_structures:
                for struct_name in kpi.apply_structures:
                    if struct_name not in definition.structures:
                        errors.append(
                            f"KPI '{kpi.technical_name or kpi.description}' references undefined structure: {struct_name}"
                        )
...
class TimeIntelligenceHelper:
    ...
    def create_py_structure() -> Structure:
        ...
        return Structure(
            ...
            filters=["( fiscyear = $var_prior_year )", "( bic_chversion = '0000' )"],
            display_sign=1,
        )
