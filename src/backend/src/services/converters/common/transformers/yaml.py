...
from ...base.models import KPI, KPIDefinition, QueryFilter, Structure
        ...
        if "structures" in data:
            ...
            for struct_name, struct_data in data["structures"].items():
                ...
                with open("/tmp/sql_debug.log", "a") as f:
                    f.write(
                        f"YAML Parser - Structure {struct_name}: raw filter data = {filter_data}\n"
                    )
                ...
                with open("/tmp/sql_debug.log", "a") as f:
                    f.write(
                        f"YAML Parser - Structure {struct_name}: created with filters = {structure.filters}\n"
                    )
        ...
        if "kbi" in data:
            for kbi_data in data["kbi"]:
                kbi = KPI(
                    ...
                    fields_for_exception_aggregation=kbi_data.get(
                        "fields_for_exception_aggregation"
                    ),
                    fields_for_constant_selection=kbi_data.get(
