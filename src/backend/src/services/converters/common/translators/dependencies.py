...
class DependencyResolver:
    ...
    def get_dependency_order(self) -> List[str]:
        ...
        if len(result) != len(self.measure_registry):
            ...
            raise ValueError(
                f"Circular dependencies detected among measures: {remaining}"
            )
    ...
    def _resolve_recursive(
        self, measure_name: str, visited: Set[str], max_depth: int
    ) -> str:
        """Recursively resolve measure dependencies"""
        if max_depth <= 0:
            raise ValueError(
                f"Maximum recursion depth reached while resolving '{measure_name}'"
            )

        if measure_name in visited:
            raise ValueError(
                f"Circular dependency detected: {' -> '.join(visited)} -> {measure_name}"
            )
        ...
        for dep in dependencies:
            ...
            resolved_formula = re.sub(
                r"\b" + re.escape(dep) + r"\b", f"({dep_dax})", resolved_formula
            )
    ...
    def _generate_leaf_measure_dax(self, measure: KPI) -> str:
        ...
        from ...formats.powerbi.helpers.dax_aggregations import (
        ...
        kbi_dict = {
            ...
            "fields_for_exception_aggregation": measure.fields_for_exception_aggregation
            or [],
            "fields_for_constant_selection": measure.fields_for_constant_selection
            or [],
        }
