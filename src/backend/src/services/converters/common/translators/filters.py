...
class FilterResolver:
    ...
    def resolve_filters(self, kpi: KPI, definition: KPIDefinition) -> List[str]:
        ...
        for filter_item in kpi.filters:
            if isinstance(filter_item, str):
                ...
                resolved_filter = self._resolve_variables(
                    filter_item, definition.default_variables
                )
                resolved_filter = self._resolve_query_filters(
                    resolved_filter,
        ...
        if "$query_filter" in filter_text:
            ...
            if query_filters:
                ...
                for qf in query_filters:
                    ...
                    if variables:
                        resolved_expr = self._resolve_variables(
                            resolved_expr, variables
                        )
    ...
    def _resolve_complex_filter(
        self, filter_dict: Dict[str, Any], definition: KPIDefinition
    ) -> str:
        """Convert complex filter dictionary to DAX-compatible string."""
        if (
            "field" in filter_dict
            and "operator" in filter_dict
            and "value" in filter_dict
        ):
        ...
        if isinstance(filter_dict, str):
            resolved = self._resolve_variables(
                filter_dict, definition.default_variables
            )
            return self._resolve_query_filters(
                resolved, definition.query_filters, definition.default_variables
            )
