...
class SmartDAXGenerator:
    ...
    def generate_dax_measure(self, definition: KPIDefinition, kpi: KPI) -> DAXMeasure:
        ...
        if self._has_dependencies(definition):
            ...
            measures = self.tree_generator.generate_measure_with_separate_dependencies(
                definition, kpi.technical_name
            )
    ...
    def generate_all_measures(self, definition: KPIDefinition) -> List[DAXMeasure]:
        ...
        if self._has_dependencies(definition):
            ...
            for kpi in definition.kpis:
                measures.append(
                    self.standard_generator.generate_dax_measure(definition, kpi)
                )
            return measures

    def generate_measures_with_dependencies(
        ...
        return self.tree_generator.generate_measure_with_separate_dependencies(
            definition, target_measure_name
        )
    ...
    def _has_dependencies(self, definition: KPIDefinition) -> bool:
        ...
        for (
            measure_name,
            deps,
        ) in self.tree_generator.dependency_resolver.dependency_graph.items():
    ...
    def get_analysis_report(self, definition: KPIDefinition) -> dict:
        ...
        if report["has_dependencies"]:
            ...
            report.update(
                {
                    "complexity": self.tree_generator.get_measure_complexity_report(
                        definition
                    )
                }
            )
