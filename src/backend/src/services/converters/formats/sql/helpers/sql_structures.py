...
class SQLStructureExpander:
    ...
    def process_definition(
        ...
        with open("/tmp/sql_debug.log", "a") as f:
            ...
            if definition.kpis:
                ...
                for kpi in definition.kpis:
                    f.write(
                        f"KPI {kpi.technical_name}: apply_structures={kpi.apply_structures}\n"
                    )
        """
        ...
        if not definition.structures:
            ...
            sql_definition.sql_measures = self._convert_kbis_to_sql_measures(
                definition.kpis, definition, options
            )
            return sql_definition

        # Convert SAP BW structures to SQL structures
        sql_structures = self._convert_structures_to_sql(
            definition.structures, definition
        )
        ...
        self.logger.info(
            f"Found {len(self._base_kbi_contexts)} unique base KBI contexts"
        )
        ...
        if self._is_base_kbi(kbi):
            ...
            if parent_kbis:
                for i in range(len(parent_kbis)):
                    partial_context = SQLBaseKBIContext.get_kbi_context(
                        kbi, parent_kbis[i:]
                    )
    ...
    def _convert_structures_to_sql(
        self, structures: Dict[str, Structure], definition: KPIDefinition
    ) -> Dict[str, SQLStructure]:
        """Convert SAP BW structures to SQL structures"""
        sql_structures = {}
        ...
        for struct_name, structure in structures.items():
            ...
            converted_filters = self._convert_filters_to_sql(
                structure.filters, definition
            )
            ...
            if self._is_time_intelligence_structure(struct_name, structure):
                sql_structure = self._enhance_time_intelligence_sql_structure(
                    sql_structure, struct_name, structure
                )
    ...
    def _is_time_intelligence_structure(
        self, struct_name: str, structure: Structure
    ) -> bool:
    ...
    def _enhance_time_intelligence_sql_structure(
        self, sql_structure: SQLStructure, struct_name: str, structure: Structure
    ) -> SQLStructure:
        ...
        if "ytd" in struct_name_lower:
            ...
            sql_structure.sql_template = self._create_prior_period_sql_template(
                date_column
            )
    ...
    def _convert_filters_to_sql(
        ...
        with open("/tmp/sql_debug.log", "a") as f:
            f.write(
                f"_convert_filters_to_sql: definition.filters = {definition.filters}\n"
            )
    ...
    def _convert_kbis_to_sql_measures(
        self, kpis: List[KPI], definition: KPIDefinition, options: SQLTranslationOptions
    ) -> List[SQLMeasure]:
    ...
    def _create_combined_sql_measures(
        self,
        base_kbi: KPI,
        ...
        for struct_name in structure_names:
            if struct_name not in sql_structures:
                self.logger.warning(
                    f"SQL structure '{struct_name}' not found, skipping"
                )
                continue
            ...
            base_name = base_kbi.technical_name or self._generate_technical_name(
                base_kbi.description
            )
            combined_name = f"{base_name}_{struct_name}"

            # Create combined KBI for processing
            combined_kbi = self._create_combined_kbi(
                base_kbi, sql_structure, combined_name, definition
            )

            # Convert to SQL measure
            combined_sql_measure = self._convert_kbi_to_sql_measure(
                combined_kbi, definition, options
            )

            # Update description to reflect combination
            combined_sql_measure.name = (
                f"{base_kbi.description} - {sql_structure.description}"
            )
            combined_sql_measure.description = (
                f"{base_kbi.description} - {sql_structure.description}"
            )
    ...
    def _create_combined_kbi(
        ...
        display_sign = (
            sql_structure.display_sign
            if sql_structure.display_sign != 1
            else base_kbi.display_sign
        )
    ...
    def _resolve_structure_formula_in_sql(
        self,
        sql_measure: SQLMeasure,
        sql_structure: SQLStructure,
        ...
        sql_measure.aggregation_type = (
            SQLAggregationType.SUM
        )  # Will be wrapped in calculation logic
    ...
    def generate_sql_queries_from_definition(
        ...
        if options.separate_measures:
            ...
            for sql_measure in sql_definition.sql_measures:
                query = self._create_query_for_sql_measure(
                    sql_measure, sql_definition, options
                )
                queries.append(query)
        else:
            # Generate combined query for all measures
            if sql_definition.sql_measures:
                combined_query = self._create_combined_sql_query(
                    sql_definition.sql_measures, sql_definition, options
                )
                queries.append(combined_query)

        return queries

    def _create_query_for_sql_measure(
        self,
        sql_measure: SQLMeasure,
        sql_definition: SQLDefinition,
        options: SQLTranslationOptions,
        ...
        if (
            hasattr(sql_measure, "original_kbi")
            and sql_measure.original_kbi
            and sql_measure.original_kbi.aggregation_type == "EXCEPTION_AGGREGATION"
        ):
            # This is an exception aggregation - handle it specially
            return self._create_exception_aggregation_query(
                sql_measure, sql_definition, options
            )
        ...
        if hasattr(sql_measure, "original_kbi") and sql_measure.original_kbi:
            const_selection_fields = (
                sql_measure.original_kbi.fields_for_constant_selection or []
            )
        ...
        if const_selection_fields:
            select_clause.extend(
                [self._quote_identifier(col) for col in const_selection_fields]
            )
        elif sql_measure.group_by_columns:
            # Fallback to group_by_columns if no explicit constant selection
            select_clause.extend(
                [self._quote_identifier(col) for col in sql_measure.group_by_columns]
            )

        # Add the measure expression
        measure_alias = sql_measure.technical_name or "measure_value"
        select_clause.append(
            f"{sql_measure.to_sql_expression()} AS {self._quote_identifier(measure_alias)}"
        )
        ...
        processed_filters = self._process_filters_for_constant_selection(
            sql_measure.filters, const_selection_fields, sql_definition
        )
        ...
        group_by_columns = (
            const_selection_fields
            if const_selection_fields
            else sql_measure.group_by_columns
        )
    ...
    def _process_filters_for_constant_selection(
        self,
        filters: List[str],
    ...
    def _create_exception_aggregation_query(
        self,
        sql_measure: SQLMeasure,
        sql_definition: SQLDefinition,
        options: SQLTranslationOptions,
    ...
    def _create_combined_sql_query(
        self,
        sql_measures: List[SQLMeasure],
        sql_definition: SQLDefinition,
        options: SQLTranslationOptions,
    ) -> SQLQuery:
        ...
        if len(table_measures) > 1:
            return self._create_multi_table_sql_query(
                table_measures, sql_definition, options
            )
        else:
            # Single table - create simple query
            table_name = list(table_measures.keys())[0]
            measures = table_measures[table_name]
            return self._create_single_table_sql_query(
                measures, table_name, sql_definition, options
            )

    def _create_single_table_sql_query(
        self,
        sql_measures: List[SQLMeasure],
        table_name: str,
        sql_definition: SQLDefinition,
        ...
        for sql_measure in sql_measures:
            alias = (
                sql_measure.technical_name or f"measure_{len(select_expressions) + 1}"
            )
            select_expressions.append(
                f"{sql_measure.to_sql_expression()} AS {self._quote_identifier(alias)}"
            )
        ...
        unique_filters = self._process_and_deduplicate_filters(
            all_filters, sql_definition
        )
    ...
    def _create_multi_table_sql_query(
        self,
        ...
        for table_name, measures in table_measures.items():
            for measure in measures:
                ...
                # Process filters
                processed_filters = self._process_and_deduplicate_filters(
                    measure.filters, sql_definition
                )
    ...
    def _process_and_deduplicate_filters(
        self, filters: List[str], sql_definition: SQLDefinition
    ) -> List[str]:
            ...
            if filter_condition == "$query_filter":
                ...
                if hasattr(sql_definition, "filters") and sql_definition.filters:
                    ...
                    for filter_name, filter_value in query_filters.items():
                        processed_filter = self._substitute_variables_in_filter(
                ...
                # Then try expanded_filters
                    ...
                    processed_filter = self._substitute_variables_in_filter(
                        expanded_filters["query_filter"], variables, expanded_filters
                    )
                    if processed_filter:
                        processed_filters.append(processed_filter)
                continue
    ...
    def _substitute_variables_in_filter(
        self,
...
class SQLTimeIntelligenceHelper:
    ...
    def create_ytd_sql_structure(
        self, date_column: str = "fiscal_date"
    ) -> SQLStructure:
    ...
    def create_prior_year_sql_structure(
        self, date_column: str = "fiscal_date"
    ) -> SQLStructure:
    ...
    def create_variance_sql_structure(self, base_measures: List[str]) -> SQLStructure:
        ...
        return SQLStructure(
            description="Variance Analysis", formula=formula, display_sign=1
        )
