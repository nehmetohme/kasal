...
class SQLDefinition(BaseModel):
    ...
    database_schema: Optional[str] = (
        None  # Renamed from 'schema' to avoid Pydantic conflict
    )
...
class SQLTranslationResult(BaseModel):
    ...
    def get_formatted_sql_output(self) -> str:
        ...
        output_lines.append(
            f"-- Target Dialect: {self.translation_options.target_dialect.value}"
        )
        output_lines.append(
            f"-- Generated {len(self.sql_queries)} quer{'y' if len(self.sql_queries) == 1 else 'ies'} for {self.measures_count} measure{'s' if self.measures_count != 1 else ''}"
        )
