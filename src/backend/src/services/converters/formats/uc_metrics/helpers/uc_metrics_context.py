"""
UC Metrics KBI Context Tracking
Implements context-aware filter tracking for Unity Catalog Metrics
"""
...
class UCBaseKBIContext:
    """
    Defines Base KBI context in relation to calculated KBIs for UC Metrics.
    ...
    Mirrors the pattern from SQLBaseKBIContext but adapted for UC Metrics specifics.
    """
    ...
    def __init__(
        ...
        """
        Initialize UC Base KBI Context
    ...
    def __repr__(self):
        ...
        return f"UCContext[{parent_names} → {self.kbi.technical_name}]"

    def __eq__(self, other):
        if isinstance(other, UCBaseKBIContext):
            return (
    ...
    def combined_filters(self) -> List[str]:
        ...
        All filters are ANDed together in filter expression.
        """
        filters = []
    ...
    def get_kbi_context(
        ...
        Returns:
            UCBaseKBIContext instance
        """
        return UCBaseKBIContext(kbi=kbi, parent_kbis=parent_kbis)
    ...
    def get_filter_expression(self) -> Optional[str]:
        """
        Build filter expression from combined filters for UC Metrics

        Returns:
            Filter expression string (Spark SQL syntax)
        """
        if not self.combined_filters:
            return None

        # Join all filters with AND (Spark SQL syntax)
...
class UCKBIContextCache:
    """
    Cache for UC KBI contexts to avoid recalculating the same combinations

    Similar to SQLKBIContextCache pattern.
    """

    def __init__(self):
        self._cache: Set[UCBaseKBIContext] = set()

    def add_context(self, context: UCBaseKBIContext) -> None:
        """Add a context to the cache"""
        self._cache.add(context)

    def get_all_contexts(self) -> Set[UCBaseKBIContext]:
        """Get all cached contexts"""
        return self._cache

    def get_contexts_for_kbi(self, kbi_technical_name: str) -> List[UCBaseKBIContext]:
    ...
    def get_unique_filter_combinations(self) -> List[str]:
        ...
        for ctx in self._cache:
            filter_expr = ctx.get_filter_expression()
            if filter_expr:
                filter_combinations.add(filter_expr)
