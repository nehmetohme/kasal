...
class LakebaseConnectionService(BaseService):
    ...
    def __init__(
        self, user_token: Optional[str] = None, user_email: Optional[str] = None
    ):
        """
    ...
    async def get_workspace_client(self) -> WorkspaceClient:
        ...
        if not self._workspace_client:
            ...
            if client_id and client_secret and host:
                logger.info(
                    "[LAKEBASE AUTH] Using SPN OAuth (client credentials) — required for postgres scope"
                )
            ...
            logger.info(
                "[LAKEBASE AUTH] No SPN credentials, falling back to PAT/OBO (local dev)"
            )
            # Set custom User-Agent for Lakebase operations
            with_product(f"{KASAL_BASE}_{KasalProduct.LAKEBASE}", VERSION)
    ...
    async def get_username(self) -> str:
        ...
        try:
            ...
            if (
                ...
                logger.info(
                    f"Using workspace user as PG username: {current_user.user_name}"
                )
    ...
    async def generate_credentials(self, instance_name: str) -> Any:
        ...
        try:
            ...
            if "not found" not in str(e).lower() and "not_found" not in str(e).lower():
                raise
            logger.info(
                f"Instance {instance_name} not found in DatabaseAPI, trying PostgresAPI"
            )
    ...
    async def test_connection(
        self, endpoint: str, instance_name: str
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            return {"success": False, "error": str(e), "username_used": username}
        finally:
    ...
    def create_lakebase_engine_sync(
        self, endpoint: str, username: str, token: str, statement_timeout_ms: int = 0
    ) -> Engine:
        """
