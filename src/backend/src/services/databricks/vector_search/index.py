...
class DatabricksIndexService:
    ...
    def _get_index_repository(
        self, workspace_url: str, group_id: Optional[str] = None
    ) -> DatabricksVectorIndexRepository:
        """
        ...
        if (
            not self._index_repo
            or self._index_repo.workspace_url != workspace_url
            or self._index_repo.group_id != group_id
        ):
            self._index_repo = DatabricksVectorIndexRepository(
                workspace_url, group_id=group_id
            )
        return self._index_repo

    def _get_endpoint_repository(
        ...
        """
        if (
            not self._endpoint_repo
            or self._endpoint_repo.workspace_url != workspace_url
        ):
    ...
    async def create_databricks_index(
        ...
        try:
            ...
            memory_logger.info(
                f"Using embedding dimension {embedding_dimension} for {index_type} index"
            )

            # Determine which endpoint to use based on index type
            # For document embeddings, use storage optimized endpoint if available
            use_document_endpoint = (
                index_type == "document" and config.document_endpoint_name
            )
            target_endpoint = (
                config.document_endpoint_name
                if use_document_endpoint
                else config.endpoint_name
            )
            ...
            if response.success:
                ...
                memory_logger.info(
                    f"Successfully created {index_type} index: {index_name}"
                )
                ...
                if "already exists" in str(response.error).lower():
                    memory_logger.warning(
                        f"Index {index_name} already exists for {index_type}"
                    )
                    return {
                        "success": False,
                    ...
                    memory_logger.error(
                        f"Failed to create {index_type} index: {response.error}"
                    )
                    return {
                        "success": False,
                        "message": response.message,
                        "details": {"error": response.error, "index_type": index_type},
                    }
            ...
            return {
                ...
                "details": {"error": str(e), "index_type": index_type},
            }
    ...
    async def get_databricks_indexes(
        ...
        try:
            ...
            if response.success:
                ...
                return {"success": False, "message": response.message, "indexes": []}
    ...
    async def delete_databricks_index(
        ...
        try:
            ...
            return {"success": False, "message": f"Failed to delete index: {str(e)}"}
    ...
    async def delete_databricks_endpoint(
        ...
        try:
            ...
            return {"success": False, "message": f"Failed to delete endpoint: {str(e)}"}
    ...
    async def wait_for_index_ready(
        ...
        try:
            ...
            logger.info(
                f"Waiting for index {index_name} to be ready (max {max_wait_seconds}s, check every {check_interval_seconds}s)"
            )
            ...
            while True:
                ...
                if elapsed_time >= max_wait_seconds:
                    logger.warning(
                        f"Index {index_name} did not become ready within {max_wait_seconds} seconds"
                    )
                    return {
                        "success": False,
                        "ready": False,
                ...
                logger.info(
                    f"Attempt {attempt}: Checking index {index_name} readiness..."
                )

                try:
                    # Get index info using repository
                    response = await repo.get_index(
                    ...
                    if response.success and response.index:
                        ...
                        logger.info(
                            f"Index {index_name} state: {state}, ready: {ready}"
                        )

                        # Also try describe_index method for additional verification
                        try:
                            describe_response = await repo.describe_index(
                                index_name, endpoint_name, user_token
                            )
                            if describe_response.get("success"):
                                ...
                                logger.info(
                                    f"Describe method - state: {describe_state}, ready: {describe_ready}"
                                )

                                # Use describe results if they differ and seem more reliable
                                if describe_ready and not ready:
                                    logger.info(
                                        "Describe method indicates ready=True, using that value"
                                    )
                                    ready = True
                                    state = describe_state
                        except Exception as e:
                            logger.debug(
                                f"Describe method failed, using get_index results: {e}"
                            )

                        if ready:
                            logger.info(
                                f"✅ Index {index_name} is ready after {elapsed_time:.1f}s ({attempt} attempts)"
                            )
                            return {
                                "success": True,
                                "ready": True,
                            ...
                            logger.info(
                                f"Index {index_name} not ready yet (state: {state})"
                            )
                    else:
                    ...
                    logger.warning(
                        f"Error checking index readiness (attempt {attempt}): {e}"
                    )
    ...
    async def get_index_info(
        ...
        try:
            ...
            if response.success and response.index:
                ...
                index_type = (
                    "Direct Access"
                    if index_info.index_type == IndexType.DIRECT_ACCESS
                    else "Delta Sync"
                )
    ...
    async def empty_index(
        ...
        try:
            ...
            result = await repo.empty_index(
                index_name, endpoint_name, embedding_dimension, user_token, index_type
            )
            ...
            if result.get("success"):
                ...
                logger.error(
                    f"Failed to empty index {index_name}: {result.get('message')}"
                )
            ...
            return {"success": False, "message": f"Failed to empty index: {str(e)}"}
        ...
        try:
            ...
            try:
                ...
                databricks_logger.info(
                    f"Using embedding dimension {embedding_dimension} for index {index_name}"
                )
                ...
                if results:
                    ...
                    if isinstance(results, dict):
                        ...
                        if "result" in results and isinstance(results["result"], dict):
                            ...
                            logger.info(
                                f"Found {len(data_array)} documents in results['result']['data_array']"
                            )
                        # Alternative: results['data_array']
                        elif "data_array" in results:
                            data_array = results.get("data_array", [])
                            logger.info(
                                f"Found {len(data_array)} documents in results['data_array']"
                            )
                        # Alternative: results['data']
                        elif "data" in results:
                            data_array = results.get("data", [])
                            logger.info(
                                f"Found {len(data_array)} documents in results['data']"
                            )
                    ...
                    for idx, item in enumerate(data_array):
                        ...
                        if isinstance(item, list):
                            ...
                            column_positions = (
                                DatabricksIndexSchemas.get_column_positions(memory_type)
                            )
                            item_dict = {}
                            ...
                            logger.warning(
                                f"Unexpected item type at index {idx}: {type(item)}"
                            )
                            continue
    ...
    async def query_entity_data(
        ...
        try:
            ...
            if isinstance(search_results, dict) and "result" in search_results:
                entity_logger.info(
                    f"Result keys: {list(search_results['result'].keys()) if isinstance(search_results['result'], dict) else 'Not a dict'}"
                )
            ...
            if search_results:
                ...
                if isinstance(search_results, dict):
                    if "result" in search_results:
                        ...
                        column_names = [
                            col["name"] for col in manifest.get("columns", [])
                        ]
                ...
                if data_array and len(data_array) > 0:
                    entity_logger.info(
                        f"First row sample (first 5 values): {data_array[0][:5] if len(data_array[0]) > 5 else data_array[0]}"
                    )
                ...
                for idx, row in enumerate(data_array):
                    ...
                    if not entity_id or entity_id == "null" or entity_id == "None":
                        ...
                        if entity_data.get("entity_name") and entity_data[
                            "entity_name"
                        ] not in ["null", "None", ""]:
                    ...
                    if idx < 5:
                        entity_logger.info(
                            f" Entity {idx}: id={entity_id}, name='{entity_name}', type='{entity_type}'"
                        )
                        entity_logger.info(
                            f" Raw data keys: {list(entity_data.keys())}"
                        )
                        entity_logger.info(
                            f" Raw entity_name value: {repr(entity_data.get('entity_name'))}"
                        )
                        entity_logger.info(
                            f" Raw entity_type value: {repr(entity_data.get('entity_type'))}"
                        )
                    ...
                    if "relationships" in entity_data and entity_data["relationships"]:
                        try:
                            ...
                            parsed_relationships = json.loads(
                    ...
                    if (
                        "relationship_data" in entity_data
                        and entity_data["relationship_data"]
                    ):
                        try:
                            import json
                    ...
                    if entity_relationships:
                        ...
                        if idx < 3 and entity_relationships:
                            entity_logger.info(
                                f" Entity {idx} relationships: {entity_relationships[:2]}"
                            )
                ...
                for rel in relationships:
                    if rel.get("pending_resolution"):
                        ...
                        if target_name_lower in entity_map:
                            ...
                            target_id = (
                                f"target_{target_name.replace(' ', '_').lower()}"
                            )
                            rel["target"] = target_id
                            del rel["pending_resolution"]

                entity_logger.info(
                    f"Creating {len(targets_to_create)} additional nodes for relationship targets"
                )
                entity_logger.info(
                    f"Primary entities: {len(entities)}, Relationships found: {len(relationships)}"
                )
                ...
                for target_name in targets_to_create:
                    ...
                    if any(
                        ...
                        word in target_lower
                        for word in ["company", "organization", "department"]
                    ):
                        entity_type = "organization"
                    elif any(
                        word in target_lower
                        for word in ["event", "meetup", "gathering", "summit"]
                    ):
                        entity_type = "event"
                    elif any(
                        word in target_lower
                        for word in ["conference", "symposium", "workshop"]
                    ):
                        entity_type = "conference"
                    elif any(
                        word in target_lower
                        for word in ["tool", "software", "application"]
                    ):
                        entity_type = "tool"
                    elif any(
                        word in target_lower for word in ["community", "forum", "group"]
                    ):
                        entity_type = "organization"
            ...
            if "not found" in str(e).lower():
                entity_logger.error(
                    f" Index not found: {index_name} on endpoint {endpoint_name}"
                )
                return {
                    "success": False,
                    "entities": [],
    ...
    def _process_search_results(
        self, raw_results: Dict[str, Any], memory_type: str
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            if raw_results and "result" in raw_results:
                ...
                memory_logger.info(
                    f"[_process_search_results] Processing {len(data_array)} raw results for {memory_type}"
                )
                ...
                columns = DatabricksIndexSchemas.get_search_columns(memory_type)
                memory_logger.debug(
                    f"[_process_search_results] Expected columns: {len(columns)}"
                )
                ...
                for idx, row in enumerate(data_array):
                    if row:
                        ...
                        memory_logger.debug(
                            f"[_process_search_results] Row {idx}: length={len(row)}, expected={len(columns)}"
                        )
                        ...
                        if len(row) > len(columns):
                            ...
                            memory_logger.debug(
                                f"[_process_search_results] Row {idx} score: {score}"
                            )
                        else:
                        ...
                        memory_logger.warning(
                            f"[_process_search_results] Empty row at index {idx}"
                        )

            memory_logger.info(
                f"[_process_search_results] Processed {len(processed_results)} results for {memory_type}"
            )
