...
class DatabricksVectorSearchSetupService:
    ...
    async def one_click_databricks_setup(
        ...
        try:
            ...
            logger.info(
                f"[DEBUG] Starting one-click setup with workspace_url: {workspace_url}"
            )
            ...
            endpoint_request = EndpointCreate(
                name=memory_endpoint_name, endpoint_type=EndpointType.STANDARD
            )

            endpoint_response = await endpoint_repo.create_endpoint(
                endpoint_request, user_token
            )
            ...
            if endpoint_response.success:
                results["endpoints"]["memory"] = {
                    ...
                    "status": (
                        "created"
                        if "already exists" not in endpoint_response.message
                        else "already_exists"
                    ),
                }
            else:
                raise Exception(
                    f"Failed to create memory endpoint: {endpoint_response.message}"
                )
            ...
            doc_endpoint_request = EndpointCreate(
                name=doc_endpoint_name, endpoint_type=EndpointType.STANDARD
            )

            doc_endpoint_response = await endpoint_repo.create_endpoint(
                doc_endpoint_request, user_token
            )
            ...
            if doc_endpoint_response.success:
                results["endpoints"]["document"] = {
                    ...
                    "status": (
                        "created"
                        if "already exists" not in doc_endpoint_response.message
                        else "already_exists"
                    ),
                }
            else:
                ...
                results["endpoints"]["document"] = {
                    "name": doc_endpoint_name,
                    "error": doc_endpoint_response.error
                    or doc_endpoint_response.message,
                }
            ...
            if doc_endpoint_name and "error" not in results["endpoints"].get(
                "document", {}
            ):
            ...
            async def create_index_with_retry(
                ...
                for attempt in range(max_retries):
                    try:
                        ...
                        index_response = await index_repo.create_index(
                            index_request, user_token
                        )

                        if index_response.success:
                            logger.info(f"Successfully created index {index_name}")
                            return {"name": index_name, "status": "created"}
                        else:
                            # Check if it's a table doesn't exist error
                            error_msg = str(index_response.message)
                            if (
                                "does not exist" in error_msg.lower()
                                and "table" in error_msg.lower()
                            ):
                                if attempt < max_retries - 1:
                                    wait_time = (attempt + 1) * 10  # 10, 20, 30 seconds
                                    logger.info(
                                        f"Table for {index_name} not ready yet, retrying in {wait_time} seconds (attempt {attempt + 1}/{max_retries})"
                                    )
                                    await asyncio.sleep(wait_time)
                                    continue
                            raise Exception(
                                f"Failed to create index: {index_response.message}"
                            )
                    except Exception as e:
                        error_str = str(e)
                        if "already exists" in error_str.lower():
                            logger.info(f"Index {index_name} already exists")
                            return {"name": index_name, "status": "already_exists"}
                        elif (
                            ...
                            wait_time = (attempt + 1) * 10  # 10, 20, 30 seconds
                            logger.info(
                                f"Resource for {index_name} not ready yet, retrying in {wait_time} seconds (attempt {attempt + 1}/{max_retries})"
                            )
                            await asyncio.sleep(wait_time)
                            continue
                        else:
            ...
            for index_type, table_name, endpoint_name in index_configs:
                result = await create_index_with_retry(
                    index_type, table_name, endpoint_name
                )
            ...
            config = DatabricksMemoryConfig(
                ...
                document_endpoint_name=(
                    doc_endpoint_name
                    if "error" not in results["endpoints"]["document"]
                    else None
                ),
                memory_index=results["indexes"].get("unified", {}).get("name", ""),
                document_index=(
                    results["indexes"].get("document", {}).get("name", "")
                    if "document" in results["indexes"]
                    else None
                ),
            ...
            if group_id and self.session:
                try:
                    ...
                    if existing_configs:
                        logger.info(
                            f"Found {len(existing_configs)} existing memory backend configurations for group {group_id}"
                        )
                    ...
                    saved_backend = await base_service.create_memory_backend(
                        group_id, backend_config
                    )
                    ...
                    if "foreign key constraint" in str(save_error).lower():
                        results["warning"] = (
                            "Setup completed but configuration not saved. Please ensure you are logged in."
                        )
                        results["info"] = (
                            "The Databricks resources were created successfully. You can configure them manually in your agent or crew settings."
                        )
                    else:
                        results["warning"] = (
                            f"Setup completed but failed to save configuration: {str(save_error)}"
                        )
            else:
                results["info"] = (
                    "Databricks resources created successfully. Please log in to save the configuration."
                )
