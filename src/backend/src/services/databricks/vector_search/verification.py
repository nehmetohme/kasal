...
class DatabricksVectorSearchVerificationService:
    ...
    async def verify_databricks_resources(
        ...
        try:
            logger.info(
                f"Verifying Databricks resources - workspace_url: {workspace_url}, user_token provided: {bool(user_token)}"
            )
            ...
            for endpoint_name in endpoints_to_check:
                try:
                    ...
                    endpoint_response = await endpoint_repo.get_endpoint(
                        endpoint_name, user_token
                    )

                    if endpoint_response.success and endpoint_response.endpoint:
                        endpoint = endpoint_response.endpoint
                        logger.debug(
                            f"Endpoint {endpoint_name} exists with state {endpoint.state}"
                        )
                        ...
                        result["endpoints"][endpoint_name] = {
                            ...
                            "state": (
                                endpoint.state
                                if isinstance(endpoint.state, str)
                                else endpoint.state.value
                            ),
                            "ready": endpoint.ready,
                            "type": (
                                endpoint.endpoint_type
                                if isinstance(endpoint.endpoint_type, str)
                                else endpoint.endpoint_type.value
                            ),
                        }
                    else:
                        ...
                        if (
                            ...
                            and endpoint_response.endpoint.state
                            ...
                            logger.warning(
                                f"Error checking endpoint {endpoint_name}: {endpoint_response.message}"
                            )
                            result["endpoints"][endpoint_name] = {
                                "exists": False,
                                "state": "ERROR",
            ...
            for index_name in indexes_to_check:
                ...
                for endpoint_name in endpoints_to_check:
                    ...
                    try:
                        ...
                        indexes_response = await index_repo.list_indexes(
                            endpoint_name, user_token
                        )
                        ...
                        if indexes_response.success:
                            ...
                            for index in indexes_response.indexes:
                                if index.name == index_name:
                                    logger.info(
                                        f"Index {index_name} found on endpoint {endpoint_name}"
                                    )
                                    ...
                                    result["indexes"][index_name] = {
                                        ...
                                        "state": (
                                            index.state
                                            if isinstance(index.state, str)
                                            else index.state.value
                                        ),
                                        "ready": index.ready,
                                    }
                                    found = True
                            ...
                            logger.warning(
                                f"Failed to get indexes for endpoint {endpoint_name}: {indexes_response.message}"
                            )
                    except Exception as e:
                        logger.error(
                            f"Error getting indexes for endpoint {endpoint_name}: {e}"
                        )
                ...
                if not found:
                    ...
                    for endpoint_name in endpoints_to_check:
                        try:
                            ...
                            if index_response.success and index_response.index:
                                if index_response.index.state != IndexState.NOT_FOUND:
                                    ...
                                    result["indexes"][index_name] = {
                                        ...
                                        "endpoint": endpoint_name,
                                        "state": (
                                            index_response.index.state
                                            if isinstance(
                                                index_response.index.state, str
                                            )
                                            else index_response.index.state.value
                                        ),
                            ...
                            logger.debug(
                                f"Index {index_name} not found on endpoint {endpoint_name}: {e}"
                            )

                    # If still not found
                    if not found:
                        logger.info(
                            f"Index {index_name} not found on any configured endpoint"
                        )
                        result["indexes"][index_name] = {
                            "exists": False,
                            "endpoint": None,
