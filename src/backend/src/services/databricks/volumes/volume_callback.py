...
class DatabricksVolumeCallback(KasalCallback):
        ...
        if not self.workspace_url or not self.token:
            try:
                ...
                if auth:
                    ...
                    logger.debug(
                        f"Using unified {auth.auth_method} authentication for Volume callback"
                    )
    ...
    async def execute(self, output: Any) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Successfully uploaded output to Databricks Volume: {full_path}"
            )
        ...
        if self.file_format == "json":
            if hasattr(output, "raw"):
                ...
                content = {
                    "raw": output.raw,
                    "json_dict": (
                        output.json_dict if hasattr(output, "json_dict") else None
                    ),
                    "pydantic": (
    ...
    async def _upload_to_volume(self, file_path: str, content: str) -> str:
        ...
        if len(path_parts) < 4 or path_parts[0] != "Volumes":
            raise ValueError(
                f"Invalid volume path format: {self.volume_path}. Expected: /Volumes/catalog/schema/volume"
            )
        ...
        if parent_dir_path:
            ...
            if not dir_result.get("success"):
                logger.warning(
                    f"Failed to create directory {parent_dir_path}: {dir_result.get('error')}"
                )
