...
class DatabricksConnectionService:
    ...
    async def test_databricks_connection(
        ...
        try:
            ...
            try:
                endpoint_response = await endpoint_repo.get_endpoint_status(
                    config.endpoint_name, user_token
                )

                if not endpoint_response.get("success"):
                    return {
                        "success": False,
                        "message": endpoint_response.get(
                            "message", "Failed to get endpoint"
                        ),
                        "details": {"error": endpoint_response.get("error")},
                    }
                    ...
                    if index_name:
                        ...
                        if index_response.success and index_response.index:
                            ...
                            indexes_missing.append(
                                {"name": index_name, "type": index_type}
                            )
                ...
                return {
                    ...
                    "details": {"error": str(e), "auth_method": "Repository Pattern"},
                }
            ...
            return {
                ...
                "details": {"error": "Please install databricks-vectorsearch package"},
            }
        except Exception as e:
            return {
                "success": False,
