...
class ManagerConfigBuilder:
    ...
    async def configure_manager(
        ...
        try:
            ...
            logger.warning(
                "Enhanced Databricks auth not available for crew preparation"
            )
    ...
    async def _configure_hierarchical_manager(
        ...
        # Check for manager_llm
            ...
            if requested_model and group_id:
                try:
                    manager_llm = await LLMManager.configure_kasal_llm(
                        requested_model, group_id
                    )
                    crew_kwargs["manager_llm"] = manager_llm
                    logger.info(
                        f"Set manager LLM for hierarchical process: {requested_model}"
                    )
                ...
                logger.warning(
                    "Hierarchical process requires manager_llm or manager_agent - using fallback"
                )
                crew_kwargs = await self._set_fallback_manager_llm(crew_kwargs)
            ...
            if isinstance(provided_manager_llm, str):
                logger.info(
                    f"Converting manager_llm string '{provided_manager_llm}' to LLM object"
                )
                if not group_id:
                    logger.error(
                        "Cannot convert manager_llm string: no group_id in config"
                    )
                    crew_kwargs = await self._set_fallback_manager_llm(crew_kwargs)
                else:
                    try:
                        crew_kwargs["manager_llm"] = (
                            await LLMManager.configure_kasal_llm(
                                provided_manager_llm, group_id
                            )
                        )
                        logger.info(
                            f"Successfully converted manager_llm '{provided_manager_llm}' to LLM object"
                        )
                    except Exception as llm_error:
                        logger.error(
                            f"Failed to convert manager_llm string to LLM object: {llm_error}"
                        )

                        # Try with databricks/ prefix
                        if (
                            "databricks" in provided_manager_llm.lower()
                            and not provided_manager_llm.startswith("databricks/")
                        ):
                            prefixed_model = f"databricks/{provided_manager_llm}"
                            logger.info(
                                f"Retrying with databricks/ prefix: {prefixed_model}"
                            )
                            try:
                                crew_kwargs["manager_llm"] = (
                                    await LLMManager.configure_kasal_llm(
                                        prefixed_model, group_id
                                    )
                                )
                                logger.info(
                                    f"Successfully created manager_llm with prefix: {prefixed_model}"
                                )
                            except Exception as retry_error:
                                logger.error(
                                    f"Failed to create manager_llm even with prefix: {retry_error}"
                                )
                                crew_kwargs = await self._set_fallback_manager_llm(
                                    crew_kwargs
                                )
                        else:
                            crew_kwargs = await self._set_fallback_manager_llm(
                ...
                logger.info(
                    "Using provided manager_llm object for hierarchical process"
                )

        return crew_kwargs

    async def _set_fallback_manager_llm(
        ...
        try:
            ...
            fallback_llm = await LLMManager.configure_kasal_llm(
                "databricks-llama-4-maverick", group_id
            )
