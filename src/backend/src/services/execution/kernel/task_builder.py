...
async def build_task_args(
    ...
    *,
    ...
    # retry_on_fail is not an engine Task field (it would be silently dropped);
    # an explicit False means "no guardrail retries", which the engine reads
    # from max_retries.
    if task_config.get("retry_on_fail") is False:
...
def _apply_code_guardrail(task_args, task_config, agent, config, task_key):
    ...
    guardrail_logger.info(
        f"Task {task_key} has guardrail configuration: {guardrail_config}"
    )
    ...
    try:
        ...
        if (
            ...
            _parsed_config = {
                **_parsed_config,
                "llm_model": resolve_guardrail_model(
                    _parsed_config.get("llm_model"), agent, config
                ),
            }

        factory_config = (
            json.dumps(_parsed_config)
            if isinstance(_parsed_config, dict)
            else _parsed_config
        )
        guardrail = GuardrailFactory.create_guardrail(factory_config)
        if guardrail:
            # GuardrailWrapper gives the callable a stable class identity
            # (readable guardrail label in events/trace).
        ...
        guardrail_logger.error(
            f"Error setting up guardrail for task {task_key}: {str(e)}"
        )
...
async def _apply_llm_guardrail(task_args, task_config, agent, config, task_key):
    ...
    guardrail_logger.info(
        f"Task {task_key} has LLM guardrail configuration: {llm_guardrail_config}"
    )
    ...
    try:
        ...
        if description and description != "Validate the task output":
            ...
            task_args["description"] = (
                task_args["description"] + validation_augmentation
            )
            guardrail_logger.info(
        ...
        group_id = (gc.primary_group_id if gc else None) or (
            config.get("group_id") if config else None
        )
        if not group_id:
        ...
        guardrail_logger.error(
            f"Could not import LLMGuardrail for task {task_key}: {str(e)}"
        )
    except ValueError:
        ...
        guardrail_logger.error(
            f"Error configuring LLM guardrail for task {task_key}: {str(e)}"
        )
        guardrail_logger.error(f"Stack trace: {traceback.format_exc()}")
...
async def _apply_output_pydantic(task_args, task_config, agent, task_key):
    ...
    converter_cls, pydantic_cls, use_output_json, is_compatible = (
        get_compatible_converter_for_model(agent, pydantic_class)
    )
