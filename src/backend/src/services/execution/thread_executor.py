...
class CrewExecutor:
    ...
    def __init__(self, max_workers: int = 20):
        ...
        self._executor = ThreadPoolExecutor(
            max_workers=max_workers, thread_name_prefix="CrewWorker"
        )
    ...
    async def run_crew(
        ...
        try:
            ...
            logger.info(
                f"Crew execution {execution_id} completed in {duration:.2f} seconds"
            )
            return result
            ...
            if on_error:
                on_error(
                    asyncio.TimeoutError(f"Execution timed out after {timeout} seconds")
                )
            raise
            ...
            if execution_id in self._active_executions:
                ...
                if len(completed_executions) > 100:
                    ...
                    oldest = sorted(
                        completed_executions,
                        key=lambda x: self._active_executions[x].get(
                    ...
                    for old_exec_id in oldest:
                        ...
                        logger.debug(
                            f"Cleaned up old execution {old_exec_id} from history"
                        )
            ...
            if status == "RUNNING":
                ...
                if execution_id in self._running_tasks:
                    ...
                    if not future.done():
                        ...
                        if cancelled:
                            logger.info(
                                f"Successfully cancelled future for execution {execution_id}"
                            )
                            # Update status to indicate cancellation in progress
                            self._active_executions[execution_id]["status"] = "STOPPING"
                        else:
                            logger.warning(
                                f"Failed to cancel future for execution {execution_id} (may be running in thread)"
                            )
                    ...
                    logger.warning(
                        f"No running future found for execution {execution_id}"
                    )
