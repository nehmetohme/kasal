        ...
        try:
            ...
            try:
                ...
                async with request_scoped_session() as session:
                    ...
                    logger.info(
                        f"Created ToolFactory with api_keys_service for agent {agent_data.name}"
                    )
            ...
            agent = await build_agent_with_tools(
                ...
                custom_attrs={
                    "_kasal_memory_disabled": getattr(agent_data, "memory", None)
                    is False
                },
            )
            logger.info(
                f"Successfully configured agent: {agent_data.name} with {len(tool_ids)} sourced tool id(s)"
            )
            logger.info(
                f"Agent {agent_data.name} memory disabled: {agent._kasal_memory_disabled}"
            )
            ...
            logger.error(
                f"Error configuring agent {getattr(agent_data, 'name', 'unknown')}: {e}",
                exc_info=True,
            )
            return None
            ...
            if agent_id:
                ...
                for node in nodes:
                    if node.get("id") == agent_node_id and "data" in node:
                        ...
                        if node_tools:
                            logger.info(
                                f"Found tools in node data for agent {agent_id}: {node_tools}"
                            )
        ...
        try:
            if isinstance(raw, dict):
                ...
                if isinstance(parsed, dict):
                    ...
                    logger.warning(
                        f"Parsed config is not a dictionary for agent: {agent_data.name}"
                    )
