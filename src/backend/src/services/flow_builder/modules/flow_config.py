...
class FlowConfigManager:
    ...
    async def collect_agent_mcp_requirements(
        ...
        try:
            ...
            if not task_repo:
                logger.warning(
                    "No task repository provided, cannot collect MCP requirements"
                )
            ...
            logger.info(
                f"Found {len(task_info)} unique tasks in flow to check for MCP requirements"
            )
            ...
            for task_id, info in task_info.items():
                try:
                    ...
                    if task_mcp_servers:
                        logger.info(
                            f"Task {task_id} ({task_data.name}) requires MCP servers: {task_mcp_servers}"
                        )
                        ...
                        if agent_id:
                            ...
                            for server in task_mcp_servers:
                                if (
                                    server
                                    and server not in agent_requirements[agent_id]
                                ):
                                    agent_requirements[agent_id].append(server)
                            logger.info(
                                f"Mapped MCP servers to agent {agent_id} from task.agent_id"
                            )
                        else:
                            # Store for later resolution from crew structure
                            info["mcp_servers"] = task_mcp_servers
                            logger.info(
                                f"Task {task_id} has MCP servers but no agent_id in database, will resolve from crew structure"
                            )

                except Exception as e:
                    logger.error(
                        f"Error processing task {task_id} for MCP requirements: {e}",
                        exc_info=True,
                    )
            ...
            if crew_repo:
                for task_id, info in task_info.items():
                    ...
                    if not crew_id:
                        logger.warning(
                            f"Task {task_id} has MCP servers but no crew_id to resolve agent"
                        )
                        continue
                        ...
                        if agent_id:
                            ...
                            for server in info["mcp_servers"]:
                                if (
                                    ...
                                    agent_requirements[agent_id].append(server)
                            logger.info(
                                f"Resolved agent {agent_id} for task {task_id} from crew structure, mapped MCP servers: {info['mcp_servers']}"
                            )
                        else:
                            logger.warning(
                                f"Could not resolve agent for task {task_id} from crew structure"
                            )

                    except Exception as e:
                        logger.error(
                            f"Error resolving agent for task {task_id} from crew {crew_id}: {e}",
                            exc_info=True,
                        )
