...
class TaskConfig:
    ...
    async def configure_task(
        ...
        try:
            ...
            if agent is None:
                agent = await TaskConfig._resolve_agent_for_task(
                    task_data, flow_data, repositories, group_context
                )

                if not agent:
                    logger.error(
                        f"No agent provided or configured for task: {task_data.name}"
                    )
                    return None

            # Check if task has specific tools and add them to the agent
            await TaskConfig._configure_task_tools(
                task_data, agent, flow_data, group_context
            )
            ...
            logger.info(
                f"Successfully configured task: {task_data.name} with agent role: {agent.role}"
            )
            ...
            logger.error(
                f"Error configuring task {getattr(task_data, 'name', 'unknown')}: {e}",
                exc_info=True,
            )
            return None
    ...
    def _task_data_to_spec(task_data):
        ...
        spec = {
            ...
            "expected_output": (
                ...
                if getattr(task_data, "expected_output", None)
                else ""
            ),
        ...
        if hasattr(task_data, "agent_id") and task_data.agent_id:
            ...
            if not agent_data:
                try:
                    ...
                    logger.error(
                        f"Error querying database for agent: {db_error}", exc_info=True
                    )

            if agent_data:
                # Configure the agent
                agent = await AgentConfig.configure_agent_and_tools(
                    agent_data, flow_data, repositories, group_context
                )
                if agent:
                    return agent
            ...
            logger.error(
                f"Agent with ID {task_data.agent_id} not found for task: {task_data.name}"
            )

        # If no agent_id or agent not found, try to infer from flow edges
        if flow_data and hasattr(flow_data, "edges"):
            logger.info(
                f"No agent_id in task data or agent not found, attempting to infer from edges for task {task_data.id}"
            )
            try:
                ...
                for edge in edges:
                    if edge.get("target") == task_node_id and edge.get(
                        ...
                        logger.info(
                            f"Inferred agent_id {inferred_agent_id} for task {task_data.id} from edge {edge.get('id')}"
                        )

                        # Get the agent with this ID
                        agent_data = None
                        agent_repo = (
                            None if repositories is None else repositories.get("agent")
                        )
                        ...
                        if not agent_data:
                            try:
                                ...
                                logger.error(
                                    f"Error querying database for inferred agent: {db_error}",
                                    exc_info=True,
                                )
                ...
                logger.error(
                    f"Error inferring agent from edges for task {task_data.id}: {e}",
                    exc_info=True,
                )
    ...
    async def _configure_task_tools(
        ...
        if (
            ...
            logger.info(
                f"Creating ToolFactory with group_id: {group_context.primary_group_id}"
            )
        ...
        try:
            ...
            logger.warning(
                f"Error creating ToolFactory with api_keys_service: {e}, falling back to basic factory"
            )
            ...
            if isinstance(task_data.tools, list):
                ...
                try:
                    ...
                    if isinstance(task_data.tools, str):
                        task_tools = [
                            str(tool_id) for tool_id in json.loads(task_data.tools)
                        ]
                except Exception as e:
            ...
            if task_tools:
                ...
                for tool_id in task_tools:
                    try:
                        ...
                        tool_override = _resolve_tool_override(
                            tool_factory, tool_id, task_tool_configs
                        )
                        # Try to create the tool using the factory
                        tool = tool_factory.create_tool(
                            tool_id, tool_config_override=tool_override
                        )
                        if tool:
                            tools.append(tool)
                            logger.info(
                                f"Added tool {tool_id} for task: {task_data.name}"
                            )
                        else:
                            logger.warning(
                                f"Tool factory couldn't create tool with ID: {tool_id} for task: {task_data.name}"
                            )
                ...
                if tools:
                    ...
                    logger.warning(
                        f"No valid tools could be created for task {task_data.name}, using agent's existing tools"
                    )
            else:
                logger.info(
                    f"Task {task_data.name} has empty tools list, using agent's existing tools"
                )
                ...
                if task_id:
                    ...
                    for node in nodes:
                        if node.get("id") == task_node_id and "data" in node:
                            ...
                            if node_tools:
                                logger.info(
                                    f"Found tools in node data for task {task_id}: {node_tools}"
                                )
                                tools = []

                                # Extract tool_configs for per-tool overrides
                                task_tool_configs = (
                                    getattr(task_data, "tool_configs", {}) or {}
                                )
                                ...
                                for tool_id in node_tools:
                                    try:
                                        ...
                                        if tool:
                                            tools.append(tool)
                                            logger.info(
                                                f"Added tool {tool_id} from node data for task: {task_data.name}"
                                            )
                                        else:
                                            logger.warning(
                                                f"Tool factory couldn't create tool with ID: {tool_id} from node data for task: {task_data.name}"
                                            )
                                    except Exception as tool_error:
                                        logger.warning(
                                ...
                                if tools:
                                    ...
                                    logger.warning(
                                        f"No valid tools could be created from node data for task {task_data.name}, using agent's existing tools"
                                    )
                            else:
                                logger.info(
                                    f"No tools found in node data for task {task_data.name}, using agent's existing tools"
                                )
                            break
            except Exception as e:
                logger.error(
                    f"Error looking for tools in flow nodes for task {task_data.name}: {e}",
                    exc_info=True,
                )
        else:
            logger.info(
                f"No tools explicitly assigned to task {task_data.name}, using agent's existing tools"
            )
