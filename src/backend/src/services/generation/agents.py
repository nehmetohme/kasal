...
class AgentGenerationService:
    ...
    async def _prepare_prompt_template(
        self, tools: List[str], group_context: Optional[GroupContext]
    ) -> str:
        """
        ...
        system_message = await TemplateService.get_effective_template_content(
            "generate_agent", group_context
        )

        if not system_message:
            raise ValueError(
                "Required prompt template 'generate_agent' not found in database"
            )
    ...
    def _process_agent_config(
        ...
        for field in required_fields:
            if field not in setup:
                raise ValueError(
                    f"Missing required field in agent configuration: {field}"
                )
