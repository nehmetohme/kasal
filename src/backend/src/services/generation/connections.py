...
class ConnectionService:
    ...
    async def _format_agents_and_tasks(
        ...
        for idx, task in enumerate(request.tasks, 1):
            ...
            if task.context:
                ...
                if context.required_skills:
                    tasks_info += (
                        f"   Required Skills: {', '.join(context.required_skills)}\n"
                    )

        return agents_info, tasks_info

    async def _validate_response(
        self, response_data: Dict[str, Any], request: ConnectionRequest
    ) -> None:
        """
        ...
        if (
            ...
            or "assignments" not in response_data
            or "dependencies" not in response_data
        ):
        ...
        for assignment in response_data["assignments"]:
            if (
                ...
                or "agent_name" not in assignment
                or "tasks" not in assignment
            ):
        ...
        if unassigned_tasks:
            ...
            error_detail += (
                "- Try making agent descriptions more clearly aligned with tasks\n"
            )
    ...
    async def generate_connections(
        ...
        logger.info(
            f"Number of agents: {len(request.agents)}, Number of tasks: {len(request.tasks)}"
        )
        ...
        try:
            ...
            if not system_message:
                raise ValueError(
                    "Required prompt template 'generate_connections' not found"
                )
            ...
            async with aiohttp.ClientSession() as session:
                ...
                async with session.get(
                    ...
                    if response.status == 200:
                        ...
                        return (
                            False,
                            f"API key validation failed: {response.status} - {error_text}",
                        )
        except Exception as e:
