...
class MinimumNumberGuardrail(BaseGuardrail):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.field_name = config_dict.get(
            "field_name", "total_count"
        )  # Default to total_count
        self.message = config_dict.get(
            "message",
            f"The output should contain a '{self.field_name}' value greater than {self.min_value}",
        )
        logger.info(
            f"Initialized MinimumNumberGuardrail with min_value={self.min_value}, field_name={self.field_name}"
        )
    ...
    def validate(self, output: Union[str, Any, Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if is_task_output(output):
            logger.info(
                "Detected task-output object, checking for special Linkup Search Tool format"
            )
            ...
            if hasattr(output, "results") and hasattr(output, "source"):
                ...
                if isinstance(source, str) and (
                    "linkup" in source.lower() or "search tool" in source.lower()
                ):
                    ...
                    if isinstance(results, list):
                        count = len(results)
                        logger.info(
                            f"Counted {count} results in Linkup Search Tool output"
                        )

                        # Validate this count against the minimum
                        if count > self.min_value:
                            logger.info(
                                f"Validation passed: {count} > {self.min_value}"
                            )
                            return {"valid": True, "feedback": ""}
                        else:
                            logger.info(
                                f"Validation failed: {count} ≤ {self.min_value}"
                            )
                            return {
                                "valid": False,
        ...
        try:
            ...
            if value is None:
                ...
                if is_task_output(output) and hasattr(output, "results"):
                    ...
                    if isinstance(results, list):
                        count = len(results)
                        logger.info(
                            f"Fallback to counting results, found {count} items"
                        )
                        value = count
            ...
            if value > self.min_value:
                ...
                return {"valid": False, "feedback": self.message}
    ...
    def _extract_value(
        ...
        # If output is a task-output object
            ...
            for attr in [
                ...
                try:
                    if hasattr(output, attr):
                        ...
                        logger.info(
                            f"task output .{attr}: {type(value)} = {str(value)[:100]}{'...' if len(str(value)) > 100 else ''}"
                        )
            ...
            if _has("results") and _has("total_count"):
                logger.info(
                    "Found 'results' and 'total_count' attributes in the task output"
                )
                try:
                    ...
                    if total_count is not None:
                        logger.info(
                            f"Using direct total_count attribute: {total_count}"
                        )
            ...
            if hasattr(output, "content"):
                ...
                if isinstance(content, str):
                    try:
                        ...
                        logger.info(
                            "Content is not valid JSON, trying regex extraction"
                        )
            ...
            try:
                ...
                logger.info(
                    f"Converting the task output to string (length: {len(output_str)})"
                )
                ...
                if "results=" in output_str or "total_count=" in output_str:
                    ...
                    if total_count_match:
                        logger.info(
                            f"Found total_count in string: {total_count_match.group(1)}"
                        )
                    ...
                    if results_match and len(results_match) > 0:
                        logger.info(
                            f"Counted {len(results_match)} potential results items in string"
                        )
        ...
        # If output is a string, try to parse it as JSON
            ...
            try:
                ...
                logger.info(
                    "String is not valid JSON, attempting to extract numbers with regex"
                )
    ...
    def _get_value_from_dict(
        ...
        if (
            ...
            and self.field_name.lower()
            in ["count", "total_count", "results_count", "length", "size"]
        ):
        ...
        for key, value in data.items():
            if isinstance(value, dict):
                if self.field_name in value:
                    ...
                    logger.info(
                        f"Found {self.field_name} in nested dict under '{key}' = {nested_value}"
                    )
                    return nested_value

                # Look for count-related fields in nested dicts
                if self.field_name.lower() in [
                    "count",
                    "total_count",
                    "items",
                    "results",
                    ...
                    for nested_key in [
                        ...
                        if nested_key in value:
                            ...
                            logger.info(
                                f"Found related field {nested_key} in nested dict under '{key}' = {nested_value}"
                            )
                            return nested_value
    ...
    def _extract_value_from_text(self, text: str) -> Optional[Union[int, float]]:
        ...
        field_pattern = (
            rf'["\']?{re.escape(self.field_name)}["\']?\s*[:=]\s*([0-9]+(?:\.[0-9]+)?)'
        )
        ...
        if any(
            count_term in self.field_name.lower()
            for count_term in [
                "count",
                "total",
                "items",
            ...
            logger.info(
                "Field name contains count-related terms, looking for count statements"
            )
            count_patterns = [
        ...
        if number_matches:
            ...
            try:
                ...
                significant_numbers = [
                    float(num) for num in number_matches if float(num) > 1
                ]
        ...
        logger.warning(
            f"No suitable numeric value found in text for field: {self.field_name}"
        )
        return None
