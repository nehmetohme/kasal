...
class CompanyCountGuardrail(BaseGuardrail):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        logger.info(
            f"Initialized CompanyCountGuardrail with min_companies={self.min_companies}"
        )
    ...
    def validate(self, output: Union[str, Any, Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if company_count >= self.min_companies:
            ...
            logger.info(
                f"Validation failed: Found only {company_count} companies, minimum required is {self.min_companies}"
            )
