    ...
    def create_guardrail(config: Union[str, Dict[str, Any]]) -> Optional[BaseGuardrail]:
        ...
        try:
            ...
            if guardrail_type == "company_count":
                ...
                logger.info(
                    f"Successfully created DataProcessingGuardrail: {guardrail}"
                )
                ...
                logger.info(
                    f"Successfully created EmptyDataProcessingGuardrail: {guardrail}"
                )
                ...
                logger.info(
                    f"Successfully created DataProcessingCountGuardrail: {guardrail}"
                )
                ...
                logger.info(
                    f"Successfully created CompanyNameNotNullGuardrail: {guardrail}"
                )
                ...
                logger.info(
                    f"Successfully created SelfReflectionGuardrail: {guardrail}"
                )
            else:
            ...
            if guardrail is None:
                logger.error(
                    f"Failed to create guardrail of type {guardrail_type} - returned None"
                )
                return None
