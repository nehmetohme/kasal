...
class DocumentationEmbeddingService:
    ...
    async def _get_databricks_storage(self, user_token: Optional[str] = None):
        ...
        try:
            ...
            if not db_config:
                logger.warning(
                    "Databricks configuration is None, cannot initialize storage"
                )
                return None
            ...
            if not index_name:
                ...
                if memory_index:
                    index_name = (
                        memory_index.rsplit(".", 1)[0] + ".documentation_embeddings"
                    )
                else:
            ...
            logger.info(
                f"Checking if index is ready before initializing Databricks storage - endpoint: {endpoint_name}, index: {index_name}"
            )

            # Use DatabricksIndexService to wait for index readiness with retries
            from src.services.databricks.vector_search.index import (
            ...
            if not readiness_result.get("ready"):
                ...
                logger.info(
                    f"Index {index_name} not ready after {attempts} attempts ({elapsed_time:.1f}s): {message}"
                )
                logger.info(
                    "Skipping Databricks storage initialization - will retry on next embedding attempt"
                )
                return None

            logger.info(
                f"Index {index_name} is ready after {readiness_result.get('attempts', 0)} attempts ({readiness_result.get('elapsed_time', 0):.1f}s)"
            )

            # Only create DatabricksVectorStorage if index is ready
            logger.info(
                f"Index is ready, initializing Databricks storage with endpoint: {endpoint_name}, index: {index_name}"
            )
            ...
            logger.info(
                f"Successfully initialized Databricks storage for documentation with endpoint: {endpoint_name}, index: {index_name}"
            )
            return self._databricks_storage
