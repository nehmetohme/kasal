...
class DatabricksVectorStorage:
    ...
    def __init__(
        ...
        self.memory_logger.info(
            f"Initializing Databricks Vector Storage for {memory_type} memory"
        )
            ...
            try:
                ...
                if auth:
                    ...
                    self.memory_logger.debug(
                        f"Using unified {auth.auth_method} authentication for Vector Storage"
                    )
            except Exception as e:
                self.memory_logger.warning(
                    f"Failed to get unified auth for Vector Storage: {e}"
                )
                workspace_url = ""
        ...
        if not os.environ.get("USE_NULLPOOL"):
            os.environ["USE_NULLPOOL"] = "true"
            self.memory_logger.info(
                "Set USE_NULLPOOL=true to prevent asyncpg connection pool issues"
            )
        ...
        try:
            ...
            if self.memory_type == "short_term":
                ...
                if "session_id" in schema:
                    ...
                    record["session_id"] = self.job_id or data.get("context", {}).get(
                        "session_id", str(uuid.uuid4())
                    )
                if "interaction_sequence" in schema:
                    record["interaction_sequence"] = data.get("context", {}).get(
                ...
                if "llm_model" in schema:
                    record["llm_model"] = data.get(
                        "llm_model",
                        data.get("metadata", {}).get("llm_model", "unknown"),
                    )
                ...
                if "embedding_model" in schema:
                    record["embedding_model"] = data.get(
                ...
                if "task_hash" in schema:
                    record["task_hash"] = hashlib.md5(
                        data.get("task_description", "").encode()
                    ).hexdigest()
                ...
                if "embedding" in schema:
                    record["embedding"] = embedding
                if "embedding_model" in schema:
                    record["embedding_model"] = data.get("metadata", {}).get(
                        "embedding_model", "databricks-gte-large-en"
                    )
                if "version" in schema:
                    record["version"] = 1
                ...
                if isinstance(record["embedding"], np.ndarray):
                    ...
                    try:
                        ...
                        self.memory_logger.error(
                            f"Could not convert embedding to list: {type(record['embedding'])}"
                        )

            # Validate record is not empty
            if not record:
                self.memory_logger.error(
                    f"Record is empty after building for memory type {self.memory_type}"
                )
                self.memory_logger.error(f"Original data keys: {list(data.keys())}")
                self.memory_logger.error(f"Schema fields: {list(schema.keys())}")
                raise ValueError(
                    f"Empty record built for memory type {self.memory_type}"
                )

            # Log the record before upsert for debugging
            self.memory_logger.info(
                f"Upserting {self.memory_type} record with {len(record)} fields: {list(record.keys())}"
            )
            self.memory_logger.debug(f"Record ID: {record.get('id', 'NO_ID')}")
            if "embedding" in record:
                self.memory_logger.debug(
                    f"Embedding type: {type(record['embedding'])}, length: {len(record['embedding']) if hasattr(record['embedding'], '__len__') else 'N/A'}"
                )
            ...
            if self.group_id:
                try:
                    ...
                    self.memory_logger.debug(
                        f"Set UserContext with group_id={self.group_id} for authentication"
                    )
                except Exception as e:
            ...
            try:
                index_info = await self.repository.get_index(
                    self.index_name, self.endpoint_name, self.user_token
                )
                if index_info.success and index_info.index:
                    if not index_info.index.ready:
                        self.memory_logger.warning(
                            f"Index {self.index_name} is not ready yet (state: {index_info.index.state})"
                        )
                        # Still try to upsert as it might work
            except Exception as check_error:
                self.memory_logger.warning(
                    f"Could not check index status: {check_error}"
                )

            # Upsert to Databricks Vector Search using repository
            result = await self.repository.upsert(
                self.index_name, self.endpoint_name, [record], self.user_token
            )
            ...
            if not result.get("success"):
                ...
                self.memory_logger.error(
                    f"Record that failed: {json.dumps({k: v for k, v in record.items() if k != 'embedding'}, indent=2)}"
                )
    ...
    async def search(
        ...
        try:
            ...
            self.memory_logger.info(
                f"Performing embedding search on index: {self.index_name}"
            )
            ...
            if results and "result" in results:
                ...
                self.memory_logger.info(
                    f"Embedding search returned {len(data_array)} results"
                )
            else:
                self.memory_logger.info(
            ...
            if results and "result" in results:
                ...
                column_positions = DatabricksIndexSchemas.get_column_positions(
                    self.memory_type
                )
                            ...
                            if col_name in [
                                ...
                                try:
                                    result_dict[col_name] = (
                                        json.loads(row[col_idx]) if row[col_idx] else {}
                                    )
            ...
            self.memory_logger.debug(
                f"Found {len(processed_results)} similar {self.memory_type} memories"
            )
    ...
    async def delete(self, memory_id: str) -> bool:
        ...
        try:
            result = await self.repository.delete_records(
                self.index_name, self.endpoint_name, [memory_id], self.user_token
            )

            if result.get("success"):
                self.memory_logger.debug(
                    f"Deleted {self.memory_type} memory {memory_id} from index {self.index_name}"
                )
                return True
            else:
            ...
            self.memory_logger.error(
                f"Failed to delete from Databricks Vector Search: {e}"
            )
            return False
        ...
        try:
            ...
            if result.get("success") and result.get("results"):
                ...
                if results and "result" in results:
                    data_array = results["result"].get("data_array", [])
                    ids_to_delete = [
                        row[0] for row in data_array if row and len(row) > 0
                    ]

            # Delete all found records using repository
            if ids_to_delete:
                delete_result = await self.repository.delete_records(
                    self.index_name, self.endpoint_name, ids_to_delete, self.user_token
                )

                if delete_result.get("success"):
                    self.memory_logger.info(
                        f"Cleared {len(ids_to_delete)} {self.memory_type} memories for crew {self.crew_id}"
                    )
                else:
                    self.memory_logger.error(
                        f"Failed to delete records: {delete_result.get('message')}"
                    )
                    return False
            else:
                self.memory_logger.info(
                    f"No {self.memory_type} memories found to clear for crew {self.crew_id}"
                )
            ...
            self.memory_logger.error(
                f"Failed to clear memories from Databricks Vector Search: {e}"
            )
            return False
    ...
    async def get_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.memory_logger.error(
                f"Failed to get stats from Databricks Vector Search: {e}"
            )
            return {
                "index_name": self.index_name,
                "endpoint_name": self.endpoint_name,
    ...
    async def count_documents(self) -> int:
        ...
        try:
            ...
            count = await self.repository.count_documents(
                self.index_name, self.endpoint_name, filters, self.user_token
            )

            self.memory_logger.debug(
                f"Counted {count} documents for crew {self.crew_id}"
            )
            return count
