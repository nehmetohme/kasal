...
class PromptInjectionDetector:
    ...
    _HIGH: _PatternList = _compile(
        [
            ...
            (r"\[SYSTEM\]\s*:", "system_marker"),
            (r"\[ADMIN\]\s*:", "admin_marker"),
            (r"\[OVERRIDE\]\s*:", "override_marker"),
            (
    ...
    _MEDIUM: _PatternList = _compile(
        [
            ...
            (r"\boutput\s+the\s+following\b", "output_following"),
            (r"\brepeat\s+after\s+me\b", "repeat_after_me"),
            (
                r"\bprint\s+(the\s+)?(contents?\s+of|your\s+(system\s+)?prompt)\b",
                "print_prompt",
            ),
            (r"\breveal\s+(your\s+)?(system\s+)?prompt\b", "reveal_prompt"),
            (
    ...
    _LOW: _PatternList = _compile(
        [
            (r"\binstructions?\s+override\b", "instructions_override"),
            (r"\bsystem\s+role\b", "system_role_mention"),
        ]
    )
