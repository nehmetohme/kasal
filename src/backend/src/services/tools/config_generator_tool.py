...
class ConfigGeneratorSchema(BaseModel):
    ...
    workspace_id: Optional[str] = Field(
        None,
        description="PBI workspace ID — used to auto-load metadata from Tool 79 cache",
    )
    dataset_id: Optional[str] = Field(
        None,
        description="PBI dataset ID — used to auto-load metadata from Tool 79 cache",
    )
    measures_json: Optional[str] = Field(
        None,
        description="JSON string of measure_table_mapping (from Tool 73/79). If not provided, reads from Tool 79 cache.",
    )
    ...
    catalog: Optional[str] = Field(
        None, description="Target UC catalog for mapping_only_tables"
    )
...
class ConfigGeneratorTool(BaseTool):
    ...
    def _run(self, **kwargs: Any) -> str:
        ...
        try:
            ...
            from src.services.tools.metric_view_utils.scan_data_parser import (
                ScanDataParser,
            )
            ...
            if measures_raw == "[]" and workspace_id and dataset_id:
                logger.info(
                    f"[ConfigGenerator] No measures_json provided, trying Tool 79 cache for workspace={workspace_id}, dataset={dataset_id}"
                )
                try:
                    ...
                    group_id = (
                        ...
                        or (_group_ctx.primary_group_id if _group_ctx else None)
                        or "default"
                    )
                    logger.info(
                    ...
                    if cached:
                        logger.info(
                            f"[ConfigGenerator] Loaded metadata from Tool 79 cache: {len(str(cached))} chars"
                        )
                        ...
                        if cached_measures:
                                ...
                                [
                                    {
                                        ...
                                        "dax_expression": m.get(
                                            "expression", m.get("dax_expression", "")
                                        ),
                                        "table_name": m.get(
                                            "table", m.get("table_name", "")
                                        ),
                                        "proposed_allocation": m.get(
                                            "table", m.get("table_name", "")
                                        ),
                            ...
                            logger.info(
                                f"[ConfigGenerator] Extracted {len(cached_measures)} measures from cache"
                            )

                        if cached_relationships:
                            relationships_raw = json.dumps(cached_relationships)
                            logger.info(
                                f"[ConfigGenerator] Extracted {len(cached_relationships)} relationships from cache"
                            )
                    else:
                        logger.warning(
                            "[ConfigGenerator] No cached metadata found for this workspace/dataset"
                        )
                ...
                for j in joins:
                    ...
                    if on_match:
                        ...
                        for tname, tinfo in mquery_tables.items():
                            if (
                                tname.lower().replace(" ", "_") == alias.lower()
                                or alias in tname.lower()
                            ):
                                dim_table = tinfo
                                break
            ...
            for m in measures:
                ...
                for ref_match in re.finditer(r"(\w+)\[(\w+)\]", dax):
                    ...
                    if snake not in all_cols and dax_col.lower() not in {
            ...
            mapping_tables = {m.get("proposed_allocation", "") for m in measures} - {
                "",
                "__unassigned__",
            }
            ...
            for m in measures:
                ...
                if "SELECTEDVALUE" in dax.upper() and "SWITCH" in dax.upper():
                    ...
                    if branches:
                        for branch_name, branch_expr in branches[
                            :5
                        ]:  # limit per measure
            ...
            try:
                pipeline = MetricViewPipeline(
                    ...
                    scan_data=scan_data,
                    unflatten_tables=bool(scan_data),
                )
                pipeline.run()
                ...
                for spec in pipeline.all_specs.values():
                    for m in spec.untranslatable:
                        ref_match = re.search(
                            r"Cannot resolve \[([^\]]+)\]", m.skip_reason
                        )
                        if ref_match:
            ...
            for table_decomps in switch_decomps.values():
                for d in table_decomps:
                    ...
                    if in_match:
                        values = [
                            v.strip().strip("'\"") for v in in_match.group(1).split(",")
                        ]
            ...
            output = {
                ...
                "summary": {
                    ...
                    "todo_count": sum(
                        ...
                        and any("TODO" in str(vv) for vv in v.values())
                    ),
                },
            }
