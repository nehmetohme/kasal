...
def _log_exception_group(exc: Exception, context: str) -> None:
    ...
    if hasattr(exc, "exceptions"):
        for i, sub_exc in enumerate(exc.exceptions):
            logger.error(
                f"{context} sub-exception [{i}]: {type(sub_exc).__name__}: {sub_exc}"
            )
            # Log HTTP response body for 4xx/5xx errors
            if hasattr(sub_exc, "response"):
                try:
                    body = (
                        sub_exc.response.text[:500]
                        if sub_exc.response.text
                        else "(empty)"
                    )
...
class MCPAdapter:
    ...
    def __init__(self, server_params: Dict[str, Any]):
        ...
        self.initialization_error: Optional[MCPConnectionError] = (
            None  # Structured error for UI
        )
        self._spn_fallback_headers: Optional[Dict[str, str]] = (
    ...
    async def _discover_tools_with_mcp_client(
        ...
        if (
            last_error
            and _is_http_auth_error(last_error)
            and self.server_params.get("auth_type")
            ...
            if spn_headers:
                ...
                if auth_value.startswith("Bearer "):
                    logger.info(
                        f"MCP SPN fallback using Bearer token (length={len(auth_value) - 7})"
                    )
                ...
                try:
                    ...
                    if tools:
                        ...
                        logger.info(
                            f"SPN fallback succeeded via streamable HTTP: {len(tools)} tools. Headers saved for tool execution."
                        )
                        return tools
                except Exception as e:
                ...
                try:
                    ...
                    if tools:
                        ...
                        self._spn_fallback_headers = spn_clean
                        logger.info(
                            f"SPN fallback succeeded via SSE: {len(tools)} tools. Headers saved for tool execution."
                        )
                        return tools
                except Exception as e:
                    last_error = e
    ...
    def _fail_discovery_with(
        self, mcp_error: BaseException, cause: Exception
    ) -> List[Dict[str, Any]]:
    ...
    async def _discover_via_streamable_http(
        self, clean_headers: Dict[str, str]
    ) -> List[Dict[str, Any]]:
        ...
        async def _attempt() -> List[Dict[str, Any]]:
            async with connect(
                self.server_url, headers=clean_headers, timeout=self.timeout_seconds
            ) as (read_stream, write_stream, _):
                logger.info(
                    "Connected to MCP streamable HTTP endpoint for tool discovery"
                )
    ...
    def _convert_parameters(
        ...
        try:
            ...
            if not schema:
                logger.warning(
                    f"No schema found for tool {tool_name}, using parameters as-is"
                )
                return parameters
            ...
            for param_name, param_value in parameters.items():
                ...
                try:
                    ...
                    if param_type == "number":
                        ...
                        # Convert to bool
                        if isinstance(param_value, str):
                            converted[param_name] = param_value.lower() in (
                                "true",
                                "1",
                                "yes",
                            )
                    ...
                    logger.warning(
                        f"Failed to convert parameter {param_name}={param_value} to {param_type}: {e}"
                    )
            ...
            skipped_params = (
                set(parameters.keys()) - set(converted.keys()) - {None, "", "null"}
            )
            if skipped_params:
        ...
        if self._transport == "sse":
            ...
            async with sse_client(
                ...
                async with ClientSession(read_stream, write_stream) as session:
                    await session.initialize()
                    logger.info(
                        f"Executing MCP tool (SSE): {tool_name} with parameters: {params}"
                    )
            ...
            async with connect(
                ...
                async with ClientSession(read_stream, write_stream) as session:
                    await session.initialize()
                    logger.info(
                        f"Executing MCP tool (streamable HTTP): {tool_name} with parameters: {params}"
                    )
                    result = await session.call_tool(tool_name, params)
                    logger.info(
                        f"Tool {tool_name} executed successfully (streamable HTTP)"
                    )
                    return result
    ...
    async def _wait_for_rate_limit(self) -> None:
        ...
        if len(self._call_timestamps) >= self.rate_limit:
            ...
            if wait_time > 0:
                ...
                now = time.monotonic()
                while (
                    self._call_timestamps and (now - self._call_timestamps[0]) > window
                ):
                    self._call_timestamps.popleft()
    ...
    async def _get_spn_fallback_headers(self) -> Optional[Dict[str, str]]:
        ...
        try:
            ...
            if auth_context and auth_context.token:
                logger.info(
                    f"SPN/PAT fallback obtained token via auth method: {auth_context.auth_method}"
                )
    ...
    async def _get_authentication_headers(self) -> Optional[Dict[str, str]]:
        ...
        try:
            ...
            if self._spn_fallback_headers:
                logger.info(
                    "Using SPN fallback headers (OBO was rejected during discovery)"
                )
