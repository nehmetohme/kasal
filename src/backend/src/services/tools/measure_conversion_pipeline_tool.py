...
class MeasureConversionPipelineSchema(BaseModel):
    ...
    powerbi_filter_pattern: Optional[str] = Field(
        None, description="[Power BI] Regex pattern to filter measure names (optional)"
    )
    ...
    yaml_file_path: Optional[str] = Field(
        None, description="[YAML] Path to YAML file (alternative to yaml_content)"
    )
    ...
    sql_process_structures: bool = Field(
        True, description="[SQL] Process time intelligence structures (default: True)"
    )

    # ===== OUTBOUND: UC METRICS CONFIGURATION =====
    uc_catalog: str = Field(
        "main", description="[UC Metrics] Unity Catalog catalog name (default: 'main')"
    )
    uc_schema: str = Field(
        "default",
    ...
    dax_process_structures: bool = Field(
        True, description="[DAX] Process time intelligence structures (default: True)"
    )
...
class MeasureConversionPipelineTool(BaseTool):
    ...
    **Configuration**:
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        logger.info(
            f"[MeasureConversionPipelineTool.__init__] Instance ID: {instance_id}"
        )
        logger.info(
            f"[MeasureConversionPipelineTool.__init__] Received kwargs keys: {list(kwargs.keys())}"
        )
        logger.info(
            f"[MeasureConversionPipelineTool.__init__] inbound_connector: {kwargs.get('inbound_connector', 'NOT PROVIDED')}"
        )
        logger.info(
            f"[MeasureConversionPipelineTool.__init__] powerbi_semantic_model_id: {kwargs.get('powerbi_semantic_model_id', 'NOT PROVIDED')}"
        )
        logger.info(
            f"[MeasureConversionPipelineTool.__init__] outbound_format: {kwargs.get('outbound_format', 'NOT PROVIDED')}"
        )
        logger.info(
            f"[MeasureConversionPipelineTool.__init__] 🔍 powerbi_auth_method FROM TOOL_FACTORY: {kwargs.get('powerbi_auth_method', 'NOT PROVIDED')}"
        )
        ...
        default_config = {
            ...
            "powerbi_info_table_name": kwargs.get(
        ...
        if execution_inputs:
            logger.info(
                f"[MeasureConversionPipelineTool.__init__] Instance {instance_id} - Resolving parameters from execution_inputs: {list(execution_inputs.keys())}"
            )
            resolved_config = {}
    ...
    def _run(self, **kwargs: Any) -> str:
        ...
        try:
            ...
            logger.info("=" * 80)
            logger.info(
                "🔧 TOOL EXECUTION: MeasureConversionPipelineTool._run() STARTED"
            )
            logger.info("=" * 80)
            ...
            logger.info(
                f"[TOOL CALL] Instance {instance_id} - _default_config inbound_connector: {self._default_config.get('inbound_connector', 'NOT SET')}"
            )
            logger.info(
                f"[TOOL CALL] Instance {instance_id} - _default_config powerbi_semantic_model_id: {self._default_config.get('powerbi_semantic_model_id', 'NOT SET')}"
            )
            logger.info(
                f"[TOOL CALL] Instance {instance_id} - _default_config outbound_format: {self._default_config.get('outbound_format', 'NOT SET')}"
            )
            ...
            logger.info(
                f"[MeasureConversionPipelineTool] DEBUG AGENT - powerbi_semantic_model_id value: {kwargs.get('powerbi_semantic_model_id')}"
            )
            logger.info(
                f"[MeasureConversionPipelineTool] DEBUG AGENT - powerbi_group_id value: {kwargs.get('powerbi_group_id')}"
            )
            logger.info(
                f"[MeasureConversionPipelineTool] DEBUG AGENT - powerbi_tenant_id value: {kwargs.get('powerbi_tenant_id')}"
            )
            logger.info(
                f"[MeasureConversionPipelineTool] DEBUG AGENT - powerbi_client_id value: {kwargs.get('powerbi_client_id')}"
            )
            logger.info(
                f"[MeasureConversionPipelineTool] DEBUG AGENT - powerbi_client_secret length: {len(kwargs.get('powerbi_client_secret') or '')}"
            )
            logger.info(
                f"[MeasureConversionPipelineTool] DEBUG AGENT - powerbi_username value: {kwargs.get('powerbi_username')}"
            )
            logger.info(
                f"[MeasureConversionPipelineTool] DEBUG AGENT - powerbi_auth_method value: {kwargs.get('powerbi_auth_method')}"
            )
            ...
            logger.info(
                f"[MeasureConversionPipelineTool] DEBUG MERGED - powerbi_semantic_model_id: {merged_kwargs.get('powerbi_semantic_model_id')}"
            )
            logger.info(
                f"[MeasureConversionPipelineTool] DEBUG MERGED - powerbi_group_id: {merged_kwargs.get('powerbi_group_id')}"
            )
            logger.info(
                f"[MeasureConversionPipelineTool] DEBUG MERGED - powerbi_tenant_id: {merged_kwargs.get('powerbi_tenant_id')}"
            )
            logger.info(
                f"[MeasureConversionPipelineTool] DEBUG MERGED - powerbi_client_id: {merged_kwargs.get('powerbi_client_id')}"
            )
            logger.info(
                f"[MeasureConversionPipelineTool] DEBUG MERGED - powerbi_client_secret length: {len(merged_kwargs.get('powerbi_client_secret') or '')}"
            )
            logger.info(
                f"[MeasureConversionPipelineTool] DEBUG MERGED - powerbi_username: {merged_kwargs.get('powerbi_username')}"
            )
            logger.info(
                f"[MeasureConversionPipelineTool] DEBUG MERGED - powerbi_password length: {len(merged_kwargs.get('powerbi_password') or '')}"
            )
            logger.info(
                f"[MeasureConversionPipelineTool] DEBUG MERGED - powerbi_auth_method: {merged_kwargs.get('powerbi_auth_method')}"
            )
            ...
            logger.info(
                f"[TOOL CALL] Instance {instance_id} - Executing conversion: inbound={inbound_connector}, outbound={outbound_format}"
            )
            ...
            logger.info(
                f"Executing conversion: {inbound_connector} → {outbound_format}"
            )
            ...
            if inbound_connector == "powerbi":
                ...
                logger.info(
                    f"[TOOL DEBUG]   client_id: '{auth_config.get('client_id')}'"
                )
                logger.info(
                    f"[TOOL DEBUG]   client_secret length: {len(auth_config.get('client_secret') or '')}"
                )
                logger.info(f"[TOOL DEBUG]   username: '{auth_config.get('username')}'")
                logger.info(
                    f"[TOOL DEBUG]   password length: {len(auth_config.get('password') or '')}"
                )
                logger.info(
                    f"[TOOL DEBUG]   auth_method (explicit): '{auth_config.get('auth_method')}'"
                )
                logger.info(
                    f"[TOOL DEBUG]   access_token length: {len(auth_config.get('access_token') or '')}"
                )
                ...
                if explicit_auth_method:
                    logger.info(
                        f"[TOOL DEBUG] ✓ EXPLICIT AUTH METHOD (from UI): {explicit_auth_method}"
                    )
                    logger.info(
                        f"[TOOL DEBUG] Authentication will be DETERMINISTIC - only {explicit_auth_method} will be attempted"
                    )
                else:
                ...
                inbound_params = {
                    ...
                    "group_id": group_id,
                    "info_table_name": merged_kwargs.get(
                        "powerbi_info_table_name", "Info Measures"
                    ),
                }
                ...
                if merged_kwargs.get("powerbi_filter_pattern"):
                    extract_params["filter_pattern"] = merged_kwargs[
            ...
            if outbound_format == "sql":
                outbound_params = {
                    ...
                    "process_structures": merged_kwargs.get(
    ...
    def _handle_yaml_conversion(
        ...
        print(
            f"[YAML DEBUG] _handle_yaml_conversion called: outbound_format={outbound_format}"
        )
        print(
            f"[YAML DEBUG] yaml_content length={len(yaml_content) if yaml_content else 0}, yaml_file_path={yaml_file_path}"
        )
        ...
        try:
            ...
            if outbound_format == "dax":
                ...
                logger.info(
                    f"Starting SQL generation for {len(definition.kpis)} KPIs with dialect {dialect}"
                )
                ...
                try:
                    ...
                    print(
                        f"[SQL DEBUG] SQL generation completed: measures_count={result.measures_count}, queries_count={result.queries_count}"
                    )
                    print(
                        f"[SQL DEBUG] Result has sql_queries={hasattr(result, 'sql_queries')}, type={type(result.sql_queries) if hasattr(result, 'sql_queries') else 'N/A'}"
                    )

                    logger.info(
                        f"SQL generation completed: measures_count={result.measures_count if hasattr(result, 'measures_count') else 'N/A'}, queries_count={result.queries_count if hasattr(result, 'queries_count') else 'N/A'}"
                    )
                    ...
                    print(
                        f"[SQL DEBUG] output is list={isinstance(output, list)}, length={len(output) if output else 0}"
                    )
                    if output:
                        print(f"[SQL DEBUG] First query type={type(output[0])}")
                        print(
                            f"[SQL DEBUG] First query has to_sql method={hasattr(output[0], 'to_sql')}"
                        )

                    if not output:
                        print(
                            f"[SQL DEBUG] NO OUTPUT! Result object: measures_count={result.measures_count}, queries_count={result.queries_count}"
                        )
                        print(f"[SQL DEBUG] sql_queries value: {result.sql_queries}")
                        print(
                            f"[SQL DEBUG] sql_measures count: {len(result.sql_measures) if hasattr(result, 'sql_measures') else 'N/A'}"
                        )
                        logger.warning(
                            f"SQL generator returned no queries! Result: {result}"
                        )
                        logger.warning(
                            f"Definition had {len(definition.kpis)} KPIs: {[kpi.technical_name for kpi in definition.kpis]}"
                        )

                    logger.info(
                        f"SQL conversion: result type={type(result)}, sql_queries type={type(result.sql_queries) if hasattr(result, 'sql_queries') else 'N/A'}, count={len(output) if output else 0}"
                    )
                except Exception as e:
    ...
    def _format_output(
        ...
        if outbound_format == "dax":
            ...
            logger.info(
                f"Formatting SQL output: output type={type(output)}, is list={isinstance(output, list)}, length={len(output) if hasattr(output, '__len__') else 'N/A'}"
            )
            formatted = header
            ...
            for i, sql_query in enumerate(output):
                logger.info(
                    f"Processing SQL query {i+1}: type={type(sql_query)}, has original_kbi={hasattr(sql_query, 'original_kbi')}"
                )
                ...
                if isinstance(sql_query, str):
                    ...
                    sql_text = sql_query
                elif isinstance(sql_query, dict) or (
                    hasattr(sql_query, "get") and not hasattr(sql_query, "to_sql")
                ):
                    ...
                    if hasattr(sql_query, "original_kbi") and sql_query.original_kbi:
                        measure_name = (
                            sql_query.original_kbi.technical_name
                            or sql_query.original_kbi.description
                            or "SQL Measure"
                        )
                    else:
                    ...
                    logger.warning(
                        f"Unknown SQL query type {type(sql_query)} for query {i+1}, converting to string"
                    )
                    measure_name = f"SQL Measure {i+1}"
                ...
                formatted += f"```sql\n{sql_text}\n```\n\n"
                logger.info(
                    f"Successfully formatted SQL query {i+1}, length={len(sql_text)}"
                )
