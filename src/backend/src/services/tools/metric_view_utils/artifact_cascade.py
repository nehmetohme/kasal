        ...
        for spec in all_specs.values():
            for m in spec.untranslatable:
                ...
                if (
                    ...
                    m.skip_reason = (
                        "PY/DIVIDE over PBI artifacts (display-only, cross-table)"
                    )
                    changed += 1
        if changed == 0:
            break
...
def collect_unassigned(
    ...
    stats["__unassigned__"] = {
        ...
        "cross_table": len(measures),
        "base": 0,
        "dax": 0,
        "skipped": True,
        "skip_reason": "Unassigned (multi-table or no fact table refs)",
    }
