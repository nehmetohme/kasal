...
_CURATED: dict[str, dict[str, str | None]] = {
    ...
    "PRODUCT": {
        "sql": "EXP(SUM(LN(source.col)))  -- col > 0 only",
        "cls": "translatable_direct",
        "note": "no native product aggregate; log-sum-exp identity, positive values only.",
    },
    "PRODUCTX": {
        "sql": "EXP(SUM(LN(source.factor)))  -- factor > 0",
        "cls": "translatable_direct",
        "note": "per-row expr collapses into the aggregate; positive values only.",
    },
    "COUNTBLANK": {
        "sql": "COUNT(1) - COUNT(source.col)",
        "cls": "translatable_direct",
        "note": "count of NULLs.",
    },
    "APPROXIMATEDISTINCTCOUNT": {
        "sql": "approx_count_distinct(source.col)",
        "cls": "translatable_direct",
        "note": "",
    },
    # Statistical aggregates (these DO appear in measures)
    "MEDIAN": {
        "sql": "median(source.col)",
        "cls": "translatable_direct",
        "note": "or approx_percentile(source.col, 0.5) on large data.",
    },
    "MEDIANX": {
        "sql": "median(source.col)",
        "cls": "translatable_direct",
        "note": "iterator collapses to the aggregate.",
    },
    "PERCENTILE.INC": {
        "sql": "percentile(source.col, 0.75)",
        "cls": "translatable_direct",
        "note": "inclusive.",
    },
    "PERCENTILEX.INC": {
    ...
    "PERCENTILE.EXC": {
        "sql": "percentile_disc(0.75) WITHIN GROUP (ORDER BY source.col)",
        "cls": "translatable_direct",
        "note": "exclusive.",
    },
    "PERCENTILEX.EXC": {
    ...
    "STDEV.S": {
        "sql": "stddev_samp(source.col)",
        "cls": "translatable_direct",
        "note": "",
    },
    "STDEV.P": {
        "sql": "stddev_pop(source.col)",
        "cls": "translatable_direct",
        "note": "",
    },
    "STDEVX.S": {
    ...
    "GEOMEAN": {
        "sql": "EXP(AVG(LN(source.col)))  -- col > 0",
        "cls": "translatable_direct",
        "note": "",
    },
    "GEOMEANX": {
    ...
    # Math traps
    "QUOTIENT": {
        "sql": "DIV(source.a, source.b)",
        "cls": "translatable_direct",
        "note": "integer division.",
    },
    "LOG": {
        "sql": "LOG(base, source.n)",
        "cls": "translatable_direct",
        "note": "ARG ORDER FLIPS vs DAX LOG(n, base).",
    },
    "INT": {"sql": "FLOOR(source.x)", "cls": "translatable_direct", "note": ""},
    "ROUNDDOWN": {
        "sql": "FLOOR(source.x)",
        "cls": "translatable_direct",
        "note": "or TRUNC for toward-zero.",
    },
    "ROUNDUP": {"sql": "CEIL(source.x)", "cls": "translatable_direct", "note": ""},
    "CEILING": {
        "sql": "CEIL(source.x / sig) * sig",
        "cls": "translatable_direct",
        "note": "DAX takes a significance arg.",
    },
    "FLOOR": {
        "sql": "FLOOR(source.x / sig) * sig",
        "cls": "translatable_direct",
        "note": "DAX takes a significance arg.",
    },
    "ACOT": {
        "sql": "ATAN(1.0 / source.x)",
        "cls": "translatable_direct",
        "note": "no native cot family.",
    },
    "COT": {"sql": "1.0 / TAN(source.x)", "cls": "translatable_direct", "note": ""},
    "FACT": {"sql": "FACTORIAL(source.n)", "cls": "translatable_direct", "note": ""},
    "SQRTPI": {
        "sql": "SQRT(source.x * PI())",
        "cls": "translatable_direct",
        "note": "",
    },
    "CURRENCY": {
        "sql": "CAST(source.x AS DECIMAL(19,4))",
        "cls": "translatable_direct",
        "note": "",
    },
    "MROUND": {
        "sql": "ROUND(source.x / m) * m",
        "cls": "translatable_direct",
        "note": "round to nearest multiple m.",
    },
    "EVEN": {
        "sql": None,
        "cls": "architecture_change",
        "note": "round up to even integer — arithmetic UDF or source precompute.",
    },
    "ODD": {
        "sql": None,
        "cls": "architecture_change",
        "note": "round up to odd integer — arithmetic UDF or source precompute.",
    },
    "GCD": {"sql": None, "cls": "architecture_change", "note": "no native GCD — UDF."},
    "LCM": {"sql": None, "cls": "architecture_change", "note": "no native LCM — UDF."},
    "RANDBETWEEN": {
        "sql": "FLOOR(RAND() * (hi - lo + 1)) + lo",
        "cls": "translatable_direct",
        "note": "",
    },
    # Text traps
    "FIND": {
        "sql": "INSTR(source.hay, 'needle')",
        "cls": "translatable_direct",
        "note": "ARG ORDER SWAPS (DAX needle-first); FIND is case-sensitive.",
    },
    "SEARCH": {
        "sql": "INSTR(LOWER(source.hay), LOWER('needle'))",
        "cls": "translatable_direct",
        "note": "ARG ORDER SWAPS; SEARCH is case-insensitive → LOWER both sides.",
    },
    "REPLACE": {
        "sql": "OVERLAY(source.text PLACING 'new' FROM start FOR len)",
        "cls": "translatable_direct",
        "note": "positional REPLACE; NOT value-based (that's SUBSTITUTE → REPLACE()).",
    },
    "SUBSTITUTE": {
        "sql": "REPLACE(source.text, 'old', 'new')",
        "cls": "translatable_direct",
        "note": "value-based replace.",
    },
    "FIXED": {
        "sql": "format_number(source.x, d)",
        "cls": "display_layer",
        "note": "returns a STRING; if a rounded number is intended use ROUND(source.x, d) and flag.",
    },
    "COMBINEVALUES": {
        "sql": "CONCAT_WS('delim', source.a, source.b)",
        "cls": "translatable_direct",
        "note": "",
    },
    "VALUE": {
        "sql": "TRY_CAST(source.s AS DOUBLE)",
        "cls": "translatable_direct",
        "note": "text → number.",
    },
    "CONCATENATEX": {
        "sql": None,
        "cls": "display_layer",
        "note": "string aggregation for a label, not a metric. If a real delimited value: array_join(collect_list(source.col), 'd').",
    },
    "UNICHAR": {"sql": "CHR(source.n)", "cls": "translatable_direct", "note": ""},
    "UNICODE": {"sql": "ASCII(source.s)", "cls": "translatable_direct", "note": ""},
    # Date traps
    "DATEDIFF": {
        "sql": "DATEDIFF(source.b, source.a)  -- DAY; months_between(source.b, source.a) for MONTH/QTR/YEAR",
        "cls": "translatable_direct",
        "note": "ARG ORDER SWAPS vs DAX; unit-dependent function.",
    },
    "EDATE": {
        "sql": "add_months(source.d, n)",
        "cls": "translatable_direct",
        "note": "",
    },
    "EOMONTH": {
        "sql": "last_day(add_months(source.d, n))",
        "cls": "translatable_direct",
        "note": "",
    },
    "YEARFRAC": {
        "sql": "datediff(source.b, source.a) / 365.25",
        "cls": "translatable_direct",
        "note": "approximation; day-count basis ignored — flag if basis matters.",
    },
    "NETWORKDAYS": {
        "sql": None,
        "cls": "architecture_change",
        "note": "business-day count — source-view calendar precompute or UDF.",
    },
    "WEEKDAY": {"sql": "DAYOFWEEK(source.d)", "cls": "translatable_direct", "note": ""},
    "WEEKNUM": {
        "sql": "WEEKOFYEAR(source.d)",
        "cls": "translatable_direct",
        "note": "",
    },
    ...
    "PATH": {
        "sql": None,
        "cls": "architecture_change",
        "note": "recursive CTE in the source view exposing a `path` column: WITH RECURSIVE anc AS (... UNION ALL ...).",
    },
    "PATHITEM": {
        "sql": "element_at(split(source.path, '|'), n)",
        "cls": "translatable_direct",
        "note": "requires `path` precomputed (see PATH).",
    },
    "PATHITEMREVERSE": {
        "sql": "element_at(split(source.path, '|'), -n)",
        "cls": "translatable_direct",
    ...
    "PATHLENGTH": {
        "sql": "size(split(source.path, '|'))",
        "cls": "translatable_direct",
        "note": "requires `path`.",
    },
    "PATHCONTAINS": {
        "sql": "array_contains(split(source.path, '|'), 'x')",
        "cls": "translatable_direct",
        "note": "requires `path`.",
    },
    # Relationship
    "RELATEDTABLE": {
        "sql": None,
        "cls": "architecture_change",
        "note": "aggregate over a declared join; if it needs a grain change, precompute in source. Never a cross-table subquery.",
    },
    # Table manipulation (tail)
    "TOPN": {
        "sql": None,
        "cls": "architecture_change",
        "note": "row-selection; source-view ROW_NUMBER() OVER (ORDER BY … DESC) filtered =1 (or QUALIFY). Never a bare MAX.",
    },
    "GENERATE": {
        "sql": None,
        "cls": "architecture_change",
        "note": "lateral join belongs in the source view.",
    },
    "GENERATEALL": {
        "sql": None,
        "cls": "architecture_change",
        "note": "outer lateral join in the source view.",
    },
    "CROSSJOIN": {
        "sql": None,
        "cls": "architecture_change",
        "note": "set/join op belongs in the source view, not a measure.",
    },
    "UNION": {
        "sql": None,
        "cls": "architecture_change",
        "note": "UNION ALL belongs in the source view.",
    },
    "INTERSECT": {
        "sql": None,
        "cls": "architecture_change",
        "note": "set op in the source view.",
    },
    "EXCEPT": {
        "sql": None,
        "cls": "architecture_change",
    ...
    "SLN": {
        "sql": "(source.cost - source.salvage) / source.life",
        "cls": "translatable_direct",
        "note": "straight-line depreciation.",
    },
    "EFFECT": {
        "sql": "POWER(1 + source.rate / source.n, source.n) - 1",
        "cls": "translatable_direct",
        "note": "",
    },
    "NOMINAL": {
        "sql": "source.n * (POWER(1 + source.eff, 1.0/source.n) - 1)",
        "cls": "translatable_direct",
        "note": "",
    },
    "RRI": {
        "sql": "POWER(source.fv / source.pv, 1.0/source.n) - 1",
        "cls": "translatable_direct",
        "note": "",
    },
    "PDURATION": {
        "sql": "LOG(1 + source.rate, source.fv / source.pv)",
        "cls": "translatable_direct",
        "note": "",
    },
    "XIRR": {
        "sql": None,
        "cls": "architecture_change",
        "note": "iterative solve — existing UC UDF or source precompute. Never invent a UDF name.",
    },
    "RATE": {"sql": None, "cls": "architecture_change", "note": "iterative — UDF."},
    "XNPV": {
        "sql": None,
        "cls": "architecture_change",
        "note": "cash-flow sum over dates — source-view aggregate or UDF.",
    },
    # Information context tests → decline
    "HASONEVALUE": {
        "sql": None,
        "cls": "unsupported",
        "note": "filter-context test; no metric-view meaning.",
    },
    "ISFILTERED": {
        "sql": None,
        "cls": "unsupported",
        "note": "visual filter-context test — skip guard.",
    },
    "ISCROSSFILTERED": {
        "sql": None,
        "cls": "unsupported",
        "note": "filter-context test — skip.",
    },
    "NAMEOF": {
        "sql": None,
        "cls": "unsupported",
        "note": "metadata name reference — not a metric.",
    },
}
...
def _render_entry(name: str) -> str:
    ...
    if cur:
        ...
        sql = cur["sql"]
        sql_line = (
            f"  SQL:  {sql}" if sql else "  SQL:  (not a measure expr — see note)"
        )
...
def render_function_refs(
    dax_expressions: Iterable[str], max_render: int = _MAX_RENDER
) -> str:
