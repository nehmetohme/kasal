...
async def _auth_headers(host_override: Optional[str] = None) -> tuple[str, dict]:
    ...
    if not _host_allowed(workspace_url):
        raise UCQueryError(
            f"untrusted host: {urllib.parse.urlparse(workspace_url).hostname}"
        )
...
async def run_query(
    ...
    try:
        if _resolved is None:
            _resolved = await resolve_workspace_and_warehouse(
...
async def select_distinct(
    ...
    if res.get("success"):
        res["values"] = [
            row[0] for row in res.get("rows", []) if row and row[0] is not None
        ]
    return res
