...
class DataInputHandler:
    ...
    def __init__(
        ...
        if not metrics_view_path or not table_mapping_path:
            raise ValueError(
                "Both metrics_view_path and table_mapping_path must be provided"
            )
    ...
    def find_matching_dax_for_yaml_measure(self, yaml_measure: Dict) -> Optional[Dict]:
        ...
        for dax in all_dax:
            ...
            if dax_name and dax_name.lower() == measure_name.lower():
                logger.debug(
                    "Found case-insensitive match for measure '%s'", measure_name
                )
                return dax
        ...
        if comment and PBI_COMMENT_MARKER in comment:
            try:
                ...
                if match:
                    ...
                    if dax_measure:
                        logger.debug(
                            "Found PBI comment match for measure '%s' -> '%s'",
                            measure_name,
                            pbi_name,
                        )
                        return dax_measure
