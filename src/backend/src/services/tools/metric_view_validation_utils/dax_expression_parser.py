...
_VAR_PATTERN = re.compile(
    r"var\s+(\w+)\s*=\s*(.+?)(?=var\s+\w+\s*=|RETURN|$)", re.IGNORECASE | re.DOTALL
)
...
class DAXExpressionParser:
    ...
    def _extract_variables(self, dax_expr: str) -> List[Dict[str, Any]]:
        ...
        return [
            {"variable_name": v.group(1), "variable_expr": v.group(2)}
    ...
    def check_variable_usage(self, variable_name: str, dax_expr: str) -> bool:
        ...
        matches = list(
            re.finditer(variable_pattern, dax_expr, re.IGNORECASE | re.DOTALL)
        )
        return bool(matches)
    ...
    def decompose_toplevel(self, dax_expr: str) -> Dict[str, Any]:
        ...
        if variables:
            ...
            for i, var in enumerate(variables):
                ...
                if return_expr and self.check_variable_usage(
                    var["variable_name"], return_expr
                ):
                    variables[i]["used_in_return"] = True
                # Check if used in another variable
                for var_expr in variables:
                    if self.check_variable_usage(
                        var["variable_name"], var_expr["variable_expr"]
                    ):
            ...
            return {"type": "no_return", "expr": dax_expr}

    def _substitute_single_variable(
        self, expression: str, var_name: str, var_expr: str
    ) -> str:
        """
    ...
    def substitute_all_variables_recursively(
        ...
        while iteration < _MAX_VAR_SUBSTITUTION_ITERATIONS:
            ...
            for i, var in enumerate(working_vars):
                ...
                other_vars = [
                    ...
                    if v["variable_name"] != var["variable_name"]
                ]
    ...
    def decompose(self, dax_expr: str) -> Dict[str, Any]:
        ...
        if toplevel_components.get("variables"):
            substituted_vars = self.substitute_all_variables_recursively(
                toplevel_components["variables"]
            )
            ...
            for svar in substituted_vars:
                resulting_return_expr = self._substitute_single_variable(
                    resulting_return_expr, svar["variable_name"], svar["variable_expr"]
                )
    ...
    def _parse_condition(self, condition: str) -> Dict[str, Any]:
        ...
        in_match = re.search(
            r"(\w+)\[(\w+)\]\s+in\s*\{([^}]+)\}", condition, re.IGNORECASE
        )
        if in_match:
    ...
    def _extract_aggregations_from_tree(
