...
class ExpressionValidator:
    ...
    def __init__(
        self,
        data_handler: Optional[DataInputHandler] = None,
        ...
        for measure in self.data_handler.mv_parser.measures:
            ...
            if re.search(simple_pattern, expr, re.IGNORECASE):
                ...
                matched_measures.append(
                    {
                        ...
                        "measure_name": measure["name"],
                        "measure_eval_result": self.validate_measure_by_name(
                            measure["name"]
                        ),
                    }
                )
                ...
                unmatched_measures.append(
                    {"measure_eval": "unmatched", "measure_name": measure["name"]}
                )
        return {
    ...
    def validate_measure_by_name(self, measure_name: str) -> Dict[str, Any]:
        ...
        if not self.data_handler:
            raise ValueError(
                "DataInputHandler must be provided to use validate_measure_by_name()"
            )
    ...
    def validate(
        self, db_expr: str, dax_expr: str, strict: bool = False
    ) -> Dict[str, Any]:
        """
        ...
        agg_result = self._compare_aggregations(
            db_parsed["aggregations"], dax_parsed["aggregations"]
        )
        if agg_result["match"]:
        ...
        # 2. Compare filters
        filter_result = self._compare_filters(
            db_parsed["filters"], dax_parsed["filters"]
        )
    ...
    def _compare_aggregations(
        self, db_aggs: List[Dict], dax_aggs: List[Dict]
    ) -> Dict[str, Any]:
        """
        ...
        for db_agg in db_aggs:
            ...
            db_signatures.append(
                {"type": agg_type, "attributes": attributes, "original": db_agg}
            )
        ...
        for dax_idx, dax_sig in enumerate(dax_signatures):
            ...
            for db_idx, db_sig in enumerate(db_signatures):
                ...
                if (
                    db_sig["type"] == dax_sig["type"]
                    and db_sig["attributes"] == dax_sig["attributes"]
                ):
                    # Found a match!
                    ...
                    attrs_str = (
                        ", ".join(sorted(db_sig["attributes"]))
                        if db_sig["attributes"]
                        else "no attributes"
                    )
                    matches.append(
            ...
            if not found_match:
                ...
                for db_idx, db_sig in enumerate(db_signatures):
                    ...
                    if db_sig["type"] == dax_sig["type"]:
                        ...
                        matched_dax_indices.add(dax_idx)
                        dax_attrs = (
                            ", ".join(sorted(dax_sig["attributes"]))
                            if dax_sig["attributes"]
                            else "no attributes"
                        )
                        db_attrs = (
                            ", ".join(sorted(db_sig["attributes"]))
        ...
        details = (
            "; ".join(summary_parts) if summary_parts else "No aggregations to compare"
        )
        if matches:
    ...
    def _filter_signature(parsed_condition: Dict) -> tuple:
        ...
        if cond_type == "UNKNOWN":
            ...
            if in_retry:
                ...
                values = frozenset(
                    v.strip().strip("'\"").lower() for v in in_retry.group(2).split(",")
                )
                return ("IN", column, values)
    ...
    def _compare_filters(
        self, db_filters: List[Dict], dax_filters: List[Dict]
    ) -> Dict[str, Any]:
        ...
        db_sigs = {
            self._filter_signature(f.get("parsed_condition", {})) for f in db_filters
        }
        dax_sigs = {
            self._filter_signature(f.get("parsed_condition", {})) for f in dax_filters
        }
    ...
    def _compare_columns(
        self, db_references: Set[str], dax_references: Set[str]
    ) -> Dict[str, Any]:
        ...
        if all_db_qualified and all_dax_qualified:
            ...
            if db_references == mapped_dax:
                return {
                    "match": True,
                    "details": "All {0} reference(s) match exactly".format(
                        len(db_references)
                    ),
                }
            ...
            return {
                "match": False,
                "details": details,
                "recommendation": "Ensure all table.column references are correct. {0}".format(
                    details
                ),
            }
        ...
        if db_cols == dax_cols:
            return {
                "match": True,
                "details": "All {0} column(s) match (table names may differ or be absent)".format(
                    len(db_cols)
                ),
            }
        ...
        missing_refs = {
            ref for ref in mapped_dax if extract_column(ref) in missing_cols
        }
    ...
    def _compare_structure(
        self, db_structure: Dict, dax_structure: Dict
    ) -> Dict[str, Any]:
