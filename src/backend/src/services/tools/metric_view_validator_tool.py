...
class MetricViewValidatorSchema(BaseModel):
    ...
    ucmv_output: Optional[str] = Field(
        None,
        description="Raw UCMV Generator output JSON (contains yaml, sql, stats keys). Preferred input.",
    )
    yaml_content: Optional[str] = Field(
        None,
        description="JSON dict of table_key → YAML string (alternative to ucmv_output)",
    )
    measures_json: Optional[str] = Field(
        None,
        description="JSON string of measure_table_mapping (same input as UCMV Generator)",
    )
...
class MetricViewValidatorTool(BaseTool):
    ...
    def _run(self, **kwargs: Any) -> str:
        ...
        try:
            ...
            if os.path.exists(_user_edits_path):
                ...
                if isinstance(_user_edits, dict) and "yaml" in _user_edits:
                    logger.info(
                        f"[Validator] Found user-saved UCMV edits in {_user_edits_path} — using over flow output"
                    )
        ...
        if not os.path.exists("/tmp/ucmv_user_edits.json"):
            try:
                ...
                if saved and isinstance(saved, dict) and "yaml" in saved:
                    logger.info(
                        "[Validator] Found user-saved UCMV edits in DB — using edited YAML instead of flow-passed output"
                    )
            ...
            if ucmv_raw:
                ...
                if isinstance(ucmv_data, dict):
                    if "yaml" in ucmv_data:
                        ...
                        yaml_raw = ucmv_data
                logger.info(
                    f"[Validator] Extracted YAML from ucmv_output: {len(yaml_raw) if isinstance(yaml_raw, dict) else 'str'}"
                )

            yaml_tables = (
                json.loads(yaml_raw) if isinstance(yaml_raw, str) else yaml_raw
            )
            measures = (
        ...
        if not yaml_tables:
            logger.info(
                "[Validator] No YAML in args — attempting to fetch from latest UCMV execution trace"
            )
            try:
                ...
                if isinstance(_db_result, dict) and "yaml" in _db_result:
                    ...
                    ucmv_raw = json.dumps(
                        _db_result
                    )  # Store full output for built-in validation
                    logger.info(
                        f"[Validator] Fetched full UCMV output from DB: {len(yaml_tables)} tables"
                    )
        ...
        if not measures or measures == []:
            logger.info(
                "[Validator] No measures in args — fetching from latest UCMV execution trace"
            )
            try:
        ...
        if not yaml_tables:
            logger.info(
                "[Validator] No YAML after DB fallback — checking /tmp for Generator output"
            )
            try:
                ...
                if _tmp_files:
                    ...
                    if isinstance(_tmp_data, dict) and "yaml" in _tmp_data:
                        ...
                        logger.info(
                            f"[Validator] Loaded UCMV output from {_tmp_files[0]} ({len(yaml_tables)} tables)"
                        )
        ...
        if not measures or measures == []:
            if ucmv_raw:
                try:
                    ...
                    if isinstance(ucmv_full, dict) and ucmv_full.get(
                        "resolved_measures_by_table"
                    ):
                        flat = []
                        for _tbl, _rows in ucmv_full[
                            "resolved_measures_by_table"
                        ].items():
                            flat.extend(_rows)
                        if flat:
            ...
            if ucmv_raw:
                try:
                    ...
                    if isinstance(ucmv_full, dict) and "measures_with_dax" in ucmv_full:
                        ...
                        logger.info(
                            f"[Validator] Extracted {len(measures)} measures from UCMV measures_with_dax"
                        )
                except (json.JSONDecodeError, TypeError):
                    pass
        ...
        def _run_with_retry():
            ...
            for attempt in range(5):
                try:
                    ...
                    if attempt < 4:
                        logger.info(
                            f"[Validator] Measures not in DB yet (attempt {attempt+1}/5), retrying in 3s..."
                        )
                        time.sleep(3)
                except Exception as e:
                    logger.debug(
                        f"[Validator] DB query attempt {attempt+1} failed: {e}"
                    )
                    if attempt < 4:
                        time.sleep(2)
            return []
                ...
                if row and row[0]:
                    try:
                        ...
                        if isinstance(edits, dict) and "yaml" in edits:
                            logger.info(
                                "[Validator] Using user-edited UCMV YAML from checkpoint_data.ucmv_yaml_edits"
                            )
                            return edits
                    except Exception:
                        pass
    ...
    def _fetch_latest_ucmv_from_db() -> dict:
                ...
                if row and row[0]:
                    try:
                        ...
                        if isinstance(inner, dict) and "yaml" in inner:
                            logger.info(
                                "[Validator] Found UCMV output in execution_trace"
                            )
                            return inner
                    except Exception:
                        pass
        ...
        for attempt in range(5):
            try:
                ...
                if attempt < 4:
                    logger.info(
                        f"[Validator] UCMV not in DB yet (attempt {attempt+1}/5), retrying in 3s..."
                    )
                    time.sleep(3)
            except Exception as e:
