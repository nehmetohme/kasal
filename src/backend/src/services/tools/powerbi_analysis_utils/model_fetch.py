        ...
        async with httpx.AsyncClient(timeout=180.0) as client:
            try:
                ...
                if response.status_code == 202:
                    ...
                    if not location:
                        logger.warning(
                            "[TMDL] Fabric returned 202 but no Location header"
                        )
                        return None
    ...
    async def _fetch_model_via_admin_scanner(
        ...
        async with httpx.AsyncClient(timeout=120.0) as client:
            ...
            try:
                ...
                logger.warning(
                    f"[Admin Scanner] getInfo failed HTTP {e.response.status_code}"
                )
                return [], []
            except Exception as e:
    ...
    async def _fetch_relationships(
        ...
        async with httpx.AsyncClient(timeout=120.0) as client:
            try:
                ...
                rows = (
                    data.get("results", [{}])[0].get("tables", [{}])[0].get("rows", [])
                )
                relationships = []
                seen_ids = set()
        ...
        for table in tables:
            ...
            for column in columns:
                try:
                    ...
                    if any(
                        suffix in column.lower()
                        for suffix in ["id", "key", "_pk", "_fk"]
                    ):
                        continue
                    ...
                    logger.debug(
                        f"Could not fetch sample values for {table_name}[{column}]: {e}"
                    )
                    continue
    ...
    async def _enrich_model_context_with_metadata(
        ...
        if config.get("enable_info_columns", False):
            try:
                ...
                logger.info(
                    "[Context Enrichment] INFO.COLUMNS() enrichment enabled - fetching column metadata..."
                )
                ...
                for table in tables[:10]:
                    ...
                    try:
                        ...
                        logger.debug(
                            f"[Context Enrichment] Could not fetch metadata for table '{table_name}': {e}"
                        )
                        continue
