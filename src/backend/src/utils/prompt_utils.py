...
def robust_json_parser(text):
    ...
    if re.search(
        r"</(?:think|thinking|reasoning|analysis)>", text, flags=re.IGNORECASE
    ):
        text = re.sub(
            ...
            if _c in "{[":
                ...
                if _depth == 0:
                    try:
                        ...
                        logger.info(
                            "Balanced-brace extraction didn't yield valid JSON, continuing..."
                        )
                    break
    ...
    if truncated_matches:
        try:
            ...
            logger.info(
                "Truncated code block extraction didn't yield valid JSON, continuing..."
            )
    ...
    if matches:
        try:
            ...
            text = matches.group(
                0
            )  # Continue with the extracted text for further fixes
    ...
    try:
        ...
        if stack:
            ...
            logger.info(
                f"Balanced {len(stack)} unclosed braces/brackets in truncated JSON"
            )
