...
def generate_demo():
    ...
    try:
        ...
        for sql_query_obj in sql_result.sql_queries:
            sql_text = (
                sql_query_obj.to_sql()
                if hasattr(sql_query_obj, "to_sql")
                else str(sql_query_obj)
            )
            sql_queries.append(
                (
                    (
                        sql_query_obj.measure_name
                        if hasattr(sql_query_obj, "measure_name")
                        else "SQL Query"
                    ),
                    sql_text,
                )
    ...
    try:
        ...
        uc_metrics_dict = uc_generator.generate_consolidated_uc_metrics(
            definition.kpis, yaml_metadata
        )
