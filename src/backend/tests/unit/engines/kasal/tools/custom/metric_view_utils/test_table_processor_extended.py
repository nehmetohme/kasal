...
from src.engines.kasal.tools.custom.metric_view_utils.table_processor import (
...
class TestProcessTableLlmFallback:
    def test_llm_fallback_called_when_configured(self):
        ...
        with patch(
            'src.engines.kasal.tools.custom.metric_view_utils.table_processor.run_async',
...
class TestProcessTableScanDataMSteps:
    def test_scan_data_with_m_steps(self):
        ...
        from src.engines.kasal.tools.custom.metric_view_utils.data_classes import MStep
        scan_info = MagicMock()
        scan_info.native_sql = 'SELECT region, SUM(amount) AS amount FROM raw.sales GROUP BY region'
...
class TestProcessTableUnionArmLoss:
    def test_union_arm_loss_fallback(self):
        ...
        from src.engines.kasal.tools.custom.metric_view_utils.m_transform_folder import MTransformFolder
...
class TestPromotedConverterEmitterSurvival:
    ...
    def test_sumx_filter_survives_and_uses_source_alias(self):
        ...
        from src.engines.kasal.tools.custom.metric_view_utils.yaml_emitter import emit_yaml
        ti = _make_table_info(
