...
from src.engines.kasal.tools.custom.databricks_jobs_tool import (
    DatabricksJobsTool, 
...
class TestDatabricksJobsTool(unittest.TestCase):
    ...
    def test_run_with_timing_over_2_seconds(self):
        ...
        with patch.object(tool, '_list_jobs', new_callable=AsyncMock, return_value="Results"):
            ...
            def _mock_time():
                ...
                _calls[0] = 2.5
                return val
            with patch('src.engines.kasal.tools.custom.databricks_jobs_tool.time.time',
    ...
    @patch('src.engines.kasal.tools.custom.databricks_jobs_tool.aiohttp.ClientSession')
    ...
    @patch('src.engines.kasal.tools.custom.databricks_jobs_tool.DatabricksJobsTool._make_api_call')
...
class TestCodeExecutionDisabled(unittest.TestCase):
    ...
    def _tool(self, **extra):
        from src.engines.kasal.tools.custom.databricks_jobs_tool import DatabricksJobsTool
        cfg = dict(self.BASE_CFG)
        cfg.update(extra)
        return DatabricksJobsTool(tool_config=cfg)

    def test_create_and_submit_not_in_valid_actions(self):
        from src.engines.kasal.tools.custom.databricks_jobs_tool import VALID_ACTIONS
        self.assertNotIn("create", VALID_ACTIONS)
        self.assertNotIn("submit", VALID_ACTIONS)

    def test_schema_rejects_create(self):
        from src.engines.kasal.tools.custom.databricks_jobs_tool import DatabricksJobsToolSchema
        with self.assertRaises(Exception):
