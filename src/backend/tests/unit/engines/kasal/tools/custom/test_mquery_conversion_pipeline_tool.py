...
from src.engines.kasal.tools.custom.mquery_conversion_pipeline_tool import (
...
class TestCachedDataPath:
    def test_uses_cached_scan_data(self):
        ...
        with patch(
            "src.engines.kasal.tools.custom.mquery_conversion_pipeline_tool.ToolSessionProvider.cache_service",
            fake_cm,
        ):
            tool = MqueryConversionPipelineTool()
            result = tool._run(
...
import asyncio
from src.engines.kasal.tools.custom.mquery_conversion_pipeline_tool import run_sync
...
class TestRunMainPipeline:
    ...
    @patch("src.engines.kasal.tools.custom.mquery_conversion_pipeline_tool.run_sync")
...
class TestExecuteWithValidation:
    ...
    def test_pbi_token_error_returns_error_string(self):
        ...
        with patch.dict("sys.modules", {"src.converters.services.mquery": mock_module}):
            with patch(
                "src.engines.kasal.tools.custom.powerbi_auth_utils.get_powerbi_access_token",
...
class TestSaveToConversionHistory:
    ...
    def test_persists_record_with_raw_mquery(self):
        ...
        with patch(
            "src.engines.kasal.tools.tool_session_provider.ToolSessionProvider.conversion_repo",
            _repo_ctx,
        ):
