...
class TestConfigGenConversionHistoryPersistence:
    ...
    def test_persists_measures_with_dax_diagnostic(self):
        ...
        with patch("src.engines.kasal.tools.tool_session_provider.ToolSessionProvider.conversion_repo", _ctx):
    ...
    def test_fail_open_on_repo_error(self):
        ...
        with patch("src.engines.kasal.tools.tool_session_provider.ToolSessionProvider.conversion_repo", _boom):
...
class TestEtlColumnCuration:
    ...
    def test_etl_columns_excluded_business_columns_kept(self):
        from src.engines.kasal.tools.custom.generate_config import (
...
class TestReportIdAutoDiscovery:
    ...
    def _discover(self, reports, dataset_id="ds1", status=200):
        ...
        from src.engines.kasal.tools.custom import generate_config as gc
...
class TestEnrichFilterSetsFromWarehouse:
    ...
    async def test_fills_filter_set_from_warehouse(self):
        ...
        admin = {"Dim_wkctr": {}}
        with patch("src.engines.kasal.tools.custom.generate_config._detect_cwc_filter_column",
                   return_value="cwc_filter"), \
             patch("src.engines.kasal.tools.custom.metric_view_utils.uc_query.resolve_workspace_and_warehouse",
                   new=AsyncMock(return_value=("https://x.cloud.databricks.com", "wh", {}))), \
             patch("src.engines.kasal.tools.custom.metric_view_utils.uc_query.select_distinct",
...
class TestEnrichFactJoinMapLlm:
    ...
    async def test_drafts_join_key_and_marks_verify(self):
        ...
        with patch("src.engines.kasal.tools.custom.metric_view_utils.uc_query.resolve_workspace_and_warehouse",
                   new=AsyncMock(return_value=("https://x.cloud.databricks.com", "wh", {}))), \
             patch("src.engines.kasal.tools.custom.metric_view_utils.uc_query.run_query",
