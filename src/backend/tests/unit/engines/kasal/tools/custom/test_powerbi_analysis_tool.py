...
from src.engines.kasal.tools.custom.powerbi_analysis_tool import (
...
class TestRunValidation:
    ...
    def test_sp_auth_accepted(self):
        ...
        with patch(
            "src.engines.kasal.tools.custom.powerbi_analysis_tool._run_async_in_sync_context",
...
class TestFetchColumnMetadataForTable:
    ...
    def test_successful_response_returns_columns(self):
        ...
        with patch("src.engines.kasal.tools.custom.powerbi_analysis_tool.httpx.AsyncClient", return_value=mock_client):
            result = self._run(
...
class TestEmitLlmTrace:
    ...
    def test_trace_context_with_job_id_attempts_emit(self):
        ...
        with patch("src.engines.kasal.tools.custom.powerbi_analysis_tool.PowerBIAnalysisTool._emit_llm_trace") as _:
...
class TestFetchModelViaAdminScanner:
    ...
    def test_workspace_not_found_returns_empty(self):
        ...
        with patch("src.engines.kasal.tools.custom.powerbi_analysis_tool.httpx.AsyncClient", return_value=mock_client):
            with patch("src.engines.kasal.tools.custom.powerbi_analysis_tool.asyncio.sleep", return_value=None):
                measures, tables = self._run(
                    self.tool._fetch_model_via_admin_scanner(WS_ID, DS_ID, ACCESS_TOKEN, {})
                )
...
class TestFetchModelViaDAX:
    ...
    def test_successful_table_extraction(self):
        ...
        with patch("src.engines.kasal.tools.custom.powerbi_analysis_tool.httpx.AsyncClient", return_value=mock_instance):
            measures, tables = self._run(
...
class TestFindVisualReferences:
    ...
    def test_exception_returns_empty_list(self):
        with patch("src.engines.kasal.tools.custom.powerbi_analysis_tool.httpx.AsyncClient") as mock_cls:
            mock_client = MagicMock()
...
class TestFetchTmdlViaFabric202Polling:
    ...
    def test_202_succeeded_returns_parts(self):
        ...
        with patch("src.engines.kasal.tools.custom.powerbi_analysis_tool.httpx.AsyncClient", return_value=mock_client), \
             patch("src.engines.kasal.tools.custom.powerbi_analysis_tool.asyncio.sleep", return_value=None):
            result = self._run(
...
class TestEmitLlmTraceWithContext:
    ...
    def test_emit_trace_exception_does_not_propagate(self):
        ...
        with patch("src.engines.kasal.tools.custom.powerbi_analysis_tool.get_trace_queue",
