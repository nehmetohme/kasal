...
from src.engines.kasal.tools.custom.powerbi_semantic_model_fetcher_tool import (
...
class TestFetcherRunValidation:
    ...
    def test_sp_auth_accepted(self):
        ...
        with patch(
            "src.engines.kasal.tools.custom.powerbi_semantic_model_fetcher_tool._run_async_in_sync_context",
...
class TestFetcherTmdlViaFabric:
    ...
    def test_200_response_returns_parts(self):
        ...
        with patch("src.engines.kasal.tools.custom.powerbi_semantic_model_fetcher_tool.httpx.AsyncClient", return_value=mock_client):
...
class TestFetcherModelViaAdminScanner:
    ...
    def test_successful_scan_returns_data(self):
        ...
        with patch("src.engines.kasal.tools.custom.powerbi_semantic_model_fetcher_tool.httpx.AsyncClient", return_value=mock_client):
            with patch("src.engines.kasal.tools.custom.powerbi_semantic_model_fetcher_tool.asyncio.sleep", return_value=None):
                measures, tables = self._run(
                    self.tool._fetch_model_via_admin_scanner(WS_ID, DS_ID, ACCESS_TOKEN, {})
                )
...
class TestFetcherExtractReportDefinitionParts:
    ...
    def test_202_with_location_succeeds(self):
        ...
        with patch("src.engines.kasal.tools.custom.powerbi_semantic_model_fetcher_tool.httpx.AsyncClient", return_value=mock_client), \
             patch("src.engines.kasal.tools.custom.powerbi_semantic_model_fetcher_tool.asyncio.sleep", return_value=None):
            result = self._run(self.tool._extract_report_definition_parts(WS_ID, "rpt-1", ACCESS_TOKEN))
...
class TestFetcherGetFabricToken:
    ...
    def test_get_fabric_token_calls_helper(self):
        ...
        with patch(
            "src.engines.kasal.tools.custom.powerbi_semantic_model_fetcher_tool.PowerBISemanticModelFetcherTool._get_fabric_token",
