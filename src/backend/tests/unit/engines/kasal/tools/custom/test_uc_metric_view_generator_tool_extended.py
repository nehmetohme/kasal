...
from src.engines.kasal.tools.custom.uc_metric_view_generator_tool import (
...
class TestRunValidation:
    def test_validation_runs_with_measures(self):
        ...
        with patch(
            'src.engines.kasal.tools.custom.uc_metric_view_generator_tool.UCMetricViewGeneratorTool._run'
        ) as mock_run:
...
class TestExtractFromPbiApi:
    ...
    def test_extract_with_access_token_skips_auth(self):
        ...
        with (
            patch('src.engines.kasal.tools.custom.uc_metric_view_generator_tool.UCMetricViewGeneratorTool._extract_from_pbi_api',
    ...
    def test_extract_measure_extraction_exception_logged(self):
        ...
        tool = self._make_tool()
        with (
            patch('src.engines.kasal.tools.custom.uc_metric_view_generator_tool._run_async',
    ...
    def test_extract_no_access_token_calls_auth(self):
        ...
        with (
            patch('src.engines.kasal.tools.custom.powerbi_auth_utils.get_powerbi_access_token_from_config',
                  mock_get_token),
            patch('src.engines.kasal.tools.custom.uc_metric_view_generator_tool._run_async',
...
class TestExtractFromPbiApiMQueryScan:
    def test_mquery_extraction_success(self):
        ...
        with (
            patch(
                'src.engines.kasal.tools.custom.uc_metric_view_generator_tool._run_async',
