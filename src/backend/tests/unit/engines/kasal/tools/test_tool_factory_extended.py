"""
Extended unit tests for src/engines/kasal/tools/tool_factory.py
...
class TestGetApiKeyAsync:
    ...
    async def test_returns_key_from_service(self):
        ...
        with patch("src.engines.kasal.tools.tool_factory.EncryptionUtils.decrypt_value", return_value="plain-key"):
...
class TestDatabricksKnowledgeSearchToolCreation:
    ...
    def test_creates_knowledge_search_tool_with_mock(self):
        ...
        with patch(
            "src.engines.kasal.tools.tool_factory.DatabricksKnowledgeSearchTool",
            mock_cls,
        ):
