...
class TestAgent:
    ...
    def test_agent_creation_minimal(self):
        ...
        # Act
        agent = Agent(name=name, role=role, goal=goal)
    ...
    def test_agent_defaults(self):
        ...
        # Arrange & Act
        agent = Agent(name="Test Agent", role="Test Role", goal="Test Goal")
    ...
    def test_agent_init_method_logic(self):
        ...
        agent1 = Agent(name="Agent 1", role="Role 1", goal="Goal 1", tools=None)
        assert agent1.tools == []

        # Test 2: When knowledge_sources is None
        agent2 = Agent(
            name="Agent 2", role="Role 2", goal="Goal 2", knowledge_sources=None
        )
    ...
    def test_agent_llm_configurations(self):
        ...
        for llm in llm_configs:
            agent = Agent(
                name=f"Agent for {llm}", role="Test Role", goal="Test Goal", llm=llm
            )
            assert agent.llm == llm
    ...
    def test_agent_code_execution_settings(self):
        ...
        assert (
            no_code_agent.code_execution_mode is None
            or no_code_agent.code_execution_mode == "safe"
        )
    ...
    def test_agent_repr(self):
        ...
        # Arrange
        agent = Agent(
            name="Repr Test Agent", role="Test Role", goal="Test representation"
        )
...
class TestAgentEdgeCases:
    ...
    def test_agent_with_empty_strings(self):
        ...
        # Act
        agent = Agent(name="", role="", goal="", backstory="")
    ...
    def test_agent_with_very_long_strings(self):
        ...
        # Act
        agent = Agent(
            name=long_name, role=long_role, goal=long_goal, backstory=long_backstory
        )
...
class TestAgentDateAwareness:
    ...
    def test_date_format_column_exists(self):
        ...
        assert "VARCHAR" in str(columns["date_format"].type) or "STRING" in str(
            columns["date_format"].type
        )
