...
class TestDataProcessing:
    ...
    def test_data_processing_init_full(self):
        ...
        data_processing = DataProcessing(
            che_number="CHE123456789", processed=True, company_name="Test Company"
        )
    ...
    def test_data_processing_init_processed_none(self):
        ...
        data_processing = DataProcessing(che_number="CHE123456789", processed=None)
    ...
    def test_data_processing_init_processed_explicit_false(self):
        ...
        data_processing = DataProcessing(che_number="CHE123456789", processed=False)
    ...
    def test_data_processing_init_processed_explicit_true(self):
        ...
        data_processing = DataProcessing(che_number="CHE123456789", processed=True)
    ...
    def test_data_processing_column_types(self):
        ...
        assert (
            "VARCHAR" in str(columns["che_number"].type).upper()
            or "STRING" in str(columns["che_number"].type).upper()
        )
        assert "BOOLEAN" in str(columns["processed"].type).upper()
        assert (
            "VARCHAR" in str(columns["company_name"].type).upper()
            or "STRING" in str(columns["company_name"].type).upper()
        )
    ...
    def test_data_processing_init_with_datetime_fields(self):
        ...
        now = datetime.utcnow()
        data_processing = DataProcessing(
            che_number="CHE123456789", created_at=now, updated_at=now
        )
    ...
    def test_data_processing_init_rejects_extra_kwargs(self):
        ...
        with pytest.raises(TypeError, match="invalid keyword argument"):
            DataProcessing(che_number="CHE123456789", extra_field="ignored")
    ...
    def test_data_processing_company_name_string_values(self):
        ...
        for company_name in test_cases:
            data_processing = DataProcessing(
                che_number=f"CHE{hash(str(company_name))}", company_name=company_name
            )
