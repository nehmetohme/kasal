...
class TestDocumentationEmbedding:
    ...
    def test_documentation_embedding_repr(self):
        ...
        assert (
            result
            == "DocumentationEmbedding(id=1, source=test_source, title=Test Title)"
        )
    ...
    def test_documentation_embedding_json_column(self):
        ...
        assert hasattr(doc_metadata_column.type, "python_type") or "JSON" in str(
            type(doc_metadata_column.type)
        )
    ...
    def test_documentation_embedding_string_columns(self):
        ...
        assert (
            "STRING" in str(type(source_column.type)).upper()
            or "VARCHAR" in str(type(source_column.type)).upper()
        )
        assert (
            "STRING" in str(type(title_column.type)).upper()
            or "VARCHAR" in str(type(title_column.type)).upper()
        )
