...
class TestFlowExecutionCreate:
    ...
    def test_flow_execution_create_same_validation(self):
        ...
        create_execution = FlowExecutionCreate(flow_id=uuid4(), job_id="valid_job")
...
class TestFlowNodeExecutionUpdate:
    ...
    def test_valid_flow_node_execution_update_complete(self):
        ...
        result_data = {
            "node_output": "processed successfully",
            "metrics": {"duration": 45},
        }
        update = FlowNodeExecutionUpdate(
            status=FlowExecutionStatus.COMPLETED, result=result_data, error=None
        )
        assert update.status == FlowExecutionStatus.COMPLETED
        assert update.result == result_data
        assert update.error is None
...
class TestFlowExecutionDetailResponse:
    ...
    def test_flow_execution_detail_response_inheritance(self):
        ...
        detail_response = FlowExecutionDetailResponse(
            flow_id=flow_id, job_id="inheritance_test", id=456, created_at=now
        )
...
class TestFlowExecutionSchemaIntegration:
    ...
    def test_complete_flow_execution_lifecycle(self):
        ...
        execution_response = FlowExecutionResponse(
            **create_execution.model_dump(), id=100, created_at=now
        )

        # Update execution to running
        update_to_running = FlowExecutionUpdate(status=FlowExecutionStatus.RUNNING)
    ...
    def test_flow_node_execution_workflow(self):
        ...
        for i, node_id in enumerate(["input", "process", "output"]):
            ...
            node_response = FlowNodeExecutionResponse(
                **create_node.model_dump(), id=i + 1, created_at=now
            )
    ...
    def test_flow_execution_error_handling(self):
        ...
        failed_detail = FlowExecutionDetailResponse(
            **failed_execution.model_dump(), nodes=[failed_node]
        )
    ...
    def test_complex_flow_execution_scenario(self):
        ...
        complex_detail = FlowExecutionDetailResponse(
            **complex_execution.model_dump(), nodes=node_executions
        )
        ...
        # Count node statuses
        completed_nodes = [
            n for n in complex_detail.nodes if n.status == FlowExecutionStatus.COMPLETED
        ]
        running_nodes = [
            n for n in complex_detail.nodes if n.status == FlowExecutionStatus.RUNNING
        ]
        pending_nodes = [
            n for n in complex_detail.nodes if n.status == FlowExecutionStatus.PENDING
        ]
        ...
        data_transformer = next(
            n for n in complex_detail.nodes if n.node_id == "data_transformer"
        )
