...
class TestAgent:
    ...
    def test_valid_agent(self):
        ...
        assert (
            agent.backstory
            == "Expert with 10 years of experience in data science and analytics"
        )
...
class TestTaskGenerationRequest:
    ...
    def test_valid_task_generation_request_minimal(self):
        ...
        request_data = {"text": "Generate a task for data analysis"}
    ...
    def test_task_generation_request_various_models(self):
        ...
        for model in models:
            request = TaskGenerationRequest(text="Generate task", model=model)
    ...
    def test_task_generation_request_with_agent_object(self):
        ...
        request = TaskGenerationRequest(text="Generate a specialized task", agent=agent)
    ...
    def test_task_generation_request_markdown_variations(self):
        ...
        request_markdown = TaskGenerationRequest(
            text="Generate task with markdown", markdown=True
        )
        assert request_markdown.markdown is True

        # Markdown disabled
        request_no_markdown = TaskGenerationRequest(
            text="Generate task without markdown", markdown=False
        )
...
class TestAdvancedConfig:
    ...
    def test_advanced_config_output_configurations(self):
        ...
        pydantic_config = AdvancedConfig(output_pydantic="AnalysisResult")
        assert pydantic_config.output_pydantic == "AnalysisResult"

        # File output
        file_config = AdvancedConfig(output_file="/outputs/analysis_results.json")
    ...
    def test_advanced_config_retry_and_timeout(self):
        ...
        retry_config = AdvancedConfig(retry_on_fail=True, max_retries=10, timeout=600)
        assert retry_config.retry_on_fail is True
        assert retry_config.max_retries == 10
        assert retry_config.timeout == 600

        # No retry configuration
        no_retry_config = AdvancedConfig(retry_on_fail=False, max_retries=0)
    ...
    def test_advanced_config_priority_and_dependencies(self):
        ...
        simple_config = AdvancedConfig(priority=1, dependencies=[])
    ...
    def test_advanced_config_caching_configurations(self):
        ...
        long_cache = AdvancedConfig(cache_response=True, cache_ttl=86400)  # 24 hours
        assert long_cache.cache_response is True
        assert long_cache.cache_ttl == 86400

        # No cache configuration
        no_cache = AdvancedConfig(cache_response=False, cache_ttl=0)
...
class TestTaskGenerationResponse:
    ...
    def test_valid_task_generation_response_minimal(self):
        ...
        assert (
            ...
            == "Analyze the provided dataset to identify key trends"
        )
        assert (
    ...
    def test_valid_task_generation_response_complete(self):
        ...
        advanced_config = AdvancedConfig(
            async_execution=True, timeout=300, priority=5, cache_response=True
        )
        ...
        assert (
            response.description
            == "Perform comprehensive data analysis with multiple tools"
        )
        assert (
            response.expected_output
            == "Detailed analysis report with visualizations and ML insights"
        )
    ...
    def test_task_generation_response_tools_validation(self):
        ...
        response_empty_tools = TaskGenerationResponse(
            name="task", description="description", expected_output="output", tools=[]
        )
        ...
        complex_tools = [
            {
                ...
                "credentials": {"username": "analyst", "password_env": "DB_PASSWORD"},
            },
            {
                "name": "ml_pipeline",
                "type": "processing",
        ...
        assert (
            response_complex_tools.tools[1]["parameters"]["algorithm"]
            == "random_forest"
        )
    ...
    def test_task_generation_response_realistic_examples(self):
        ...
        data_analysis = TaskGenerationResponse(
            ...
            advanced_config=AdvancedConfig(
                timeout=600, priority=8, cache_response=True, cache_ttl=7200
            ),
        )
...
class TestTaskGenerationSchemaIntegration:
    ...
    def test_task_generation_with_dependencies(self):
        ...
        setup_config = AdvancedConfig(
            priority=10, dependencies=[], async_execution=False
        )
    ...
    def test_task_generation_error_handling_scenarios(self):
        ...
        strict_config = AdvancedConfig(
            error_handling="strict", retry_on_fail=False, max_retries=0, timeout=60
        )
        ...
        lenient_config = AdvancedConfig(
            error_handling="lenient", retry_on_fail=True, max_retries=5, timeout=300
        )
...
class TestLLMGuardrailConfig:
    ...
    def test_valid_llm_guardrail_config(self):
        ...
        config = LLMGuardrailConfig(description="Validate output format and accuracy")
    ...
    def test_llm_guardrail_config_with_custom_model(self):
        ...
        config = LLMGuardrailConfig(
            description="Check for factual accuracy", llm_model="gpt-4"
        )
...
class TestTaskGenerationResponseWithGuardrail:
    ...
    def test_task_generation_response_with_guardrail(self):
        ...
        assert (
            response.llm_guardrail.description
    ...
    def test_task_generation_response_guardrail_serialization(self):
        ...
        guardrail = LLMGuardrailConfig(
            description="Validate data integrity", llm_model="claude-3-opus"
        )
        response = TaskGenerationResponse(
        ...
        assert (
            response_dict["llm_guardrail"]["description"] == "Validate data integrity"
        )
...
class TestTaskGenerationRequestAvailableTools:
    ...
    def test_task_generation_request_available_tools(self):
        ...
        request = TaskGenerationRequest(
            text="Analyze quarterly sales data", available_tools=tools
        )
    ...
    def test_task_generation_request_available_tools_empty_list(self):
        ...
        request = TaskGenerationRequest(text="Generate a task", available_tools=[])
