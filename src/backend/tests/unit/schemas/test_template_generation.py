...
class TestTemplateGenerationRequest:
    ...
    def test_template_generation_request_missing_required_fields(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            TemplateGenerationRequest(goal="Test goal", backstory="Test backstory")
        ...
        # Missing goal
        with pytest.raises(ValidationError) as exc_info:
            TemplateGenerationRequest(role="Test role", backstory="Test backstory")
        errors = exc_info.value.errors()
        missing_fields = [
            error["loc"][0] for error in errors if error["type"] == "missing"
        ]
        ...
        # Missing backstory
        with pytest.raises(ValidationError) as exc_info:
            TemplateGenerationRequest(role="Test role", goal="Test goal")
        errors = exc_info.value.errors()
        missing_fields = [
            error["loc"][0] for error in errors if error["type"] == "missing"
        ]
    ...
    def test_template_generation_request_empty_fields(self):
        ...
        request_data = {"role": "", "goal": "", "backstory": ""}
        request = TemplateGenerationRequest(**request_data)
        assert request.role == ""
        assert request.goal == ""
...
class TestTemplateGenerationResponse:
    ...
    def test_template_generation_response_missing_required_fields(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            TemplateGenerationResponse(
                prompt_template="Test prompt", response_template="Test response"
            )
        errors = exc_info.value.errors()
        missing_fields = [
            error["loc"][0] for error in errors if error["type"] == "missing"
        ...
        with pytest.raises(ValidationError) as exc_info:
            TemplateGenerationResponse(
                system_template="Test system", response_template="Test response"
            )
        errors = exc_info.value.errors()
        missing_fields = [
            error["loc"][0] for error in errors if error["type"] == "missing"
        ...
        with pytest.raises(ValidationError) as exc_info:
            TemplateGenerationResponse(
                system_template="Test system", prompt_template="Test prompt"
            )
        errors = exc_info.value.errors()
        missing_fields = [
            error["loc"][0] for error in errors if error["type"] == "missing"
...
class TestTemplateGenerationIntegration:
    ...
    def test_template_validation_scenarios(self):
        ...
        minimal_request = TemplateGenerationRequest(
            role="Assistant", goal="Help users", backstory="Helpful AI"
        )
...
{references}""",
        ...
        assert (
            complex_response.response_template.count("{") >= 6
    ...
    def test_edge_cases_and_error_handling(self):
        ...
        long_request = TemplateGenerationRequest(
            role=very_long_text, goal=very_long_text, backstory=very_long_text
        )
        assert len(long_request.role) == 10000

        # Test with minimal content
        minimal_response = TemplateGenerationResponse(
            system_template="x", prompt_template="y", response_template="z"
        )
        assert minimal_response.system_template == "x"

        # Test with None values should fail validation
        with pytest.raises(ValidationError):
            TemplateGenerationRequest(role=None, goal="test", backstory="test")

        with pytest.raises(ValidationError):
            TemplateGenerationResponse(
                system_template=None, prompt_template="test", response_template="test"
            )
