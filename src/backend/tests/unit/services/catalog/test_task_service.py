...
def task_service(mock_session, mock_repository):
    ...
    with patch(
        "src.services.catalog.tasks.TaskRepository", return_value=mock_repository
    ):
...
class TestTaskServiceCreate:
    ...
    async def test_create_success(
        ...
        created_task = MockTask(
            name=sample_task_create.name, description=sample_task_create.description
        )
    ...
    @pytest.mark.asyncio
    async def test_create_with_agent_id(
        self, task_service, mock_repository, sample_task_create_with_agent
    ):
...
class TestTaskServiceUpdateWithPartialData:
    ...
    async def test_update_with_partial_data_success(
        self, task_service, mock_repository, sample_task_update
    ):
        """Test successful partial update."""
        updated_task = MockTask(
            name=sample_task_update.name, description=sample_task_update.description
        )
        mock_repository.update.return_value = updated_task

        result = await task_service.update_with_partial_data(
            "task-123", sample_task_update
        )
    ...
    @pytest.mark.asyncio
    async def test_update_with_partial_data_empty_agent_id(
        self, task_service, mock_repository
    ):
    ...
    async def test_update_with_partial_data_not_found(
        ...
        result = await task_service.update_with_partial_data(
            "non-existent", sample_task_update
        )
...
class TestTaskServiceCreateWithGroup:
    ...
    async def test_create_with_group_success(
        self, task_service, mock_repository, sample_task_create, sample_group_context
    ):
        ...
        result = await task_service.create_with_group(
    ...
    @pytest.mark.asyncio
    async def test_create_with_group_empty_agent_id(
        self, task_service, mock_repository, sample_group_context
    ):
    ...
    async def test_create_with_group_all_fields(
        ...
        result = await task_service.create_with_group(
            full_task_data, sample_group_context
        )
...
class TestTaskServiceFindByGroup:
    ...
    async def test_find_by_group_success(
        self, task_service, mock_session, sample_group_context
    ):
    ...
    async def test_find_by_group_empty_group_context(
        self, task_service, sample_group_context
    ):
    ...
    @pytest.mark.asyncio
    async def test_find_by_group_no_tasks(
        self, task_service, mock_session, sample_group_context
    ):
...
@pytest.mark.asyncio
async def test_update_with_partial_data_exclude_none_and_empty_agent_to_none(
    monkeypatch,
):
    from src.services.catalog import tasks as module
