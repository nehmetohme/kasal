...
async def test_connection_inner_exception():
    ...
    with patch('src.services.databricks.connection.DatabricksVectorEndpointRepository') as MockEndpointRepo, \
         patch('src.services.databricks.connection.DatabricksVectorIndexRepository'):
...
async def test_connection_import_error():
    ...
    with patch('src.services.databricks.connection.DatabricksVectorEndpointRepository',
...
async def test_get_auth_token_success():
    ...
    with patch('src.services.databricks.connection.get_auth_context', new_callable=AsyncMock, return_value=mock_auth):
...
async def test_get_auth_token_no_auth_raises():
    ...
    with patch('src.services.databricks.connection.get_auth_context', new_callable=AsyncMock, return_value=None):
...
async def test_get_auth_token_exception_raises():
    ...
    with patch('src.services.databricks.connection.get_auth_context',
...
async def test_get_endpoint_status_success():
    ...
    with patch('src.services.databricks.connection.DatabricksVectorEndpointRepository') as MockRepo:
