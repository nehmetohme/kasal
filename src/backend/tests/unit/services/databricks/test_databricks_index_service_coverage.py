...
def make_svc(workspace_url=None):
    with patch("src.services.databricks.index.DatabricksVectorIndexRepository"), \
         patch("src.services.databricks.index.DatabricksVectorEndpointRepository"):
...
class TestGetIndexRepository:
    def test_creates_new_repo(self):
        svc = make_svc()
        with patch("src.services.databricks.index.DatabricksVectorIndexRepository") as cls:
...
class TestGetEndpointRepository:
    def test_creates_new_endpoint_repo(self):
        svc = make_svc()
        with patch("src.services.databricks.index.DatabricksVectorEndpointRepository") as cls:
...
class TestCreateDatabricksIndex:
    ...
    async def test_create_unified_memory_success(self):
        ...
        with patch.object(svc, "_get_index_repository", return_value=mock_repo):
            with patch("src.services.databricks.index.DatabricksIndexSchemas") as schemas:
...
class TestGetIndexInfo:
    ...
    async def test_returns_index_info_on_success(self):
        ...
        with patch.object(svc, "_get_index_repository", return_value=mock_repo):
            with patch("src.services.databricks.index.IndexType") as it:
