...
class TestOneClickSetupHappyPath:
    ...
    async def test_full_success_no_group_id(self, service):
        ...
        with (
            patch(
                "src.services.databricks.vector_search.setup.DatabricksVectorEndpointRepository"
            ) as MockEpRepo,
            patch(
...
class TestIndexCreationRetry:
    ...
    async def test_unknown_index_type_returns_error(self, service):
        ...
        with (
            ...
            patch(
                "src.services.databricks.vector_search.setup.DatabricksIndexSchemas"
            ) as MockSchemas,
        ):
            mock_ep = AsyncMock()
            mock_ep.create_endpoint.return_value = ep_resp
...
class TestConfigSaving:
    ...
    async def test_config_saved_when_group_id_and_session(self):
        ...
        with (
            ...
            patch(
                "src.services.databricks.vector_search.setup.MemoryBackendBaseService"
            ) as MockBaseService,
        ):
            mock_ep = AsyncMock()
            mock_ep.create_endpoint.return_value = ep_resp
