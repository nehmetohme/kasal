...
class TestDatabricksVectorSearchVerificationService:
    ...
    def service(self):
        ...
        from src.services.databricks.vector_search.verification import DatabricksVectorSearchVerificationService
    ...
    async def test_verify_resources_success_case(self, service, mock_endpoint_repo, mock_index_repo):
        ...
        with patch('src.services.databricks.vector_search.verification.DatabricksVectorEndpointRepository', return_value=mock_endpoint_repo):
            with patch('src.services.databricks.vector_search.verification.DatabricksVectorIndexRepository', return_value=mock_index_repo):
                config = {
                    'databricks_config': {
                        'endpoint_name': 'test-endpoint',
