...
class TestDatabricksVectorSearchVerificationService:
    ...
    @pytest.mark.asyncio
    async def test_verify_resources_success_case(
        self, service, mock_endpoint_repo, mock_index_repo
    ):
        ...
        mock_endpoint_repo.get_endpoint = AsyncMock(
            return_value=EndpointResponse(
                success=True, endpoint=endpoint_info, message="Success"
            )
        )
        ...
        mock_index_repo.list_indexes = AsyncMock(
            return_value=IndexListResponse(
                success=True, indexes=[index_info], message="Success"
            )
        )

        with patch(
            "src.services.databricks.vector_search.verification.DatabricksVectorEndpointRepository",
            return_value=mock_endpoint_repo,
        ):
            with patch(
                "src.services.databricks.vector_search.verification.DatabricksVectorIndexRepository",
                return_value=mock_index_repo,
            ):
                config = {
                    "databricks_config": {
                        "endpoint_name": "test-endpoint",
                ...
                result = await service.verify_databricks_resources(
                    "https://test.databricks.com", "user-token", config
                )
    ...
    @pytest.mark.asyncio
    async def test_verify_resources_auth_failure(
        self, service, mock_endpoint_repo, mock_index_repo
    ):
    ...
    async def test_verify_resources_no_config(self, service):
        ...
        result = await service.verify_databricks_resources(
            "https://test.databricks.com", None, None  # No config
        )
    ...
    @pytest.mark.asyncio
    async def test_verify_resources_exception_handling(
        self, service, mock_endpoint_repo, mock_index_repo
    ):
    ...
    async def test_verify_resources_endpoint_not_found(
        ...
        mock_endpoint_repo.get_endpoint = AsyncMock(
            return_value=EndpointResponse(
                success=False, endpoint=endpoint_info, message="Not found"
            )
        )
        ...
        with patch(
            ...
            with patch(
                ...
                config = {"databricks_config": {"endpoint_name": "missing-endpoint"}}

                result = await service.verify_databricks_resources(
                    "https://test.databricks.com", None, config
                )
        ...
        assert (
            result["resources"]["endpoints"]["missing-endpoint"]["state"] == "NOT_FOUND"
        )

    @pytest.mark.asyncio
    async def test_verify_resources_with_multiple_resources(
        self, service, mock_endpoint_repo, mock_index_repo
    ):
        ...
        def get_endpoint_side_effect(endpoint_name, user_token=None):
            if endpoint_name == "endpoint1":
                return EndpointResponse(
                    success=True, endpoint=endpoint1_info, message="Success"
                )
            elif endpoint_name == "endpoint2":
                return EndpointResponse(
                    success=True, endpoint=endpoint2_info, message="Success"
                )
            return EndpointResponse(success=False, endpoint=None, message="Not found")

        mock_endpoint_repo.get_endpoint = AsyncMock(
            side_effect=get_endpoint_side_effect
        )
    ...
    @pytest.mark.asyncio
    async def test_verify_resources_with_env_token(
        self, service, mock_endpoint_repo, mock_index_repo
    ):
        ...
        with patch(
            ...
            with patch(
                ...
                config = {"databricks_config": {"endpoint_name": "test-endpoint"}}
    ...
    @pytest.mark.asyncio
    async def test_verify_resources_with_service_principal(
        self, service, mock_endpoint_repo, mock_index_repo
    ):
    ...
    @pytest.mark.asyncio
    async def test_verify_resources_with_database_token(
        self, service, mock_endpoint_repo, mock_index_repo
    ):
