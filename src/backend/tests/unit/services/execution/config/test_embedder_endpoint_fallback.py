            ...
            with patch("src.db.session.request_scoped_session") as mock_rss:
                ...
                with patch(
                    ...
                    with patch.object(
                        ...
                        with patch(
                            "src.services.execution.config.embedder_config_builder.DatabricksURLUtils"
                        ) as mock_utils:
                            mock_utils.normalize_workspace_url.return_value = (
                                "https://from-db.databricks.com"
                            )
                            with patch.dict(
                                os.environ,
                                {"DATABRICKS_HOST": "from-env.databricks.com"},
                            ):
                                result = await builder._get_databricks_endpoint()
