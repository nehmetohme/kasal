...
class TestManagerConfigBuilder:
    ...
    @pytest.mark.asyncio
    async def test_configure_manager_hierarchical_with_requested_model(
        self, manager_builder, config_with_group_id
    ):
        ...
        with patch(
            ...
            mock_configure.assert_called_once_with(
                "databricks-llama-4-maverick", "test_group_123"
            )

    @pytest.mark.asyncio
    async def test_configure_manager_hierarchical_without_requested_model(
        self, config_with_group_id
    ):
        ...
        with patch(
            "src.services.execution.config.manager_config_builder.LLMManager.configure_kasal_llm",
            return_value=mock_fallback_llm,
        ) as mock_configure:
    ...
    @pytest.mark.asyncio
    async def test_configure_manager_hierarchical_without_group_id(
        self, config_without_group_id
    ):
    ...
    @pytest.mark.asyncio
    async def test_configure_manager_hierarchical_llm_creation_fails_uses_fallback(
    ...
    @pytest.mark.asyncio
    async def test_configure_manager_with_manager_llm_string_in_config(
        self, manager_builder
    ):
    ...
    @pytest.mark.asyncio
    async def test_configure_manager_with_manager_llm_object_in_config(
        self, manager_builder
    ):
    ...
    @pytest.mark.asyncio
    async def test_configure_manager_with_manager_agent_in_config(
        self, manager_builder
    ):
    ...
    @pytest.mark.asyncio
    async def test_configure_manager_sequential_ignores_legacy_planning(
        self, config_with_group_id
    ):
        ...
        with patch(
            ...
            result = await manager_builder.configure_manager(
                crew_kwargs, Process.sequential
            )
    ...
    @pytest.mark.asyncio
    async def test_configure_manager_sequential_sets_no_manager_llm(
        self, manager_builder
    ):
    ...
    @pytest.mark.asyncio
    async def test_set_fallback_manager_llm_without_group_id(
        self, config_without_group_id
    ):
    ...
    @pytest.mark.asyncio
    async def test_configure_manager_with_databricks_prefix_retry(
        self, manager_builder
    ):
        """Test manager configuration retries with databricks/ prefix."""
        crew_kwargs = {}
        manager_builder.config["crew"][
            "manager_llm"
        ] = "databricks-llama-4-maverick"  # Without prefix
        mock_llm = MagicMock()
        ...
        with patch(
            ...
            mock_configure.side_effect = [Exception("Model not found"), mock_llm]
            ...
            mock_configure.assert_any_call(
                "databricks/databricks-llama-4-maverick", "test_group_123"
            )
