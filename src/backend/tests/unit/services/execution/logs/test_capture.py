...
class TestExecutionLogCapture:
    ...
    def test_setup_for_job(self, crew_logger_instance, mock_group_context):
        ...
        with patch(
            "src.services.execution.logs.capture.ExecutionLogHandler"
            ...
            mock_handler_class.assert_called_once_with(
                job_id=job_id, group_context=mock_group_context
            )
            crew_logger_instance._crew_logger.addHandler.assert_called_once_with(
                mock_handler
            )
    ...
    def test_cleanup_for_job(self, crew_logger_instance, mock_group_context):
        ...
        with patch(
            ...
            crew_logger_instance._crew_logger.removeHandler.assert_called_with(
                mock_handler
            )
...
def test_patch_printer_exception():
    ...
    with patch(
        "src.services.execution.logs.capture.Printer",
        side_effect=Exception("Printer unavailable"),
    ):
        # Should not raise
        try:
    ...
    with patch("src.services.execution.logs.capture.Printer") as MockPrinter:
        ...
        with patch("src.services.execution.logs.capture.enqueue_log"):
            ...
            if callable(custom_fn) and custom_fn is not original_print:
                ...
                custom_fn(
                    fake_self,
                    "created tenant context: something primary_tenant_id here",
                )  # Filter
