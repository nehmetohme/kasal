...
class TestTaskConfig:
    ...
    def mock_flow_data(self):
        ...
        flow_data = Mock()
        flow_data.edges = [{"source": "agent-1", "target": "task-1", "id": "edge1"}]
        flow_data.nodes = [{"id": "task-1", "data": {"tools": ["tool1", "tool2"]}}]
        return flow_data
        ...
        class MockTaskConfig:
            ...
            async def configure_task(
                ...
                try:
                    ...
                    if agent is None:
                        agent = await MockTaskConfig._resolve_agent_for_task(
                            task_data, flow_data, repositories
                        )
                        if not agent:
                            return None

                    # Mock tool configuration
                    await MockTaskConfig._configure_task_tools(
                        task_data, agent, flow_data
                    )
                    ...
                    expected_output = (
                        ...
                        if hasattr(task_data, "expected_output")
                        and task_data.expected_output is not None
                        else ""
                    )
                    ...
                    # Build through the engine binding, exactly as the real
                    # configure_task does — this stand-in only means anything
                    # while it mirrors the construction path under test.
                    from src.services.flow_builder.modules.task_adapter import (
                        active_harness,
                    )

                    mock_task = active_harness().build_task(
            ...
            async def _resolve_agent_for_task(task_data, flow_data, repositories):
                ...
                if flow_data and hasattr(flow_data, "edges"):
                    try:
                        ...
                        for edge in edges:
                            if edge.get("target") == task_node_id and edge.get(
                                "source", ""
                            ).startswith("agent-"):
                    ...
                    if isinstance(task_data.tools, list):
                        ...
                        try:
                            if isinstance(task_data.tools, str):
                                task_tools = [
                                    str(tool_id)
                                    for tool_id in json.loads(task_data.tools)
                                ]
                        except Exception:
                            pass
    ...
    @pytest.mark.asyncio
    async def test_configure_task_success(
        self, task_config_class, mock_task_data, mock_agent
    ):
        """Test successful task configuration."""
        with patch_build(
            "src.services.flow_builder.modules.task_adapter", "task"
        ) as mock_task_class:
            mock_task = Mock()
    ...
    @pytest.mark.asyncio
    async def test_configure_task_no_agent_provided(
        self, task_config_class, mock_task_data, mock_agent
    ):
        """Test task configuration without agent provided - should resolve agent."""
        with (
            patch.object(
                task_config_class, "_resolve_agent_for_task", new_callable=AsyncMock
            ) as mock_resolve,
            patch_build(
                "src.services.flow_builder.modules.task_adapter", "task"
            ) as mock_task_class,
    ...
    @pytest.mark.asyncio
    async def test_configure_task_no_agent_resolved(
        self, task_config_class, mock_task_data
    ):
    ...
    @pytest.mark.asyncio
    async def test_configure_task_with_markdown(
        self, task_config_class, mock_task_data_markdown, mock_agent
    ):
        ...
        with patch_build(
            ...
            result = await task_config_class.configure_task(
                mock_task_data_markdown, mock_agent
            )
    ...
    @pytest.mark.asyncio
    async def test_configure_task_with_advanced_properties(
        self, task_config_class, mock_task_data_markdown, mock_agent
    ):
    ...
    @pytest.mark.asyncio
    async def test_configure_task_with_callback(
        self, task_config_class, mock_task_data, mock_agent, mock_task_output_callback
    ):
    ...
    @pytest.mark.asyncio
    async def test_configure_task_with_callback_and_process_output_handler(
        self, task_config_class, mock_task_data, mock_agent, mock_task_output_callback
    ):
    ...
    @pytest.mark.asyncio
    async def test_configure_task_with_callback_no_process_attribute(
        self, task_config_class, mock_task_data, mock_agent, mock_task_output_callback
    ):
    ...
    @pytest.mark.asyncio
    async def test_configure_task_no_expected_output(
        self, task_config_class, mock_agent
    ):
    ...
    @pytest.mark.asyncio
    async def test_configure_task_exception_handling(
        self, task_config_class, mock_agent
    ):
        ...
        type(mock_task_data).description = property(
    ...
    @pytest.mark.asyncio
    async def test_configure_task_task_creation_exception(
        self, task_config_class, mock_agent
    ):
    ...
    @pytest.mark.asyncio
    async def test_resolve_agent_for_task_with_agent_id(
        self, task_config_class, mock_task_data, mock_repositories, mock_agent_data
    ):
    ...
    @pytest.mark.asyncio
    async def test_resolve_agent_for_task_no_repositories(
        self, task_config_class, mock_task_data, mock_agent_data
    ):
        """Test resolving agent without repositories."""
        result = await task_config_class._resolve_agent_for_task(
            mock_task_data, None, None
        )
    ...
    @pytest.mark.asyncio
    async def test_resolve_agent_for_task_no_agent_found(
        self, task_config_class, mock_task_data, mock_repositories
    ):
    ...
    @pytest.mark.asyncio
    async def test_resolve_agent_for_task_from_edges(
        self, task_config_class, mock_flow_data, mock_repositories, mock_agent_data
    ):
    ...
    @pytest.mark.asyncio
    async def test_resolve_agent_for_task_edges_string_json(
        self, task_config_class, mock_repositories, mock_agent_data
    ):
        ...
        mock_flow_data.edges = json.dumps(
            [{"source": "agent-1", "target": "task-1", "id": "edge1"}]
        )
    ...
    @pytest.mark.asyncio
    async def test_resolve_agent_for_task_edges_exception(
        self, task_config_class, mock_repositories
    ):
    ...
    @pytest.mark.asyncio
    async def test_resolve_agent_for_task_no_agent_id_no_edges(
        self, task_config_class, mock_repositories
    ):
    ...
    @pytest.mark.asyncio
    async def test_resolve_agent_task_has_no_agent_id_attribute(
        self, task_config_class, mock_repositories
    ):
    ...
    @pytest.mark.asyncio
    async def test_resolve_agent_flow_data_missing_edges_attribute(
        self, task_config_class, mock_repositories
    ):
    ...
    @pytest.mark.asyncio
    async def test_configure_task_tools_with_tools_list(
        self, task_config_class, mock_task_data_with_tools, mock_agent
    ):
        """Test configuring task tools when task has tools as list."""
        await task_config_class._configure_task_tools(
            mock_task_data_with_tools, mock_agent, None
        )

        assert len(mock_agent.tools) == 2  # Two tools

    @pytest.mark.asyncio
    async def test_configure_task_tools_with_tools_json_string(
        self, task_config_class, mock_agent
    ):
    ...
    @pytest.mark.asyncio
    async def test_configure_task_tools_invalid_json(
        self, task_config_class, mock_agent
    ):
    ...
    @pytest.mark.asyncio
    async def test_configure_task_tools_from_flow_nodes(
        self, task_config_class, mock_agent, mock_flow_data
    ):
        ...
        await task_config_class._configure_task_tools(
            mock_task_data, mock_agent, mock_flow_data
        )

        assert len(mock_agent.tools) == 2  # Two tools from node data

    @pytest.mark.asyncio
    async def test_configure_task_tools_from_flow_nodes_string_json(
        self, task_config_class, mock_agent
    ):
        ...
        mock_flow_data.nodes = json.dumps(
            [{"id": "task-1", "data": {"tools": ["tool1", "tool2"]}}]
        )
    ...
    @pytest.mark.asyncio
    async def test_configure_task_tools_no_tools(
        self, task_config_class, mock_task_data, mock_agent
    ):
    ...
    @pytest.mark.asyncio
    async def test_configure_task_tools_empty_tools_list(
        self, task_config_class, mock_agent
    ):
    ...
    @pytest.mark.asyncio
    async def test_configure_task_tools_with_task_missing_tools_attribute(
        self, task_config_class, mock_agent
    ):
    ...
    @pytest.mark.asyncio
    async def test_configure_task_tools_flow_data_missing_nodes_attribute(
        self, task_config_class, mock_agent
    ):
    ...
    @pytest.mark.asyncio
    async def test_configure_task_tools_non_list_non_string_tools(
        self, task_config_class, mock_agent
    ):
    ...
    @pytest.mark.asyncio
    async def test_configure_task_with_tools_integration(
        self, task_config_class, mock_task_data_with_tools, mock_agent
    ):
        """Test full task configuration integration with tools."""
        with (
            patch.object(
                task_config_class, "_configure_task_tools", new_callable=AsyncMock
            ) as mock_configure_tools,
            patch_build(
                "src.services.flow_builder.modules.task_adapter", "task"
            ) as mock_task_class,
        ):
            ...
            result = await task_config_class.configure_task(
                mock_task_data_with_tools, mock_agent
            )
...
class TestTaskConfigGuardrails:
    ...
    @pytest.mark.asyncio
    async def test_configure_task_with_code_guardrail(
        self, mock_task_data_with_code_guardrail, mock_agent
    ):
    ...
    @pytest.mark.asyncio
    async def test_configure_task_with_llm_guardrail(
        self, mock_task_data_with_llm_guardrail, mock_agent
    ):
    ...
    @pytest.mark.asyncio
    async def test_configure_task_llm_guardrail_takes_priority(
        self, mock_task_data_with_both_guardrails, mock_agent
    ):
    ...
    @pytest.mark.asyncio
    async def test_configure_task_guardrail_creation_fails(
        self, mock_task_data_with_code_guardrail, mock_agent
    ):
    ...
    @pytest.mark.asyncio
    async def test_configure_task_guardrail_setup_exception(
        self, mock_task_data_with_code_guardrail, mock_agent
    ):
    ...
    @pytest.mark.asyncio
    async def test_configure_task_llm_guardrail_augments_description(
        self, mock_task_data_with_llm_guardrail, mock_agent
    ):
    ...
    async def test_configure_task_no_guardrail(self, mock_agent):
        ...
        with patch_build(
            ...
            result = await TaskConfig.configure_task(mock_task_data, agent=mock_agent)
    ...
    async def test_configure_task_llm_guardrail_column_only_not_applied(
        ...
        with patch_build(
            ...
            result = await TaskConfig.configure_task(
                mock_task_data_with_llm_guardrail_column_only, agent=mock_agent
            )
