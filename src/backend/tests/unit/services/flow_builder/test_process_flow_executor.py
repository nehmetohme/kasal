...
def _run(flow_result, flow_config=None, group_context=None):
    ...
    with patch(
        ...
        with patch(
            "src.services.execution.subprocess_bootstrap.restore_stdout_stderr",
            p["restore"],
        ):
            with patch(
                        ...
                        with patch("asyncio.set_event_loop"):
                            with patch("asyncio.all_tasks", return_value=set()):
                                with patch.dict(
                                    ...
                                    with patch(
                                        "src.services.execution.logs.writer_task.LogWriterTask.stop_writer",
...
class TestRunFlowIsolated:
    def _mk(self):
        ...
        with patch(
            "src.services.flow_builder.process_executor.mp.get_context",
            return_value=ctx,
        ):
            e = ProcessFlowExecutor()
        e._ctx = ctx
    ...
    async def test_completed(self):
        ...
        assert (
            r["status"] == "COMPLETED" and e.get_metrics()["completed_executions"] == 1
        )
    ...
    async def test_group_ctx(self):
        ...
        assert (
            fc["group_id"] == "g1"
            and fc["user_token"] == "t1"
            and fc["execution_id"] == "e1"
        )
    ...
    async def test_no_match(self):
        ...
        with patch.dict(os.environ, {"LOG_DIR": "/tmp/l"}):
            with patch("os.path.exists", return_value=True):
                with patch("os.path.join", return_value="/tmp/l/flow.log"):
                    with patch(
                        "builtins.open", MagicMock(return_value=StringIO("other\n"))
                    ):
                        with patch(
                            "src.db.database_router.get_smart_db_session",
                            _fake_smart_session,
...
class TestWriteLogsPostgresAsync:
    ...
    def _e(self, exc=None):
        ...
        cm.__aexit__ = (
            AsyncMock(side_effect=exc) if exc else AsyncMock(return_value=False)
        )
        me = MagicMock()
...
class TestRunFlowInProcess:
                        ...
                        with patch("asyncio.new_event_loop", return_value=ml):
                            with patch("asyncio.set_event_loop"):
                                with patch(
                                    ...
                                    assert (
                                        run_flow_in_process("e1", '{"k":"v"}')["status"]
                                        == "FAILED"
                                    )
                ...
                with patch(
                    ...
                    with patch("signal.signal"):
                        with patch(
                            ...
                            assert (
                                "init" in run_flow_in_process("e1", {"k": "v"})["error"]
                            )
                        ...
                        with patch("asyncio.new_event_loop", return_value=ml):
                            with patch("asyncio.set_event_loop"):
                                with patch("asyncio.all_tasks", return_value={t}):
                                    with patch(
                                        "asyncio.gather", return_value=MagicMock()
                                    ):
                                        with patch.dict(
                                            "sys.modules",
                                            {
    ...
    def test_group_ids_fallback(self):
        ...
        assert (
            _run({"status": "COMPLETED", "result": "ok"}, group_context=GC())["status"]
            == "COMPLETED"
        )
...
class TestRunFlowInProcessDeep:
    def _deep_run(
        ...
        if trace_init_exc:
            mock_tm.ensure_writer_started = AsyncMock(
                side_effect=RuntimeError("trace_init")
            )
        all_patches = []
        all_patches.append(
            patch(
        ...
        all_patches.append(
            patch(
                "src.services.flow_builder.flow_runner_service.FlowRunnerService",
                return_value=mock_frs,
            )
        )
        all_patches.append(
            patch(
        ...
        all_patches.append(
            patch(
                "src.services.otel_tracing.mlflow_setup.execute_with_mlflow_trace_async",
                mock_exec_mlflow,
            )
        )
        all_patches.append(
        ...
        all_patches.append(
            patch(
                "src.services.mlflow.tracing.cleanup_async_db_connections", MagicMock()
            )
        )
        all_patches.append(
            patch(
        ...
        all_patches.append(
            patch("src.services.otel_tracing.shutdown_provider", MagicMock())
        )
        if bridge_exc:
            all_patches.append(
                patch(
                    "src.services.otel_tracing.event_bridge.OTelEventBridge",
                    side_effect=RuntimeError("bridge_fail"),
                )
            )
                            ...
                            with patch("asyncio.set_event_loop"):
                                with patch("asyncio.all_tasks", return_value=set()):
                                    with patch.dict(
                                        ...
                                        {
                                            "src.core.events": MagicMock(
                                                event_bus=MagicMock(flush=flush_mock)
                                            )
                                        },
                                    ):
                                        with patch(
...
class TestCleanupBranches:
    ...
    def test_async_cleanup_exception(self):
        ...
        p = _std()
        ml = MagicMock()
        ml.run_until_complete.side_effect = [
            flow_result,
            Exception("cleanup err"),
            None,
        ]
        with patch(
            "src.services.execution.subprocess_bootstrap.suppress_stdout_stderr",
...
class TestDeepAsyncBranches:
    ...
    def _deep_run_ext(
        self,
        gc=None,
        otel_import_error=False,
        otel_general_error=False,
        mlflow_ready=False,
        mlflow_error=False,
        flow_exc=False,
        event_bus_timeout=False,
        event_bus_error=False,
        litellm_import_error=False,
        litellm_fallback_error=False,
        async_cleanup_error=False,
        stdout_capture_error=False,
        ...
        if stdout_capture_error:
            ...
            p["suppress"] = MagicMock(
                return_value=(sys.stdout, sys.stderr, bad_capture)
            )
        mock_session = AsyncMock()
        mock_session_cm = AsyncMock()
        mock_session_cm.__aenter__ = AsyncMock(return_value=mock_session)
        ...
        mock_mlflow_result.error = (
            "mlflow warning" if (mlflow_ready and mlflow_error) else None
        )
        ...
        if not otel_import_error:
            if otel_general_error:
                all_patches.append(
                    patch(
                        "src.services.otel_tracing.create_kasal_tracer_provider",
                        side_effect=RuntimeError("otel fail"),
                    )
                )
            else:
                all_patches.append(
        ...
        if litellm_import_error:
            all_patches.append(
                patch(
                    "litellm.llms.custom_httpx.async_client_cleanup.close_litellm_async_clients",
    ...
    def test_litellm_fallback_error(self):
        ...
        r = self._deep_run_ext(
            gc=_gc(), litellm_import_error=True, litellm_fallback_error=True
        )
        assert r["status"] == "COMPLETED"
    ...
    def test_mlflow_warning_no_tracing(self):
        ...
        all_patches.append(patch("signal.signal"))
        all_patches.append(
            patch.dict("sys.modules", {"src.core.events": crewai_events_mod})
        )
        all_patches.append(
            patch("src.services.execution.logs.writer_task.LogWriterTask", mock_tm)
        )
        ...
        try:
            r = run_flow_in_process(
                "deep_exec", {"k": "v", "user_token": "tok"}, group_context=_gc()
            )
        finally:
            for p_ctx in reversed(ctx_stack):
                p_ctx.__exit__(None, None, None)
    ...
    def test_mlflow_otel_exporter_exception(self):
        ...
        mock_otel_provider.add_span_processor = MagicMock(
            side_effect=[None, None, RuntimeError("exporter fail")]
        )
        crewai_events_mod = MagicMock()
        crewai_events_mod.event_bus = mock_event_bus
        all_patches = []
...
class TestAsyncCleanupException:
    ...
    def test_all_tasks_raises(self):
        ...
        all_patches.append(
            patch("asyncio.all_tasks", side_effect=RuntimeError("all_tasks fail"))
        )
        ctx_stack = []
        for p_ctx in all_patches:
            p_ctx.__enter__()
