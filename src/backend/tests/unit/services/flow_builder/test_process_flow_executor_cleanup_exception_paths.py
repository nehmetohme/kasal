...
def _deep_run_extended(
    ...
    otel_import_err=False,
    otel_general_err=False,
    crewai_instrumentor_import_err=False,
    db_config_value=None,
    mlflow_tracing_ready=False,
    mlflow_error=None,
    mlflow_init_exc=False,
    mlflow_otel_exc=False,
    flow_config_override=None,
    event_bus_flush_return=True,
    event_bus_flush_exc=False,
    error_path_mlflow_cleanup_exc=False,
    db_cleanup_exc=False,
    psutil_children_terminate_nsp=False,
    psutil_children_kill_nsp=False,
    ...
    sys_modules_patches = {"src.core.events": crewai_events_mod}
    ...
    all_patches.append(
        patch(
            "src.services.databricks.workspace.service.DatabricksService", mock_ds_cls
        )
    )
    all_patches.append(
        patch(
    ...
    if otel_import_err:
        ...
        all_patches.append(
            patch(
                "src.services.otel_tracing.create_kasal_tracer_provider",
                side_effect=RuntimeError("otel_broken"),
            )
        )
    else:
        all_patches.append(
    ...
    if crewai_instrumentor_import_err:
        all_patches.append(
            patch.dict("sys.modules", {"openinference.instrumentation.crewai": None})
        )
    if mlflow_otel_exc:
        all_patches.append(
            patch(
                "src.services.otel_tracing.mlflow_exporter.KasalMLflowSpanExporter",
                side_effect=RuntimeError("exporter_fail"),
            )
        )
    if db_cleanup_exc:
        all_patches.append(
            patch(
                ...
                MagicMock(side_effect=RuntimeError("db_cleanup_fail")),
            )
        )
    else:
        all_patches.append(
...
class TestMLflowBranchesCov:
    def test_mlflow_tracing_ready(self):
        r = _deep_run_extended(
            gc=_gc(), db_config_value={"host": "t"}, mlflow_tracing_ready=True
        )
        assert r["status"] == "COMPLETED"
    ...
    def test_mlflow_init_exception(self):
        r = _deep_run_extended(
            gc=_gc(), db_config_value={"host": "t"}, mlflow_init_exc=True
        )
        assert r["status"] == "COMPLETED"
...
class TestErrorPathFlushAndCleanupCov:
    ...
    def test_error_path_mlflow_cleanup_exception(self):
        r = _deep_run_extended(
            gc=_gc(), flow_exc=True, error_path_mlflow_cleanup_exc=True
        )
        assert r["status"] == "FAILED"
    ...
    def test_wrapper_finally_log_fails(self):
        ...
        with patch(
            ...
            with patch("logging.shutdown"):
                with patch(
                    "logging.getLogger", side_effect=RuntimeError("logger_dead")
                ):
                    with patch("os._exit"):
