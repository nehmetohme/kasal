...
class TestBaseGuardrail:
    ...
    def test_cannot_instantiate_directly(self):
        ...
        with pytest.raises(
            TypeError, match="Can't instantiate abstract class BaseGuardrail"
        ):
            BaseGuardrail({})
    ...
    def test_concrete_implementation_required(self):
        ...
        with pytest.raises(
            TypeError, match="Can't instantiate abstract class IncompleteGuardrail"
        ):
            IncompleteGuardrail({})
    ...
    def test_init_with_complex_config(self):
        ...
        config = {
            "field": "companies",
            "min_count": 5,
            "nested": {"validation": True, "rules": ["rule1", "rule2"]},
            "callbacks": [1, 2, 3],
        }
        guardrail = TestGuardrail(config)
        ...
        class ConfigAccessingGuardrail(BaseGuardrail):
            def validate(self, output: str):
                ...
                if len(output) > threshold:
                    ...
                    return {
                        "valid": False,
                        "feedback": f"Output too short, minimum {threshold}",
                    }
