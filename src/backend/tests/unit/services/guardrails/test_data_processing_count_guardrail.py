...
class TestDataProcessingCountGuardrail:
    ...
    def test_initialization_with_dict_config_default_minimum_count(self, mock_logger):
        ...
        mock_logger.warning.assert_called_with(
            "No minimum_count found in config, defaulting to 0"
        )
        mock_logger.info.assert_called_with(
            "DataProcessingCountGuardrail initialized with minimum_count: 0"
        )
    ...
    def test_initialization_with_dict_config_custom_minimum_count(self, mock_logger):
        ...
        mock_logger.info.assert_called_with(
            "DataProcessingCountGuardrail initialized with minimum_count: 5"
        )
    ...
    def test_initialization_with_json_string_config(self, mock_logger):
        ...
        mock_logger.info.assert_called_with(
            "DataProcessingCountGuardrail initialized with minimum_count: 10"
        )
    ...
    def test_initialization_with_invalid_json_string(self, mock_logger):
        ...
        mock_logger.error.assert_called_with(
            f"Failed to parse guardrail config: {config}"
        )
        mock_logger.warning.assert_called_with(
            "No minimum_count found in config, defaulting to 0"
        )
    ...
    def test_initialization_with_string_minimum_count(self, mock_logger):
        ...
        mock_logger.info.assert_called_with(
            "DataProcessingCountGuardrail initialized with minimum_count: 15"
        )
    ...
    def test_validate_failure_with_insufficient_records(self, mock_logger):
        ...
        assert (
            "Insufficient records: The number of records in the data_processing table (3) is below the minimum count required (5)"
            in result["feedback"]
        )
        mock_logger.warning.assert_any_call(
            "Validation failed: Actual count (3) is below minimum count (5)"
        )
    ...
    def test_validate_with_table_creation(self, mock_logger, mock_uow, mock_repo_class):
        ...
        mock_repo.create_record_sync.assert_any_call(
            che_number="CHE12345", processed=False
        )
        mock_repo.create_record_sync.assert_any_call(
        ...
        mock_logger.info.assert_any_call(
            "Created table and test records via repository"
        )
    ...
    def test_validate_general_exception_handling(self, mock_logger, mock_repo_class):
        ...
        assert result["valid"] is False
        assert (
            "Error checking data processing count: General error" in result["feedback"]
        )
    ...
    def test_validate_equal_counts(self, mock_logger):
        ...
        assert result["valid"] is True
        assert (
            "Success: The number of records in the data_processing table (5) meets or exceeds the minimum count (5)"
            in result["feedback"]
        )
    ...
    def test_validate_with_zero_actual_count(self, mock_logger):
        ...
        assert result["valid"] is False
        assert (
            "Insufficient records: The number of records in the data_processing table (0) is below the minimum count required (1)"
            in result["feedback"]
        )
    ...
    def test_validate_with_negative_minimum_count_validation(self, mock_logger):
        ...
        assert result["valid"] is True
        assert (
            "Success: The number of records in the data_processing table (0) meets or exceeds the minimum count (-1)"
            in result["feedback"]
        )
    ...
    @patch("src.services.guardrails.demo.data_processing_count_guardrail.logger")
    @patch("src.services.guardrails.demo.data_processing_count_guardrail.traceback")
    def test_initialization_exception_traceback_capture(
    ...
    @patch("src.services.guardrails.demo.data_processing_count_guardrail.logger")
    @patch("src.services.guardrails.demo.data_processing_count_guardrail.traceback")
    def test_validate_exception_traceback_capture(
        self, mock_traceback, mock_logger, mock_repo_class
    ):
    ...
    def test_validate_logs_all_steps(self, mock_logger, mock_uow, mock_repo_class):
        ...
        mock_logger.info.assert_any_call(
            "Validation passed: Actual count (10) meets or exceeds minimum count (5)"
        )
    ...
    def test_validate_method_signature(self, mock_logger):
        ...
        with patch.object(
            guardrail, "validate", return_value={"valid": True, "feedback": ""}
        ) as mock_validate:
