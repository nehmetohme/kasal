...
class TestDatabaseManagementServiceInit:
    ...
    def test_init_creates_default_repository_when_none(self, mock_session):
        from src.services.lakebase.management import DatabaseManagementService
        with patch(
            "src.services.lakebase.management.DatabaseBackupRepository"
        ) as MockRepo:
...
class TestExportToVolume:
    ...
    async def test_export_sqlite_success(self, service, mock_repository):
        ...
        with patch(
            "src.services.lakebase.management.DatabaseBackupRepository.get_database_type",
            return_value="sqlite",
        ), patch(
            "src.services.lakebase.management.settings"
        ) as mock_settings, patch(
            "src.services.lakebase.management.os.path.isabs", return_value=True
        ), patch(
            "src.services.lakebase.management.os.path.exists", return_value=True
        ), patch(
            "src.services.lakebase.management.os.path.getsize",
    ...
    async def test_export_sqlite_db_not_found(self, service, mock_repository):
        ...
        with patch(
            ...
            "src.services.lakebase.management.os.path.exists", return_value=False
        ):
    ...
    async def test_export_sqlite_default_path_when_empty(self, service, mock_repository):
        ...
        with patch(
            ...
            "src.services.lakebase.management.os.path.isabs", return_value=False
        ), patch(
            "src.services.lakebase.management.os.path.abspath",
    ...
    async def test_export_sqlite_relative_path_converted(self, service, mock_repository):
        ...
        with patch(
            ...
            "src.services.lakebase.management.os.path.getsize", return_value=1024
        ):
...
class TestImportFromVolume:
    ...
    async def test_import_sqlite_success(self, service, mock_repository):
        ...
        with patch(
            ...
            with patch("src.services.lakebase.management.engine", mock_engine, create=True):
...
class TestCheckUserPermission:
    ...
    async def test_permission_exception_defaults_to_allow(self, service):
        ...
        with patch(
            "src.services.lakebase.management.logger"
        ) as mock_logger:
