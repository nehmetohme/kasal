...
async def test_test_connection_success():
    ...
    with patch.dict("os.environ", {}, clear=True):
        with patch.object(svc, "generate_credentials", new=AsyncMock(return_value=mock_cred)):
            with patch("src.services.lakebase.connection.create_async_engine") as mock_engine_cls:
                mock_conn = MagicMock()
...
def test_create_engine_with_token_refresh_asyncpg():
    ...
    with patch("src.services.lakebase.connection.create_async_engine") as mock_create:
        mock_engine = MagicMock()
        mock_engine.sync_engine = MagicMock()
        mock_create.return_value = mock_engine
        with patch("src.services.lakebase.connection.event") as mock_event:
...
def test_create_engine_with_token_refresh_pg8000():
    svc = make_service()
    token_holder = {"token": "initial-token", "refreshed_at": 0.0}
    with patch("src.services.lakebase.connection.create_engine") as mock_create:
        mock_engine = MagicMock()
        mock_create.return_value = mock_engine
        with patch("src.services.lakebase.connection.event") as mock_event:
            result = svc.create_engine_with_token_refresh(
...
async def test_get_workspace_client_with_spn():
    ...
    with patch.dict("os.environ", {
        ...
        with patch("src.services.lakebase.connection.WorkspaceClient") as mock_ws_cls:
            mock_ws = MagicMock()
...
async def test_get_workspace_client_local_dev_fallback():
    ...
    with patch.dict("os.environ", {}, clear=True):
        mock_ws = MagicMock()
        with patch("src.services.lakebase.connection.get_workspace_client", new=AsyncMock(return_value=mock_ws)):
            result = await svc.get_workspace_client()
    assert result is mock_ws
...
async def test_get_workspace_client_local_dev_raises_when_none():
    svc = make_service()
    with patch.dict("os.environ", {}, clear=True):
        with patch("src.services.lakebase.connection.get_workspace_client", new=AsyncMock(return_value=None)):
