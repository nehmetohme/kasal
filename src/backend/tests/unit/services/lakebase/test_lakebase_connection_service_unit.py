...
class TestGetWorkspaceClient:
    ...
    @pytest.mark.asyncio
    @patch("src.services.lakebase.connection.get_workspace_client")
    ...
    async def test_uses_spn_oauth_when_env_vars_set(self):
        ...
        with patch.dict("os.environ", env, clear=False), \
             patch("src.services.lakebase.connection.WorkspaceClient", return_value=mock_ws) as mock_cls:
...
class TestCreateLakebaseEngineAsync:
    ...
    @pytest.mark.asyncio
    @patch("src.services.lakebase.connection.create_async_engine")
...
class TestGetWorkspaceClientSpnPatBackup:
    ...
    async def test_spn_strips_and_restores_pat_env_vars(self):
        ...
        with patch.dict("os.environ", env, clear=True), \
             patch("src.services.lakebase.connection.WorkspaceClient", side_effect=_fake_ws):
            result = await svc.get_workspace_client()
...
class TestCreateEngineWithTokenRefresh:
    ...
    @pytest.mark.asyncio
    @patch("src.services.lakebase.connection.event")
    @patch("src.services.lakebase.connection.create_async_engine")
    ...
    @patch("src.services.lakebase.connection.event")
    @patch("src.services.lakebase.connection.create_engine")
