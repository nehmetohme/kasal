...
class TestMigrateExistingData:
    ...
    async def test_sqlite_source_success(self):
        ...
        with patch("src.services.lakebase.service.LAKEBASE_AVAILABLE", True), \
             patch("src.services.lakebase.service.settings") as mock_settings, \
             patch("src.services.lakebase.service.LakebaseMigrationService", return_value=mock_mig_svc), \
             patch("src.services.lakebase.service.AsyncSession", return_value=mock_lb_session):
...
class TestMigrateExistingDataStreamExtended:
    ...
    async def test_sqlite_source_creates_engine(self):
        ...
        with patch("src.services.lakebase.service.LAKEBASE_AVAILABLE", True), \
             patch("src.services.lakebase.service.settings") as mock_settings, \
             patch("src.services.lakebase.service.LakebaseMigrationService", return_value=mock_mig_svc), \
             patch("src.services.lakebase.service.create_engine", return_value=mock_source_engine):
    ...
    async def test_postgresql_source_creates_pg8000_engine(self):
        ...
        with patch("src.services.lakebase.service.LAKEBASE_AVAILABLE", True), \
             patch("src.services.lakebase.service.settings") as mock_settings, \
             patch("src.services.lakebase.service.LakebaseMigrationService", return_value=mock_mig_svc), \
             patch("src.services.lakebase.service.create_engine", side_effect=capture_create_engine):
            mock_settings.DATABASE_URI = "postgresql+asyncpg://user" ":pass@host/db"
            mock_settings.DATABASE_TYPE = "postgresql"
...
class TestCheckLakebaseTablesCountError:
    ...
    async def test_table_count_error_is_recorded(self):
        ...
        with patch("src.services.lakebase.service.LAKEBASE_AVAILABLE", True), \
             patch("src.services.lakebase.service._validate_identifier", side_effect=lambda n, *a: n):
