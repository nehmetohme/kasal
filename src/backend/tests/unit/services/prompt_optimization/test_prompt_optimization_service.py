...
class _FakeJudgeStore:
    ...
    def __init__(self):
        ...
        class Registry:
            ...
            def prompt_name(self, full_name):
                base = f"kasal_judge__{full_name}"
                return f"{self.uc_schema}.{base}" if self.uc_schema else base
...
class TestJudgeLifecycle:
    ...
    @pytest.mark.asyncio
    async def test_list_links_each_judge_to_its_mlflow_page(self, fake_mlflow):
        svc = _judge_service()
        await svc.create_judge("lib", "Library judge for {{ outputs }}.")
