...
class TestHelpers:
    ...
    def test_parse_response_valid_json(self):
        resp = (
            '{"success": true, "sql_expr": "SUM(source.amount)", "confidence": "high"}'
        )
    ...
    def test_validate_sql_clean(self):
        assert (
            _validate_sql("SUM(source.amount) / NULLIF(SUM(source.count), 0)") is True
        )
...
class TestTranslateWithLLM:
    ...
    async def test_llm_failure_returns_unchanged(self):
        ...
        with patch(
            "src.services.tools.metric_view_utils.dax_llm_fallback._call_llm",
            new_callable=AsyncMock,
            return_value={"content": None, "error": "unavailable"},
        ):
...
class TestBatchConcurrency:
    ...
    def test_all_translated_and_runs_concurrently(self):
        ...
        async def go():
            with patch.object(d, "_call_llm", new=fake_call):
                t = time.monotonic()
                out = await d.translate_batch_with_llm(
                    measures, "tbl", set(), {}, model="m"
                )
    ...
    def test_artifacts_skipped(self):
        ...
        async def go():
            with patch.object(d, "_call_llm", new=fake_call):
                return await d.translate_batch_with_llm(
                    [m], "tbl", set(), {}, model="m"
                )
    ...
    def test_dax_class_captured_on_success(self):
        ...
        async def go():
            with patch.object(
                d,
                "_call_llm",
                new=AsyncMock(return_value={"content": fake, "usage": {}}),
            ):
...
class TestTableContext:
    ...
    def test_same_dax_different_context_caches_separately(self):
        ...
        async def fake_call(prompt, sysp, model):
            calls["n"] += 1
            return {
                "content": json.dumps({"success": True, "sql_expr": "SUM(source.c)"}),
                "usage": {},
            }
        ...
        async def go():
            cache = OrderedDict()
            with patch.object(d, "_call_llm", new=fake_call):
                await d.translate_with_llm(
                    mk(), "t", set(), {}, cache=cache, table_context="ctx-A"
                )
                await d.translate_with_llm(
                    mk(), "t", set(), {}, cache=cache, table_context="ctx-B"
                )
