...
class TestQuickReject:
    ...
    def test_color_rejected(self, translator):
        result = translator.translate(
            {
                "measure_name": "KBI_Color",
                "dax_expression": 'IF(x>0, "green", "red")',
                "original_name": "KBI_Color",
            },
            "fact_test",
        )
        assert result.is_translatable is False
    ...
    def test_selectedvalue_switch(self, translator):
        result = translator.translate(
            {
                "measure_name": "sw",
                "dax_expression": 'SWITCH(SELECTEDVALUE(Table[Col]), "a", 1, 2)',
                "original_name": "sw",
            },
            "fact_test",
        )
...
class TestSimpleSum:
    ...
    def test_calculate_sum(self, translator):
        result = translator.translate(
            {
                "measure_name": "total",
                "dax_expression": "CALCULATE(SUM(Sales[Amount]))",
                "original_name": "Total",
            },
            "fact_test",
        )
...
class TestSimpleSumx:
    def test_sumx_without_filter(self, translator):
        result = translator.translate(
            {
                "measure_name": "total",
                "dax_expression": "SUMX(Sales, Sales[Amount])",
                "original_name": "Total",
            },
            "fact_test",
        )
...
class TestCountxFilter:
    def test_countx_filter(self, translator):
        result = translator.translate(
            {
                "measure_name": "cnt",
                "dax_expression": 'COUNTX(FILTER(Sales, Sales[Status]="Active"), Sales[ID])',
                "original_name": "Count",
            },
            "fact_test",
        )
...
class TestAveragexFilter:
    def test_averagex_filter(self, translator):
        result = translator.translate(
            {
                "measure_name": "avg",
                "dax_expression": 'AVERAGEX(FILTER(Sales, Sales[Type]="A"), Sales[Value])',
                "original_name": "Avg",
            },
            "fact_test",
        )
...
class TestDistinctCountNoBlank:
    def test_simple(self, translator):
        result = translator.translate(
            {
                "measure_name": "dc",
                "dax_expression": "DISTINCTCOUNTNOBLANK(Sales[Customer])",
                "original_name": "DC",
            },
            "fact_test",
        )
...
class TestDivideThirdArg:
    ...
    def test_two_arg_divide(self):
        ...
        assert DaxTranslator._extract_divide_args("DIVIDE(SUM(a), SUM(b))") == (
            "SUM(a)",
            "SUM(b)",
        )
    ...
    def test_three_arg_divide_drops_alt(self):
        ...
        assert DaxTranslator._extract_divide_args("DIVIDE(SUM(a), SUM(b), 0)") == (
            "SUM(a)",
            "SUM(b)",
        )
    ...
    def test_three_arg_blank_alt(self):
        ...
        assert DaxTranslator._extract_divide_args("DIVIDE([x], [y], BLANK())") == (
            "[x]",
            "[y]",
        )
    ...
    def test_nested_parens_preserved(self):
        ...
        num, den = DaxTranslator._extract_divide_args(
            "DIVIDE(SUM(a)+SUM(c), NULLIF(SUM(b),0), 0)"
        )
...
class TestMeasureRefRatioFilters:
    ...
    def test_var_chain_ratio_keeps_distinct_filters(self):
        t = self._t()
        dax = (
            'var num = CALCULATE([Base_KBI], (FT_QSE[bic_csubkbi] in {"A1","A2"})) '
    ...
    def test_inline_divide_ratio_keeps_distinct_filters(self):
        t = self._t()
        dax = (
            'DIVIDE(CALCULATE([Base_KBI], FT_QSE[bic_csubkbi] in {"A1","A2"}),'
    ...
    def test_parenthesized_predicate_parsed(self):
        ...
        t = self._t()
        parsed = t._parse_calculate_filters(
            '(FT_QSE[bic_csubkbi] in {"A1","A2"})', "FT_QSE"
        )
...
class TestFragileMatchersRouteToLLM:
    ...
    def test_trivial_only_skips_multivar_divide(self):
        ...
        res = t.translate(
            {"measure_name": "y", "dax_expression": dax, "original_name": "Y"},
            "f",
            trivial_only=True,
        )
...
class TestSameMeasureRatioKeepsPerSideFilters:
    ...
    def _sql(self, t):
        dax = (
            'DIVIDE(CALCULATE([Base], T[csubkbi] in {"A"}), '
            'CALCULATE([Base], T[csubkbi] in {"B"}))'
        )
        return t.translate(
            {"measure_name": "r", "original_name": "R", "dax_expression": dax}, "T"
        ).sql_expr
...
class TestDateWindowScaffoldingStrip:
    ...
    def test_regex_translates_cleaned_ratio(self):
        ...
        r = t.translate(
            {"measure_name": "y", "original_name": "Y", "dax_expression": dax},
            "f",
            trivial_only=False,
        )
