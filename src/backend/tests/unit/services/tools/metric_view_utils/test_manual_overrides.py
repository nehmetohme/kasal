...
class TestManualOverrides:
        ...
        config = {
            "manual_overrides": {
                "fact": [
                    {
                        "name": "custom_metric",
                        "expr": "SUM(source.val) / NULLIF(COUNT(*), 0)",
                        "comment": "Custom",
                    },
                    {
                        "name": "with_window",
    ...
    def test_manual_override_removes_from_untranslatable(self):
        ...
        measures = [
            {
                "measure_name": "Hard_One",
                "dax_expression": "CALCULATE(SUM(fact[val]), USERELATIONSHIP(x, y))",
                "original_name": "Hard_One",
                "proposed_allocation": "fact",
            },
        ]
        mquery = [
        ...
        config = {
            "manual_overrides": {
                "fact": [
                    {
                        "name": "hard_one",
                        "expr": "SUM(source.val) * 42",
                        "comment": "Manually handled",
                    },
                ]
            }
        }
