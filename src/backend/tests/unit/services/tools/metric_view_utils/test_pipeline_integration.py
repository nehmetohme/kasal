...
class TestPipelineIntegration:
    ...
    def test_measure_arithmetic_cascade(self):
        ...
        measures = [
            {
                "measure_name": "Sales",
                "dax_expression": "SUM(fact[sales])",
                "original_name": "Sales",
                "proposed_allocation": "fact",
            },
            {
                "measure_name": "Cost",
                "dax_expression": "SUM(fact[cost])",
                "original_name": "Cost",
                "proposed_allocation": "fact",
            },
            {
                "measure_name": "Profit",
                "dax_expression": "[Sales] - [Cost]",
    ...
    def test_artifact_cascade(self):
        ...
        measures = [
            {
                "measure_name": "Color",
                "dax_expression": 'IF(x>0, "green", "red")',
        ...
        for m in spec.untranslatable:
            assert (
                "Color" in m.skip_reason
                or "FORMAT" in m.skip_reason
                or "artifact" in m.skip_reason.lower()
                or "IF(" in m.dax_expression
                or "FORMAT(" in m.dax_expression
            )
    ...
    def test_switch_decomposition(self):
        ...
        measures = [
            {
                "measure_name": "Wrapper",
                "dax_expression": 'SWITCH(SELECTEDVALUE(T[x]), "a", 1)',
        ...
        config = {
            "switch_decompositions": {
                "fact": [
                    {
                        "name": "branch_a",
                        "raw_expr": "SUM(source.val) FILTER (WHERE source.col = 'A')",
                        "comment": "Branch A",
                    },
                ]
            }
        ...
        measures = [
            {
                ...
                "proposed_allocation": "sales",
            },
            {
                "measure_name": "Units",
                "dax_expression": "SUM(inventory[qty])",
                "original_name": "Units",
                "proposed_allocation": "inventory",
            },
        ]
        mquery = [
            {
...
_EXAMPLE_DIR = os.path.normpath(
    os.path.join(
        ...
        "..",
        "examples",
        "uc_metric_view_migration",
    )
)

_HAS_EXAMPLE_DATA = os.path.exists(os.path.join(_EXAMPLE_DIR, "pipeline_config.json"))
...
class TestFullPipelineEndToEnd:
    ...
    def test_yaml_parseable(self, pipeline_output):
        skipped_tables = {
            k for k, s in pipeline_output["stats"].items() if s.get("skipped")
        }
    ...
    def test_total_translated_regression_guard(self, pipeline_output):
        ...
        assert total >= 390, f"Regression: only {total} translated (expected >= 390)"
    ...
    def test_manual_overrides_injected(self, pipeline_output):
        ...
        manual = [
            m for m in spec.measures if getattr(m, "category", "") == "manual_override"
        ]
        assert len(manual) >= 5, f"Expected >= 5 manual overrides, got {len(manual)}"
    ...
    def test_window_specs_on_py_measures(self, pipeline_output):
        ...
        py_measures = [m for m in spec.measures if m.measure_name.endswith("_py")]
    ...
    def test_yaml_no_empty_dimension_names(self, pipeline_output):
        ...
        for key, yml in pipeline_output["yaml_out"].items():
            ...
            for i, line in enumerate(lines):
                if line.strip() == "- name:":
                    pytest.fail(f"{key} has empty dimension name at line {i + 1}")
...
class TestMeasureDrivenFacts:
    ...
    def _pipe(self, measures, mquery, config=None):
        ...
        return MetricViewPipeline(
            mapping=measures, mquery_tables=tables, config=config or {}
        )
        ...
        measures = [
            {
                "measure_name": "Total",
                "original_name": "Total",
                "dax_expression": "SUM(DCC_Sales[Amount])",
        ...
        mquery = [
            {
                ...
                "transpiled_sql": "SELECT * FROM lakehouse.dcc_sales",
                "validation_passed": "Yes",
            }
        ]
        ...
        mquery = [
            {
                ...
                "transpiled_sql": "SELECT * FROM lakehouse.dim_country",
                "validation_passed": "Yes",
            }
        ]
    ...
    def test_kill_switch_disables_promotion(self):
        ...
        pipe = self._pipe(
            measures, mquery, config={"allow_measure_driven_facts": False}
        )
        pipe.run()
        y = pipe.emit_all_yaml(catalog="main", schema="m")
        ...
        measures = [
            {
                "measure_name": "M",
                "original_name": "M",
                "dax_expression": "SUM(FT[amt])",
                "proposed_allocation": "FT",
            }
        ]
        mquery = [
            {
    ...
    def test_no_source_table_not_promoted(self):
        ...
        mquery = [
            {
                "table_name": "T",
                "transpiled_sql": "let Source = Foo in Source",
                "validation_passed": "Yes",
            }
        ]
        pipe = self._pipe(measures, mquery)
    ...
    def _spec(self):
        ...
        return SimpleNamespace(
            ...
            joins=[
                ...
                {
                    "name": "dim_real",
                    "source": "dc_prod.idor.ca_dim_plant",
