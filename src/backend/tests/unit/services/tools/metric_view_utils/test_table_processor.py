    ...
    *,
    ...
    return TableProcessorContext(
        ...
        cross_table_measures=(
            cross_table_measures if cross_table_measures is not None else []
        ),
...
def _extract_divide_args_fn(expr):
    ...
    return (
        m.start(),
        pos + 1,
        expr[start:comma_pos].strip(),
        expr[comma_pos + 1 : pos].strip(),
    )
...
class TestExpandCalculationGroups:
    ...
    def test_single_group_single_item(self):
        ...
        cg = [
            {
                "name": "YoY",
                "items": [{"name": "Growth", "expression": "SELECTEDMEASURE() * 1.1"}],
            }
        ]
        limitations = {}
    ...
    def test_multiple_base_measures(self):
        ...
        cg = [
            {
                "name": "Budget",
                "items": [{"name": "Plan", "expression": "SELECTEDMEASURE() * 0.9"}],
            }
        ]
        limitations = {}
        result = expand_calculation_groups(base, cg, limitations)
...
class TestProcessTableBasic:
    ...
    def test_with_calculated_columns(self):
        ti = _make_table_info(
            calculated_columns=[
                {"name": "fiscal_year", "expr": "DATE(fiscal_year, 1, 1)"}
            ],
    ...
    def test_enrichment_join_already_detected(self):
        ...
        jd = MagicMock()
        jd.detect.return_value = [
            {"name": "dim_calendar", "source": "x", "join_on": "a=b"}
        ]
        jd.detect_fact_joins.return_value = []
        jd.get_dim_dimensions.return_value = []
        ti = _make_table_info()
...
class TestProcessTableManualOverrides:
    ...
    def test_manual_override_skipped_when_already_translated(self):
        ...
        ctx = _make_context(
            config={
                "manual_overrides": {
                    "fact_test": [{"name": "amount", "expr": "SUM(source.new_amount)"}]
                }
            }
        )
        spec = _run_process_table("fact_test", ti, [], ctx)
...
class TestProcessTablePeriodDim:
    def test_period_dim_priority_override(self):
        ...
        ctx = _make_context(
            config={"period_dim_priority": ["fiscper", "fiscal_year_period"]}
        )
        dax = [
            {
                "measure_name": "py",
...
class TestProcessTableCascade:
    ...
    def test_format_wrapping_aggregate_now_translates(self):
        ...
        dax = [
            {
                "measure_name": "fmt_measure",
                "original_name": "Fmt Measure",
...
class TestProcessTableUnflattenJoins:
    def test_unflatten_3part_source(self):
        ...
        jd.detect.return_value = [
            {
                "name": "dim_prod",
                "source": "cat.sch.dim__products__v2",
                "join_on": "source.pk = dim_prod.pk",
            }
        ]
        jd.detect_fact_joins.return_value = []
        jd.get_dim_dimensions.return_value = []
...
class TestProcessTableUseRelationship:
    def test_userelationship_adds_alternate_join(self):
        ...
        dim_ti = TableInfo(
            table_name="Calendar",
            source_table="cat.sch.calendar",
            is_fact=False,
            aggregate_columns=[],
            group_by_columns=["date_key", "year"],
            calculated_columns=[],
            full_sql="",
            raw_transpiled_sql="",
            static_filters=[],
        ...
        dax = [
            {
                "measure_name": "ship_rev",
                "original_name": "Ship Rev",
...
class TestProcessTableCalcGroups:
    def test_calc_groups_expanded(self):
        ...
        calc_groups = [
            {
                "name": "TimeIntel",
                "items": [{"name": "PY", "expression": "SELECTEDMEASURE()"}],
            }
        ]
...
class TestProcessTableAutoAddJoins:
    def test_auto_adds_join_from_join_key_map(self):
        ...
        dim_ti = TableInfo(
            table_name="product",
            source_table="cat.sch.dim_product",
            is_fact=False,
            aggregate_columns=[],
            group_by_columns=["product_key"],
            calculated_columns=[],
            full_sql="",
            raw_transpiled_sql="",
            static_filters=[],
        ...
        ctx = _make_context(
            ...
            config={
                ...
                "switch_decompositions": {
                    "fact_test": [{"name": "prod_rev", "raw_expr": "SUM(dim_prod.rev)"}]
                },
            },
        )
        spec = _run_process_table("fact_test", ti, [], ctx)
...
class TestProcessTableScanData:
    def test_scan_data_builds_source_sql(self):
        ...
        scan_info.native_sql = (
            "SELECT region, SUM(amount) AS amount FROM raw.sales GROUP BY region"
        )
        scan_info.m_steps = []
...
class TestProcessTableLlmFallback:
    def test_llm_fallback_called_when_configured(self):
        ...
        mock_translated_result = TranslationResult(
            measure_name="llm_measure",
            original_name="LLM Measure",
            sql_expr="SUM(source.x)",
            is_translatable=True,
            skip_reason="",
            dax_expression="UNKNOWN()",
            confidence="medium",
        ...
        dax = [
            {
                "measure_name": "hard_measure",
                "original_name": "Hard Measure",
...
class TestProcessTableDimSourceTablesFallback:
    def test_auto_add_join_uses_dim_source_tables(self):
        ...
        ti = _make_table_info(dim_source_tables={"dim_product": "cat.sch.dim_product"})
        jd = MagicMock()
        jd.detect.return_value = []
        jd.detect_fact_joins.return_value = []
        jd.get_dim_dimensions.return_value = []
...
class TestProcessTableScanDataColumnNorm:
    def test_scan_data_mixed_case_alias_normalized(self):
        ...
        scan_info = MagicMock()
        scan_info.native_sql = (
            "SELECT Region AS Region, SUM(Amount) AS Amount FROM raw.t GROUP BY Region"
        )
        scan_info.m_steps = []
        scan_info.pbi_columns = {}
        scan_info.storage_mode = "DirectQuery"
...
class TestProcessTableCrossTableTranslated:
            ...
            def translate(self, measure, table_key, trivial_only=False):
                if measure.get("measure_name") == "cross_measure":
                    result = TranslationResult(
                        measure_name="cross_measure",
                        original_name="Cross",
                        sql_expr="SUM(other_fact.amount)",
                        is_translatable=True,
                        skip_reason="",
                        dax_expression="CROSS()",
                        confidence="high",
    ...
    def test_cross_table_category_appended(self):
        ...
        ctx = _make_context(
            ...
            join_detector=jd,
            cross_table_measures=cross_table_measures,
        )
        dax = [
            {
                "measure_name": "cross",
...
class TestProcessTableWindowOrderOverride:
    def test_window_order_overridden_when_date_key(self):
        ...
        class MockTranslator(DaxTranslator):
            def translate(self, measure, table_key, trivial_only=False):
                return TranslationResult(
                    measure_name="py_revenue",
                    original_name="PY Revenue",
                    sql_expr="SUM(source.revenue)",
                    is_translatable=True,
                    skip_reason="",
                    dax_expression="",
...
class TestProcessTableSwitchIdentityRatio:
    def test_identity_ratio_in_translated_removed(self):
        ...
        ctx = _make_context(
            ...
            config={
                "switch_decompositions": {
                    "fact_test": [{"name": "identity", "raw_expr": "SUM(source.x)"}]
                }
            },
        )
        dax = [
...
class TestProcessTableScanDataMSteps:
    def test_scan_data_with_m_steps(self):
        ...
        scan_info.m_steps = [
            MStep(step_type="SelectRows", raw_expression="each [status] = 1")
        ]
...
class TestProcessTableUnionArmLoss:
    def test_union_arm_loss_fallback(self):
        ...
        with patch.object(
            MTransformFolder,
            "fold",
            return_value="SELECT region, SUM(a) AS a FROM r.t1 GROUP BY region",
        ):
            ctx = _make_context(scan_data={"fact_test": scan_info})
            spec = _run_process_table("fact_test", ti, [], ctx)
        assert spec is not None
...
class TestProcessTableScanDataColumnsAdded:
    def test_new_columns_from_source_sql_added_as_dimensions(self):
        ...
        ti = _make_table_info(
            group_by_columns=["region"]
        )  # 'status' not in group_by_columns
        ctx = _make_context(scan_data={"fact_test": scan_info})
        spec = _run_process_table("fact_test", ti, [], ctx)
        dim_names = [d["name"] for d in spec.dimensions]
...
class TestIsRealSwitchDecomp:
    ...
    def test_real_raw_expr_is_real(self):
        entry = {
            "name": "ratio",
            "raw_expr": "SUM(source.a) / NULLIF(SUM(source.b), 0)",
        }
...
class TestReferencedByPopulation:
    ...
    def test_local_fallback_when_config_absent(self):
        ...
        dax = [
            ...
            {
                "measure_name": "derived",
                "dax_expression": "[Base] * 2",
