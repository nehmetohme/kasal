...
class TestConfigGeneratorToolExtended:
    ...
    def test_cache_loaded_populates_measures_and_relationships(self):
        ...
        with (
            patch(
                "src.services.powerbi.semantic_model_cache.PowerBISemanticModelCacheService",
                return_value=mock_svc,
            ),
            patch(
                "src.services.tools.metric_view_utils.utils.run_async",
    ...
    def test_join_key_map_built_from_relationships(self):
        ...
        result = tool._run(
            measures_json="[]", mquery_json=mquery, relationships_json=rels
        )
        parsed = json.loads(result)
    ...
    def test_column_overrides_skipped_when_no_table_match(self):
        ...
        measures = json.dumps(
            [
                {
                    "measure_name": "M",
                    "proposed_allocation": "other_table",
    ...
    def test_mapping_only_tables(self):
        ...
        measures = json.dumps(
            [
                {
                    "measure_name": "M",
                    "proposed_allocation": "external_table",
        ...
        assert (
            "my_cat.my_sch.external_table"
            in mapping_only["external_table"]["source_table"]
        )
    ...
    def test_unassigned_excluded_from_mapping_only(self):
        ...
        result = tool._run(measures_json=measures, mquery_json="[]")
        parsed = json.loads(result)
        assert "__unassigned__" not in parsed["proposed_config"].get(
            "mapping_only_tables", {}
        )
    ...
    def test_get_helper_kwarg_priority(self):
        ...
        result = tool._run(measures_json="[]", mquery_json="[]", catalog="kwarg_cat")
        parsed = json.loads(result)
        assert "proposed_config" in parsed
...
class TestConfigGeneratorMeasureResolutions:
    def test_measure_resolutions_cannot_resolve(self):
        ...
        measures = json.dumps(
            [
                {
                    "measure_name": "BaseMetric",
                    "proposed_allocation": "fact",
                    "dax_expression": "SUM(fact[amount])",
                },
                {
                    "measure_name": "DerivedMetric",
                    "proposed_allocation": "fact",
                    "dax_expression": "CALCULATE([BaseMetric])",
                },  # needs measure_resolutions
            ]
        )
        mquery = json.dumps(
            [
...
class TestConfigGeneratorFilterSetsFromIn:
    ...
    def test_measure_resolutions_loop_finds_candidate(self):
        ...
        measures = json.dumps(
            [
                {
                    "measure_name": "TotalRevenue",
                    "proposed_allocation": "fact",
                    "dax_expression": "SUM(fact[amount])",
                },  # This IS in measures list
                {
                    "measure_name": "RevShare",
                    "proposed_allocation": "fact",
                    "dax_expression": "CALCULATE([TotalRevenue])",
                },  # Cannot resolve [TotalRevenue]
            ]
        )
        mquery = json.dumps(
            [
