...
from src.services.tools.mcp_adapter import (
    MCPAdapter,
    MCPTool,
...
class TestMCPAdapter:
    ...
    async def test_initialize_success(self, server_params):
        ...
        with patch.object(
            adapter, "_get_authentication_headers", new_callable=AsyncMock
        ) as mock_auth:
            with patch.object(
                adapter, "_discover_tools_with_mcp_client", new_callable=AsyncMock
            ) as mock_discover:
...
class TestConvertParameters:
    ...
    def test_convert_number(self, adapter):
        ...
        adapter._tool_schemas["tool"] = {"properties": {"val": {"type": "number"}}}
    ...
    def test_convert_integer(self, adapter):
        ...
        adapter._tool_schemas["tool"] = {"properties": {"val": {"type": "integer"}}}
    ...
    def test_convert_boolean_true(self, adapter):
        ...
        adapter._tool_schemas["tool"] = {"properties": {"flag": {"type": "boolean"}}}
    ...
    def test_convert_array_from_json_string(self, adapter):
        ...
        adapter._tool_schemas["tool"] = {"properties": {"items": {"type": "array"}}}
    ...
    def test_convert_object_from_json_string(self, adapter):
        ...
        adapter._tool_schemas["tool"] = {"properties": {"config": {"type": "object"}}}
    ...
    def test_string_type_passthrough(self, adapter):
        ...
        adapter._tool_schemas["tool"] = {"properties": {"name": {"type": "string"}}}
    ...
    def test_unknown_type_passthrough(self, adapter):
        ...
        adapter._tool_schemas["tool"] = {"properties": {"x": {"type": "custom"}}}
    ...
    def test_no_type_in_schema(self, adapter):
        ...
        adapter._tool_schemas["tool"] = {"properties": {"x": {}}}
        result = adapter._convert_parameters("tool", {"x": "val"})
        assert result == {"x": "val"}

    def test_skip_none_values(self, adapter):
        """None values are skipped."""
        adapter._tool_schemas["tool"] = {"properties": {"a": {"type": "string"}}}
    ...
    def test_multiple_params_mixed(self, adapter):
        ...
        result = adapter._convert_parameters(
            "tool",
            {
                "count": "5",
                "name": "test",
...
class TestSPNFallbackHeaders:
    ...
    async def test_returns_headers_when_valid_context(self, adapter_with_group):
        ...
        with patch(
            ...
            mock_get_auth.assert_called_once_with(
                user_token=None, group_id="test-group-123"
            )
...
class TestSPNFallbackDiscovery:
    ...
    async def test_spn_fallback_streamable_http_success(self, databricks_spn_params):
        ...
        with (
            patch.object(
                adapter, "_discover_via_streamable_http", new_callable=AsyncMock
            ) as mock_sh,
            patch.object(
                adapter, "_discover_via_sse", new_callable=AsyncMock
            ) as mock_sse,
            patch.object(
                adapter, "_get_spn_fallback_headers", new_callable=AsyncMock
            ) as mock_spn,
        ):
        ...
        assert len(result) == 1
        assert adapter._spn_fallback_headers == {
    ...
    async def test_spn_fallback_sse_success(self, databricks_spn_params):
        ...
        assert len(result) == 1
        assert adapter._spn_fallback_headers == {
            "Authorization": "Bearer spn-token-xyz"
        }
        assert adapter._transport == "sse"
    ...
    async def test_spn_fallback_triggered_for_databricks_obo_auth(self):
        ...
        obo_error = self._make_403_error()
        spn_tools = [
            {"name": "t1", "description": "d", "mcp_tool": Mock(), "input_schema": {}}
        ]
...
class TestExtractErrorSummaryEdgeCases:
    ...
    def test_response_text_raises_exception(self):
        ...
        type(resp).text = property(
            lambda self: (_ for _ in ()).throw(RuntimeError("decode error"))
        )
        exc.response = resp
        result = _extract_error_summary(exc)
        assert "HTTP 500" in result
...
class TestLogExceptionGroupEdgeCases:
    ...
    def test_sub_exception_response_text_raises(self):
        ...
        resp = Mock()
        type(resp).text = property(
            lambda self: (_ for _ in ()).throw(RuntimeError("decode"))
        )
...
class TestHardDeadlines:
    ...
    async def test_execution_times_out_and_is_retried_as_transient(self):
        ...
        with patch.object(
            adapter, "_execute_with_transport_unbounded", side_effect=hang
        ):
            with pytest.raises(asyncio.TimeoutError):
                await adapter.execute_tool("tool", {})
...
class TestMcpErrorShortCircuit:
    ...
    async def test_sse_mcp_error_is_surfaced_too(self):
        ...
        with (
            ...
            mock_sh.side_effect = ConnectionError(
                "transport down"
            )  # genuine transport issue
