"""
Unit tests for services/tools/custom/powerbi_auth_utils.py
...
@pytest.mark.skipif(
    not HAS_AUTH_UTILS,
    reason="powerbi_auth_utils module not found or different structure",
)
class TestPowerBIAuthUtils:
    ...
    async def test_get_access_token_service_principal_success(self):
        ...
        with patch("httpx.AsyncClient.post", new_callable=AsyncMock) as mock_post:
            ...
            token = await get_access_token_service_principal(
                tenant_id="tenant123", client_id="client456", client_secret="secret789"
            )
        ...
        with patch("httpx.AsyncClient.post", new_callable=AsyncMock) as mock_post:
            ...
            with pytest.raises(Exception) as exc_info:
                await get_access_token_service_principal(
                    tenant_id="tenant123", client_id="invalid", client_secret="invalid"
                )

            assert (
                "401" in str(exc_info.value) or "invalid" in str(exc_info.value).lower()
            )
    ...
    async def test_get_access_token_service_account_invalid_credentials(self):
        ...
        with patch("httpx.AsyncClient.post", new_callable=AsyncMock) as mock_post:
            ...
            assert (
                "400" in str(exc_info.value) or "invalid" in str(exc_info.value).lower()
            )
    ...
    async def test_validate_access_token_expired(self):
        ...
        with patch("httpx.AsyncClient.get", new_callable=AsyncMock) as mock_get:
            mock_get.return_value = Mock(
                status_code=401, json=Mock(return_value={"error": "token_expired"})
            )
    ...
    async def test_refresh_access_token_failure(self):
        ...
        with patch("httpx.AsyncClient.post", new_callable=AsyncMock) as mock_post:
            mock_post.return_value = Mock(
                status_code=400, json=Mock(return_value={"error": "invalid_grant"})
            )
