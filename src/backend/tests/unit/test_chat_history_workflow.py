...
class TestChatHistoryServiceUnit:
    ...
    @pytest.mark.asyncio
    async def test_save_message_without_group_context(
        self, chat_history_service, mock_repository
    ):
    ...
    @pytest.mark.asyncio
    async def test_save_message_with_confidence_none(
        self, chat_history_service, mock_repository, group_context
    ):
    ...
    async def test_get_chat_session_success(
        ...
        result = await chat_history_service.get_chat_session(
            session_id="session-456", page=0, per_page=50, group_context=group_context
        )

        assert len(result) == 1
        assert isinstance(result[0], ChatHistoryResponse)
        mock_repository.get_by_session_and_group.assert_called_once_with(
            session_id="session-456", group_ids=["group-123"], page=0, per_page=50
        )

    @pytest.mark.asyncio
    async def test_get_chat_session_no_group_context(
        self, chat_history_service, mock_repository
    ):
        """Test chat session retrieval without group context returns empty list."""
        result = await chat_history_service.get_chat_session(
            session_id="session-456", group_context=None
        )
    ...
    @pytest.mark.asyncio
    async def test_get_chat_session_invalid_group_context(
        self, chat_history_service, mock_repository
    ):
        ...
        result = await chat_history_service.get_chat_session(
            session_id="session-456", group_context=invalid_context
        )
    ...
    async def test_get_user_sessions_success(
        ...
        result = await chat_history_service.get_user_sessions(
            user_id="user@company.com", page=0, per_page=20, group_context=group_context
        )

        assert len(result) == 1
        assert isinstance(result[0], ChatHistoryResponse)
        mock_repository.get_user_sessions.assert_called_once_with(
            user_id="user@company.com", group_ids=["group-123"], page=0, per_page=20
        )

    @pytest.mark.asyncio
    async def test_get_user_sessions_no_group_context(
        self, chat_history_service, mock_repository
    ):
    ...
    @pytest.mark.asyncio
    async def test_get_group_sessions_success(
        self, chat_history_service, mock_repository, group_context
    ):
        ...
        result = await chat_history_service.get_group_sessions(
            page=0, per_page=20, user_id="user@company.com", group_context=group_context
        )

        assert result == expected_sessions
        mock_repository.get_sessions_by_group.assert_called_once_with(
            group_ids=["group-123"], user_id="user@company.com", page=0, per_page=20
        )

    @pytest.mark.asyncio
    async def test_get_group_sessions_no_user_filter(
        self, chat_history_service, mock_repository, group_context
    ):
        ...
        mock_repository.get_sessions_by_group.assert_called_once_with(
            group_ids=["group-123"], user_id=None, page=0, per_page=20
        )

    @pytest.mark.asyncio
    async def test_get_group_sessions_no_group_context(
        self, chat_history_service, mock_repository
    ):
        """Test group sessions retrieval without group context returns empty."""
        result = await chat_history_service.get_group_sessions(
            page=0, per_page=20, group_context=None
        )
    ...
    async def test_delete_session_success(
        ...
        result = await chat_history_service.delete_session(
            session_id="session-456", group_context=group_context
        )

        assert result is True
        mock_repository.delete_session.assert_called_once_with(
            session_id="session-456", group_ids=["group-123"]
        )
    ...
    async def test_delete_session_not_found(
        ...
        result = await chat_history_service.delete_session(
            session_id="nonexistent-session", group_context=group_context
        )

        assert result is False

    @pytest.mark.asyncio
    async def test_delete_session_no_group_context(
        self, chat_history_service, mock_repository
    ):
        """Test session deletion without group context."""
        result = await chat_history_service.delete_session(session_id="session-456")

        assert result is False
        mock_repository.delete_session.assert_not_called()

    @pytest.mark.asyncio
    async def test_count_session_messages_success(
        self, chat_history_service, mock_repository, group_context
    ):
    ...
    @pytest.mark.asyncio
    async def test_count_session_messages_no_group_context(
        self, chat_history_service, mock_repository
    ):
    ...
    @pytest.mark.asyncio
    async def test_save_message_repository_exception(
        self, chat_history_service, mock_repository, group_context
    ):
    ...
    @pytest.mark.asyncio
    async def test_get_chat_session_repository_exception(
        self, chat_history_service, mock_repository, group_context
    ):
        """Test handling of repository exceptions during retrieval."""
        mock_repository.get_by_session_and_group.side_effect = Exception(
            "Database error"
        )
    ...
    @pytest.mark.asyncio
    async def test_save_message_with_complex_generation_result(
        self, chat_history_service, mock_repository, group_context
    ):
        ...
        complex_result = {
            ...
            "tasks": [{"id": "task-1", "name": "Research Task", "dependencies": []}],
            "metadata": {"created_at": "2023-01-01T00:00:00Z", "processing_time": 2.5},
        }
...
class TestChatHistoryWorkflowIntegration:
    ...
    @pytest.mark.asyncio
    async def test_multi_user_session_workflow(
        self, chat_history_service, mock_repository, group_context
    ):
        ...
        user1_result = await chat_history_service.get_user_sessions(
            user_id="user1@company.com", group_context=group_context
        )
        assert len(user1_result) == 2

        # Get user2 sessions
        user2_result = await chat_history_service.get_user_sessions(
            user_id="user2@company.com", group_context=group_context
        )
    ...
    async def test_pagination_workflow(
        ...
        page1_result = await chat_history_service.get_chat_session(
            session_id=session_id, page=0, per_page=10, group_context=group_context
        )
        assert len(page1_result) == 10

        # Get second page
        page2_result = await chat_history_service.get_chat_session(
            session_id=session_id, page=1, per_page=10, group_context=group_context
        )
        assert len(page2_result) == 5

    @pytest.mark.asyncio
    async def test_group_isolation_workflow(
        self, chat_history_service, mock_repository
    ):
        ...
        group1_messages = await chat_history_service.get_chat_session(
            session_id="shared-session-id", group_context=group1_context
        )
        ...
        group2_messages = await chat_history_service.get_chat_session(
            session_id="shared-session-id", group_context=group2_context
        )
