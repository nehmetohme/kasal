...
- **A development identity.** `run.sh` sets `LOCAL_DEV_AUTH=true`, so a request without an identity header runs as `dev@localhost`. If you start `uvicorn` yourself without it, every API call returns 401.
