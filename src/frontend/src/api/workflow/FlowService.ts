...
export class FlowService {
  static async generateFlow(prompt: string, model: string, currentCrewIds: string[], signal?: AbortSignal, onStarted?: (jobId: string) => void, sessionId?: string, mcpServers: string[] = [], tools: string[] = []): Promise<import('../../features/workflow/assistant/types').FlowDraft> {
    if (onStarted) return generateWithTrace<import('../../features/workflow/assistant/types').FlowDraft>('flow', { prompt, model, current_crew_ids: currentCrewIds, session_id: sessionId, mcp_servers: mcpServers, tools }, onStarted, signal);
    const response = await apiClient.post('/flows/generate', { prompt, model, current_crew_ids: currentCrewIds, session_id: sessionId, mcp_servers: mcpServers, tools }, { timeout: 180000, signal });
