...
import { useTabManagerStore, type TabData } from './tabManager';
...
beforeEach(() => {
  vi.clearAllMocks();
  useTabManagerStore.setState({ activeTabId: 'origin', tabs: [
  ...
  ].map(tab => ({ ...tab, createdAt: new Date(), lastModified: new Date() })) as TabData[] });
});
...
it('keeps the originating conversation and saved crew when navigation happens during refresh', async () => {
  ...
  useTabManagerStore.setState({ activeTabId: 'other' });
  await run;
